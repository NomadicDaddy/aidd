...
**Model**:
The specific model id a CLI/provider runs (e.g. defaults `model`, `initModel`, `codeModel`,
`auditModel`).
...
**Milestone**:
A named roadmap bucket (`MVP`, `v1.0`, `v2.0`) defined in `.aidd/roadmap.json` that groups Features;
every Feature directory must map to one. A roadmap axis, unrelated to Phase or Maturity stage.
...
**Director**:
The fleet-level advisor persona. It runs Cycles that emit Suggestions and also hosts a read-only
Chat. Configured by a **Director profile**.
_Avoid_: Director, planner, supervisor.
...
**Control panel**:
The bundled single-user web surface (`--web`) for operating the runtime. Intentionally excludes the
old multi-user `aidd-web` auth/RBAC/tenancy product surface.
