...
**`smoke:qc` pipeline (canonical source - do not re-transcribe here):** Read the authoritative,
ordered step list from the target repository's live smoke implementation. Spernakit uses
`scripts/smoke.json` under the `qc` mode; aidd exposes the gate through `scripts/smoke-qc.ts`, with
the step definitions in `scripts/lib/smoke-qc/steps.ts`. Never infer the active gate from a frozen
count or a prose summary. Compare the live steps to CI/package scripts and verify that each claimed
check is reachable.
