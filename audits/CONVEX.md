...
This audit targets `[react+convex]` applications (astrid.chat, keystrike, and future Convex apps). It deliberately does NOT apply to the Spernakit stack (Elysia + Drizzle + SQLite + Bun). Current workspace Convex apps use Bun, Vitest, `convex-test`, and Zod in the frontend. Follow each target's live `packageManager`, scripts, and lockfile; do not impose Spernakit's crawltest conventions on an independent Convex app.
...
| **Explicit Table IDs** | `ctx.db.get('posts', id)` and `ctx.db.patch('posts', id, value)`                            | Legacy table-less `ctx.db.get(id)` / `patch(id, value)`    |
| **TypeScript Version** | Workspace baseline 6.0.3; Convex minimum 5.0.3                                              | Below 5.0.3 or an unexplained workspace-version drift      |
| **ESLint**             | `@convex-dev/eslint-plugin` v4 recommended flat config                                      | Missing plugin or disabled recommended rules               |
...
6. **Explicit table IDs**: On Convex 1.31+, use table-name database APIs (`ctx.db.get('posts', id)`, `patch`, `replace`, and `delete`) so authorization and data-flow reviews can see the table at the call site
7. **TypeScript requirement**: Use the workspace baseline (currently 6.0.3) unless the target documents a compatibility constraint; anything below Convex's 5.0.3 floor is a failure
8. **ESLint enforcement**: Use `@convex-dev/eslint-plugin` v4's recommended flat config and investigate any explicit rule override
...
- **Database IDs**: Use explicit table names for `get`, `patch`, `replace`, and `delete` on Convex 1.31+
- **ESLint**: Enforce the v4 recommended rules, including function syntax, argument validators, explicit table IDs, index-backed queries, safe cron schedules, and acyclic schema imports
...
- **Environment**: Set secrets via `bunx convex env set` (per-deployment); `convex.json` is for build/runtime config only
...
3. [Quick Reference - Mandatory Rules](#quick-reference---mandatory-rules)
...
> - Database IDs: on Convex 1.31+, pass the table name explicitly to `get`, `patch`, `replace`, and `delete`
> - TypeScript: TS ≥ 5.0.3 and the workspace baseline where compatible; use validators for types, `WithoutSystemFields`, and generated `Doc/Id/*Ctx` (TypeScript page)
> - ESLint: Use `@convex-dev/eslint-plugin` v4's recommended flat config; inspect rather than silently overriding its recommended rules
...
> - Environment: Set secrets per deployment via `bunx convex env set`; NEVER in `convex.json` (no such field)
...
Run these commands before `bunx convex dev` to catch common issues. Use `rg` (ripgrep) for cross-platform compatibility; `grep -r` behaves differently on Windows/macOS/Linux shells.
...
export const transferOwnership = mutation({
	...
	handler: async (ctx, args) => {
		...
		const newOwner = await ctx.db.get('users', args.newOwnerId);
		if (!newOwner) {
			throw new ConvexError({
				code: 'RESOURCE_NOT_FOUND',
...
All Convex projects MUST use `@convex-dev/eslint-plugin` v4 with ESLint flat config. Use the
target's package manager; current workspace Convex apps use Bun:

```bash
# Install ESLint plugin
bun add --dev @convex-dev/eslint-plugin
```

```javascript
// eslint.config.js — spread the plugin's recommended flat-config array
...
The v4 recommended preset currently enables these six rules. Missing or disabled recommended rules
are findings unless the target documents and falsifies a specific exception:

1. **`no-old-registered-function-syntax`** - Requires object syntax for registered functions
2. **`require-argument-validators`** - Requires argument validators
3. **`explicit-table-ids`** - Requires table names in database operations
4. **`no-filter-in-query`** - Warns on inefficient database-query `.filter()` calls
5. **`no-top-of-hour-crons`** - Avoids scheduling cron work at peak `:00`
6. **`no-schema-import-cycle`** - Prevents schema-value import cycles

Two additional rules are available but are not in the recommended preset. Evaluate them for the
target; do not file a missing-rule finding solely because they are absent:

- **`import-wrong-runtime`** - Prevents Convex runtime files importing Node-runtime files
- **`no-collect-in-query`** - Prefers `.take()` or `.paginate()` when result size may grow
...
- **TypeScript**: current workspace baseline 6.0.3. Convex's minimum is 5.0.3; anything below that is a failure. A target below the workspace baseline needs an evidenced compatibility reason, not a generic "current stable" assertion.
- **SDK currency**: current published Convex SDK is 1.45.0. The workspace apps inspected on 2026-08-30 still declare 1.28/1.29 ranges, which predate explicit table IDs; record an upgrade finding and do not claim v1.31+ rules are already enforced until the target upgrades. Current `convex-test` declarations are 0.0.56; it is pre-1.0, so review changes carefully when upgrading.
...
- **Avoid N+1 reads**: Convex has no SQL joins, so batch related lookups with `Promise.all(ids.map(id => ctx.db.get('users', id)))` and build a `Map` for O(1) joins rather than awaiting `ctx.db.get` in a loop (as shown above). (Generic data-transfer/denormalization concerns are co-owned with DATABASE.)
...
> bunx convex env set CLERK_JWT_ISSUER_DOMAIN "https://clerk.your-app.com"
> bunx convex env list
> bunx convex env remove STRIPE_SECRET_KEY
> ```
>
...
bunx convex deploy --preview-create "feature-branch-name"

# ✅ Good: Pre-deployment checks (run locally before pushing)
# NOTE: Vitest / convex-test are intentional for the [react+convex] stack.
# Follow the target manifest; current workspace apps use Bun.
bunx tsc -p convex          # typecheck Convex functions
bun run lint                # ESLint with @convex-dev/eslint-plugin
bun run test                # convex-test + vitest
bunx convex deploy --prod
```
...
- **`bunx convex deploy --cmd "bun run build"`** runs the server deploy then the frontend build against the freshly-generated `_generated/` types; gate it behind `bunx tsc -p convex`, `bun run lint`, and `bun run test` (as above). Use per-branch preview deployments (`CONVEX_DEPLOY_KEY`) for isolated staging. Generic CI/CD/rollback practice is co-owned with the deployment audits.
...
3. **Environment**: Set `CLERK_JWT_ISSUER_DOMAIN` (for Convex) and `CLERK_WEBHOOK_SECRET` (for Svix verification) via `bunx convex env set`. Frontend needs `VITE_CLERK_PUBLISHABLE_KEY` in `.env.local` (frontend-only; never read by Convex server code).
...
- [ ] Convex SDK is at least 1.31 and every `get` / `patch` / `replace` / `delete` call uses an explicit table name
- [ ] `@convex-dev/eslint-plugin` v4 recommended config is spread into flat config; all six recommended rules are active
- [ ] TypeScript meets the 5.0.3 minimum and either matches the workspace baseline or has an evidenced compatibility exception
...
- An applicability result and installed-version inventory for Convex, the ESLint plugin,
  TypeScript, and `convex-test`.
- A report that covers every checklist section with `file:line` evidence and distinguishes
  recommended-preset failures from optional lint rules.
- An explicit-table-ID migration/upgrade finding when the installed SDK predates Convex 1.31.
...
- [Convex Package Versions](https://www.npmjs.com/package/convex?activeTab=versions)
