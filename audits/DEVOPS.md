...
- Runtime: Bun matches `package.json` `engines.bun` (currently `>=1.3.14`), Node matches `engines.node` (currently `>=24.0.0 <25.0.0`), `packageManager: bun@1.3.14`. Verify against `package.json` rather than memorizing the numbers; the version baseline moves.
...
Record: CI present (y/n), Dockerfile present (y/n), `engines.bun`/`engines.node`, the `qc` step set, and the branch-protection summary. These feed the [Applicability Gate](#applicability-gate).
...
- Confirm the Dockerfile base image is pinned (e.g. `oven/bun:1.3.14-alpine`, not `oven/bun:latest`)
...
- [ ] **High**: CI Bun version matches `package.json` `engines.bun` (currently `>=1.3.14`) and `packageManager`
