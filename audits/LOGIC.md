...
This audit is designed to be stack-agnostic, but the following notes adapt its generic rules to the
current **Spernakit v3** stack (React 19 + Vite 8 + Elysia + Drizzle ORM + TanStack Query + Zustand

- Bun). Read the live manifests before scoring; do not infer a release or runtime baseline from this
  summary.
