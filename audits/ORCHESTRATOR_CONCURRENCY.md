...
- The auto-cycle scheduler never stacks cycles.
...
- `backend/src/services/director/cycleScheduler.ts`: `checkAndRunScheduledCycle`, `lastCycleAt`
...
# The auto-cycle scheduler stack-guard
grep -n "checkAndRunScheduledCycle\|activeStages.size\|status, 'running'" backend/src/services/director/cycleScheduler.ts
...
The same logical work can be triggered through multiple entry points: `launch_suggestion`, `launch_run`, `run_cycle` (dispatcher at `director/chatAgentTools/dispatch.ts:109,124,147`), and the auto-cycle scheduler. The suggestion path is now guarded by an atomic compare-and-set claim; the cross-entry and scheduler windows below still warrant verification.
...
| `[ ]` | The auto-cycle scheduler does not stack with a manually-triggered `run_cycle`: `checkAndRunScheduledCycle` skips when `activeStages.size > 0` OR a `running` cycle row exists (`cycleScheduler.ts:45-51`)                                                                                                                                                                                                                                                                                                                                                                                                                  | Verify both guards; but note the check and the subsequent `startCycle` (`cycleScheduler.ts:59`) are not atomic; a manual `run_cycle` between the check and the start could stack. Document the window                                                                                                                                                                                                                                   |
| `[ ]` | `run_cycle` itself cannot start two cycles from two simultaneous chat tool calls in the same session                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Trace `startCycle`; if there is no in-flight guard, two cycles spawn                                                                                                                                                                                                                                                                                                                                                                    |

---

## 3. Max-Concurrency Bound

`launchRun` (`run/launch.ts:65`) enforces the bound through `insertRunIfUnderCeiling` (`run/launch.ts:193-223`), a **single** worker transaction that counts non-terminal runs and inserts the new row only if the count is under the ceiling (`db/commands/runCeiling.ts:19-59`). The old standalone `countNonTerminalRuns` gate (a separate read before the insert) is gone; `countNonTerminalRuns` (`run/queries.ts:72`) now serves only read-path callers. The command rejects with `{ kind: 'rejected', scope }` and `launchRun` throws (`run/launch.ts:224-230`).
...
5. **Auto-cycle vs manual `run_cycle` stack**: with auto-cycle enabled, time a manual `run_cycle` to land between `checkAndRunScheduledCycle`'s running-cycle check (`cycleScheduler.ts:46-51`) and its `startCycle` (`cycleScheduler.ts:59`). Confirm whether two cycles stack (the check-then-start is not atomic).
...
- [ ] Auto-cycle scheduler does not stack with a manual `run_cycle`
