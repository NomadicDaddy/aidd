...
> **Spernakit note**: `<Activity>` is available as a stable export in React 19.2+ (the current
> Spernakit manifest pins `react` 19.2.8). It takes a `mode="visible" | "hidden"` prop: a `hidden`
> Activity unmounts its effects but preserves component state and DOM, so reopening it restores
> the prior state without a remount. CSS `display: none` with state lifted to Zustand remains a
> valid alternative when you also need the DOM to stay laid out.
...
> **Spernakit note**: `useEffectEvent` is stable as of React 19.2 (the current Spernakit manifest
> pins `react` 19.2.8), so it is the recommended API here. The `useLatest` pattern (see 8.4)
> remains only as a legacy fallback for codebases on older React.
...
> **Spernakit note**: `useEffectEvent` is stable as of React 19.2 (the current Spernakit manifest
> pins `react` 19.2.8), so it is the **primary recommended** API for stable callback refs. Use it
> instead of the `useLatest` pattern (8.4), which is retained only as a legacy fallback for older
> React.
