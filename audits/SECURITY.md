...
Note: API-key identity is resolved by the global `authPlugin` from `X-API-Key`; downstream role
guards cap the effective role to the key's scope. There is no separate API-key pipeline stage or
per-route API-key guard. `clientIp` must be first so downstream plugins (audit, rate limiting) can
resolve the real client IP via `getClientIp(request)`.
