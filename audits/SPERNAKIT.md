...
| ----------------- | --------------------------------------------- | -------------------------------------------- |
| Stack Reference   | `docs/template/STACK.md`                      | Core architecture and commands               |
| Project Overrides | `.aidd/project.md`                            | Technology-specific constraints              |
...
**Source of truth:** the audited Spernakit repository's local `spernakit.psd1` (`ExpectedConfigs`
hashtable - app slugs, ports, names, spernakit versions).
...
| `[ ]` | Plugin pipeline: Client IP > ReqID > Logger > CORS > SecHeaders > Auth > PwdChange > CSRF > RateLimit > AuthRateLimit > Workspace > Audit; `authPlugin` resolves JWT and API-key identity      | Review plugin order against template source and STACK.md; flag documentation drift if they disagree                                   |
...
| `[ ]` | Plugin order: Client IP → ReqID → Logger → CORS → SecHeaders → Auth → PwdChange → CSRF → RateLimit → AuthRateLimit → Workspace → Audit; API-key identity is part of Auth | Verify registration against template source; flag STACK/DEVELOPMENT drift separately |
...
| `[ ]` | API-key identity is resolved in `authPlugin`; role guards cap the effective role to the key scope | Verify `plugins/auth.ts`, `plugins/authRequest.ts`, and `guards/role.ts` |
...
| ----------------- | ------ | -------------------------------------------------------------------------------------------------------- |
| Bun 1.4.2+        | `[ ]`  | `bun --version` (matches `packageManager`/`engines` in package.json)                                     |
...
| `[ ]` | Hooks live under `frontend/src/hooks/` with at most one domain-directory level      | Do not score the 2-vs-3 grouping threshold while STACK.md and DEVELOPMENT.md disagree; file the documentation drift once instead |
