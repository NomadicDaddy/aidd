...
> **Validated against Spernakit v3.11.x-lts (React 19, Vite 8, Bun 1.3.x).**
...
| Deprecation markers / `@deprecated` in `dist/` or other compiled build output | Build artifacts, not source. The single TRUE positive across 20 historical reports was a deprecated method in `dist/`. Only score source; never file findings against generated/compiled output (see [Exclusions](#exclusions)). |
