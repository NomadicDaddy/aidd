...
Based on
[Vercel's Web Interface Guidelines](https://github.com/vercel-labs/web-interface-guidelines/blob/main/command.md),
synchronized against the current `command.md` source on 2026-08-30. Companion audit to
[REACT_BEST_PRACTICES.md](./REACT_BEST_PRACTICES.md) and
[COMPOSITION_PATTERNS.md](./COMPOSITION_PATTERNS.md).

Comprehensive UI quality and accessibility guide covering all 103 upstream checks across 17
...
18. [Audit Checklist](#audit-checklist)
19. [Report Template](#report-template)
...
### 1.11 Meaningful Media Alternatives

Meaningful video and audio need captions and, when visual content carries information not available
from the audio, a transcript or audio description. Decorative media must be hidden from assistive
technology instead of being announced as unexplained content.

### 1.12 Accessible Media Controls

Custom media controls must be keyboard-operable, visibly focused, and accessibly named. Native
controls are preferred unless the custom implementation preserves equivalent semantics and input
support.

### 1.13 Color Contrast (Manual Verification)
...
### 2.5 Focus Must Not Be Obscured

Sticky headers, drawers, cookie banners, and other overlays must not cover the currently focused
element. Verify keyboard navigation at each supported viewport, including after scrolling and route
changes.
...
### 4.7 Long Autoplay Motion Needs a Control

Autoplaying motion that lasts more than five seconds must provide a pause, stop, or hide control.
The control must be keyboard accessible and preserve the user's choice.

### 4.8 Decorative Loops Respect Reduced Motion

Muted decorative loops must stop, pause, or render a still fallback when
`prefers-reduced-motion: reduce` is active. Muting audio alone does not make perpetual motion
accessible.
...
### 8.7 Prefer Video Over Animated GIFs

Use an efficient video format for animated content rather than large GIF files, and provide a
descriptive still image or equivalent alternative when the animation conveys meaning.

### 8.8 Short Decorative Loops Need Browser Fallbacks

For short nonessential loops, provide a Safari-compatible H.264 source plus a poster or still
fallback, and suppress motion under `prefers-reduced-motion`.
...
### 10.5 Gestures Need Non-Gesture Alternatives

Swipe, drag, pinch, and path-based gestures must have an equivalent click, tap, or keyboard action.
Do not make a gesture the only way to complete an operation.

### 10.6 autoFocus Sparingly
...
| Animated GIF                  | Large or meaningful motion delivered only as a GIF       | Use video plus a still alternative |
| Gesture-only interaction      | Swipe/drag/pinch is the only available control           | Add click, tap, and keyboard paths |
...
- [ ] **High**: Meaningful media has captions and any needed transcript/audio description
- [ ] **High**: Custom media controls are keyboard-operable, visibly focused, and named
- [ ] **High (manual)**: Text/controls meet WCAG AA contrast (4.5:1 normal, 3:1 large/UI) in both light and dark themes - see §1.13
...
- [ ] **High**: Sticky overlays never obscure the focused element at supported viewports
...
- [ ] **High**: No gesture-only interaction path
...
- [ ] **Medium**: Autoplay motion over five seconds has pause, stop, or hide controls
- [ ] **Medium**: Decorative loops stop or show a still under reduced motion
...
- [ ] **Medium**: Every gesture has an equivalent click, tap, or keyboard action

**Media Performance**

- [ ] **Medium**: Animated content uses efficient video instead of GIF where practical
- [ ] **Medium**: Short decorative loops include H.264, poster/still, and reduced-motion fallbacks
...
- A report covering all 103 current upstream checks and the local color-contrast adaptation.
- Manual browser evidence for contrast, focus occlusion, media controls, gestures, and motion where
  static source inspection cannot establish the behavior.
- `file:line` evidence for every source finding and exact page/viewport evidence for rendered issues.
- Valid `feature.json` records for confirmed actionable findings, using the shared severity mapping.

## References

- [Vercel Web Interface Guidelines](https://github.com/vercel-labs/web-interface-guidelines/blob/main/command.md) - 103 upstream checks reviewed 2026-08-30
