...
/**
 * In a compiled standalone binary the worker module is embedded as an extra
 * `bun build --compile` entrypoint (see scripts/build-standalone.ts) and only
 * resolves by its bundle-root-relative path — the entrypoints share
 * `backend/src` as their common ancestor, so the worker lands at
 * `db/worker/dbWorker.js` inside the bundle. The `import.meta.url` form, which
 * works from source, resolves to a path that does not exist in the bundle and
 * the Worker dies with an async ModuleNotFound error instead.
 */
function dbWorkerSpecifier(): string {
	return isCompiledBinary()
		? './db/worker/dbWorker.ts'
		: new URL('./worker/dbWorker.ts', import.meta.url).href;
}
...
export async function createWorkerWebDatabase(
	...
	// Without this, a worker that fails to load at all (e.g. its module is
	// missing from a compiled bundle) never posts ready/init-error and startup
