...
export function reconcileInvocationFromRun(
	...
	tx.update(invocationEvents)
		...
		.where(eq(invocationEvents.runId, runId))
		.run();
	// The run row learns its driver from the CLI after launch; copy it onto the run's own telemetry
	// row (nested skill rows carry their own resource hash). A launch-time resourceName of
	// 'audit-all' names the batch the operator asked for and is kept; a named subset is refined to
	// the code-point-sorted list the CLI recorded.
	const runRows = and(
		eq(invocationEvents.runId, runId),
		eq(invocationEvents.resourceType, 'run'),
	);
	if (run.driverSha256) {
		tx.update(invocationEvents).set({ resourceSha256: run.driverSha256 }).where(runRows).run();
	}
	if (run.driverKind === 'audit' && run.driverId) {
		tx.update(invocationEvents)
			.set({ resourceName: run.driverId })
			.where(and(runRows, ne(invocationEvents.resourceName, 'audit-all')))
			.run();
	}
