...
export function updateProjectPathReferences(
	...
	tx.update(invocationEvents)
		.set({ projectName, projectPath: destinationPath })
		.where(eq(invocationEvents.projectPath, sourcePath))
		.run();
	tx.update(scheduledTaskProjects)
		.set({ projectPath: destinationPath })
		.where(eq(scheduledTaskProjects.projectPath, sourcePath))
		.run();
}
...
export function projectPathMatches(column: AnySQLiteColumn, projectPath: string): SQL {
