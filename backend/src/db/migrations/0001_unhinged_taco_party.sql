-- 0001: Initial aidd web control panel schema (squashed).
--
-- This is the fresh-install baseline for the current schema. All later
-- migrations through 0015 were folded into this file.
...
CREATE TABLE pipeline_sessions (
	...
	CONSTRAINT ck_pipeline_sessions_launch_backend
		CHECK (launch_backend IS NULL OR launch_backend IN ('claude-code','codex','grok','kilocode','lmstudio','native','ollama','openai','opencode')),
...
CREATE TABLE director_profiles (
	...
	CONSTRAINT ck_director_profiles_backend
		CHECK (backend IN ('claude-code','codex','grok','kilocode','lmstudio','native','ollama','openai','opencode')),
...
CREATE TABLE invocation_events (
	...
	CONSTRAINT ck_invocation_events_backend
		CHECK (backend IS NULL OR backend IN ('claude-code','codex','grok','kilocode','lmstudio','native','ollama','openai','opencode'))
);
...
CREATE INDEX idx_pipeline_sessions_project_path
	ON pipeline_sessions(project_path);
CREATE INDEX idx_pipeline_sessions_recipe_id ON pipeline_sessions(recipe_id);
CREATE INDEX idx_pipeline_sessions_started_at
...
CREATE INDEX idx_invocation_events_started_at
	ON invocation_events(started_at DESC);
...
CREATE INDEX idx_invocation_events_project_path
	ON invocation_events(project_path);
