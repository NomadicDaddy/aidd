 ...
 *
 * The standalone artifact copies only `frontend/dist` (not `frontend/src`) next to the binary, so
 * the absence of `frontend/src` is the discriminator for the production path — there the dist is
 * intentionally prebuilt and the guard stays silent.
 */
...
export async function evaluateFrontendStaleness(rootDir: string): Promise<FrontendStalenessResult> {
	...
	// No source tree means this is the standalone/production artifact: dist is prebuilt and intended.
