...
export function createAuditsRoutes(context: WebContext) {
	...
	return new Elysia({ prefix: '/api/v1/audits' })
		...
		.post(
			'/launch',
			async ({ body }) => ({
				// Stamped here rather than taken from the body: a client cannot be trusted to say
				// who started something, and a request that reached this route came from a person
				// on the audits page. The scheduler's audit sweeps go through
				// `launchAuditsForPaths` and carry the occurrence's own answer.
				result: await auditService().launchAudits({ ...body, initiator: 'operator' }),
			}),
