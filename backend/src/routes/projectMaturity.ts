...
import { normalizeAuditName } from '../services/audit/auditHelpers.ts';
	...
	switch (invocation.kind) {
		case 'audit': {
			...
			const run = await context.runService.launchRun(
				{
					auditNames: [normalizeAuditName(normalized.auditName)],
					mode: 'audit',
					projectDir,
					...(launchTarget.backend !== undefined
						? { backend: launchTarget.backend }
						: {}),
