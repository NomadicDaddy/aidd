...
import type { RunOutputWindowRequest } from '../services/run/output.ts';
...
const safeBackendArg = t.String({ pattern: SAFE_BACKEND_ARG_PATTERN });
...
async function launchTrackedRun(context: WebContext, input: RunLaunchRequest, mode: WebRunMode) {
	if (input.auditNames) input = { ...input, auditNames: normalizeAuditNames(input.auditNames) };
	const run = await context.runService.launchRun(input, { initiator: 'operator' });
...
function outputWindow(query: {
	endByte?: number;
	startByte?: number;
}): RunOutputWindowRequest | undefined {
	if (query.startByte === undefined && query.endByte === undefined) return undefined;
	if (query.startByte === undefined || query.endByte === undefined) {
		throw new HttpError('Run output requests must include both startByte and endByte.', 400);
	}
	return { endByte: query.endByte, startByte: query.startByte };
}
...
export function createRunsRoutes(context: WebContext) {
	...
	return new Elysia({ prefix: '/api/v1/runs' })
		...
		.get(
			'/:id/output',
			async ({ params, query }) =>
				await context.runService.readOutput(params.id, outputWindow(query)),
			{
				params: t.Object({ id: t.String() }),
				query: t.Object({
					endByte: t.Optional(t.Numeric()),
					startByte: t.Optional(t.Numeric()),
				}),
			},
		)
		.get(
