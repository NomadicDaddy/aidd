...
import { applicationRootsValidationError } from '../services/settings/validation.ts';
...
const nullableAgentBaseUrl = t.Union([
	t.String({
		format: 'uri',
...
const directAiBody = t.Object({
	...
	baseUrl: t.Optional(nullableAgentBaseUrl),
...
export const settingsConfigBody = t.Object({
	...
	directorAutoLaunchAllowedRecipes: t.Optional(t.Array(t.String({ minLength: 1 }))),
	directorAutoLaunchEnabled: t.Optional(t.Boolean()),
	directorAutoLaunchMaxPerCycle: t.Optional(t.Number({ minimum: 1 })),
	directorAutoLaunchMaxRank: t.Optional(t.Number({ minimum: 1 })),
	directorAutoLaunchRiskCeiling: t.Optional(
...
export function createSettingsRoutes(
	...
	return new Elysia({ prefix: '/api/v1/settings' })
		...
		.post(
			'/application-roots/validate',
			({ body }) => {
				const error = applicationRootsValidationError(body.applicationRoots);
				if (error) throw new HttpError(error, 400);
				return { valid: true };
			},
			{ body: t.Object({ applicationRoots: applicationRootsBody }) },
		)
		.get(
			'/cli-status',
			async ({ query }) => ({
