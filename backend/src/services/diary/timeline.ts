import type { WebRunOutcomeInput, WebRunOutcomeStatus } from 'aidd-shared/runs/outcome';
...
// Union activity sources here so the server can drop a run duplicated by a skill invocation.
...
// Match projectPathFilter in run/historyQueries.ts so Diary and Runs scope paths identically.
...
async function queryRuns(
	...
	return rows.map((row) => ({
		...
		runOutcome: {
			...
			status: row.status as WebRunOutcomeStatus,
