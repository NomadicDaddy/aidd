...
export interface DiaryEntryDto {
	...
	date: string;
	/**
	 * Modification time of the entry's markdown file, in epoch milliseconds.
	 *
	 * `date` is the calendar day the entry is about and carries no time of day, so it cannot order
	 * an entry against the runs of that same day. Consumers that interleave entries with other
	 * dated events -- the project History timeline -- sort on this instead.
	 */
	fileMtimeMs: number;
