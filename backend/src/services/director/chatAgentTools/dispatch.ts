		...
		case 'launch_suggestion': {
			...
			return {
				action: {
					...
					status: 'ok',
					suggestionId,
					// Named for what happened, not for what was asked. A suggestion the Director's
					// own launcher had already claimed is running, but this call did not start it.
					summary: launched.alreadyRunning
						? `Suggestion ${suggestionId} was already running; nothing new was started`
						: `Launched suggestion ${suggestionId}`,
					tool: name,
					...(launched.pipelineSessionId
						? { pipelineSessionId: launched.pipelineSessionId }
						: {}),
					...(launched.runId ? { runId: launched.runId } : {}),
				},
...
async function dispatchFileTool(
	...
	const projectId = requireString(args, 'projectId');
	const cwd = await ctx.resolveDiscoveredProject(projectId);
