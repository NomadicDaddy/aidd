...
export interface ChatAgentToolContext {
	...
	/**
	 * Launches on behalf of the person who asked. `alreadyRunning` marks the case where something
	 * else — the Director's own auto-launcher — had already claimed it, so the agent can report what
	 * is true rather than taking credit for a run it did not start.
	 */
	launchSuggestion(id: string): Promise<{
		alreadyRunning?: true;
		id: string;
