...
import { normalizeLaunched } from 'aidd-shared';
...
// Written as one JSON document by the auto-launcher and read back whole. A row from before the
// column existed is NULL, and so is a cycle the launcher never considered; both mean the same thing
// to a reader, which is why neither is invented into an empty record here. An unparseable value is
// treated the same way rather than thrown: a cycle's history must stay readable. Launch entries are
// narrowed on the way out, because a row older than recipe auto-launch has no discriminant.
function parseAutoLaunch(value: null | string | undefined): DirectorCycleAutoLaunch | null {
	if (!value) return null;
	try {
		const parsed = JSON.parse(value) as DirectorCycleAutoLaunch;
		if (!Array.isArray(parsed.launched) || !Array.isArray(parsed.skipped)) return null;
		return { ...parsed, launched: parsed.launched.map(normalizeLaunched) };
	} catch {
		return null;
	}
}
...
export async function persistCycleResult(
	...
	// The last point where both ends are in hand: the ranked work list this cycle was given and
	// the suggestions it produced from it. After this the rank is a column, not a derivation.
	//
	// The work list is defaulted because the summary reaches here as an unvalidated cast over a
	// JSON artifact (see readFleetSummary), and one that omits the list has no
	// such field. No list means no ancestors, which is already the null-rank case; it must not be
	// a reason to lose the cycle's suggestions.
	const ranked =
		cycleFailed || !output
			? []
	...
	const outcome = await withSqliteRetry(
		() =>
			deps.commands.persistCycleResult({
				// Committed with the suggestions rather than after them: a restart between the two
				// used to leave a completed cycle nothing would ever consider again.
				autoLaunchDecision: buildAutoLaunchDecision(deps.getConfig()),
				createdAt,
				cycleId,
				cycleUpdate: {
