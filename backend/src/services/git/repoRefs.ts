...
// Branch, tag, stash, and worktree introspection for the Repository tab. Split from repoStats.ts so
// the stats module stays under the 300-line ceiling while this module owns its own git command
// group.
//
// Each command is bounded with its own timeout (same pattern as repoStats). Every list is capped so
// a repository with hundreds of refs never blocks the UI or floods the response. Tags share the
// branch cap because both are name lists of the same kind; the Snapshot block keeps the uncapped
// total, so a capped list is always readable against the real count.
...
export interface RepositoryRefs {
	...
	/** Tag names, newest first, capped at maxTags. Annotated and lightweight tags read alike. */
	tags: string[];
...
// for-each-ref prints one name per line, so the tag list needs no field splitting: the
// annotated/lightweight distinction lives in the object a tag points at, not in refname:short.
function parseTags(stdout: string): string[] {
	const tags: string[] = [];
	for (const line of stdout.split(/\r?\n/)) {
...
export async function readRepositoryRefs(projectPath: string): Promise<RepositoryRefsResult> {
	...
	const [branchList, tagList, stashList, worktreeList] = await Promise.all([
		runGit(
			projectPath,
			[
				'for-each-ref',
		...
		// Newest first, so the cap keeps the tags a reader is most likely looking for.
		// creatordate is the tag's own date for an annotated tag and the target commit's for a
		// lightweight one, which is the only ordering that reads sensibly across both.
		runGit(
			projectPath,
	...
	const tags = tagList.ok ? parseTags(tagList.stdout) : [];
	...
	return {
		reason: null,
		refs: { branches, stashes, tags, worktrees },
		state: 'ok',
	};
}
