...
type HeapStatsReader = () => Pick<ReturnType<typeof heapStats>, 'heapCapacity' | 'heapSize'>;

/**
 * Bun's Node-compatible process.memoryUsage() heap fields use different accounting and can report
 * heapUsed above heapTotal. Read both values from JavaScriptCore instead: heapSize is the live heap
 * footprint and heapCapacity is guaranteed to include at least that footprint. Invalid or
 * unavailable runtime data stays explicit rather than being clamped into a fabricated value.
 * @param readHeapStats Bun heap reader, injectable for failure-path coverage.
 * @returns A mutually consistent heap pair, or two nulls when Bun cannot provide one.
 */
export function getHeapUsage(readHeapStats: HeapStatsReader = heapStats): HeapUsage {
	try {
		const { heapCapacity, heapSize } = readHeapStats();
		const isValid =
			Number.isFinite(heapSize) &&
			Number.isFinite(heapCapacity) &&
			heapSize >= 0 &&
			heapCapacity >= heapSize;
		if (isValid) return { heapTotal: heapCapacity, heapUsed: heapSize };
		webLogger.warn({ heapCapacity, heapSize }, 'metrics: Bun returned invalid heap statistics');
	} catch (err) {
		webLogger.warn({ err }, 'metrics: failed to read Bun heap statistics');
	}
	return { heapTotal: null, heapUsed: null };
}
