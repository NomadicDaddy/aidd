...
export class StepExecutor {
	...
	constructor(input: {
		...
		this.dispatcher = new StepDispatcher({
			...
			recipeRefHandler: new RecipeRefHandler({
	...
	async executeRecipeSteps(
		...
		for (const [index, step] of recipe.steps.entries()) {
			...
			if (!shouldRunRecipeStep(step, context.parameters)) {
				...
				if (context.depth === 0) {
					await this.db
						...
						.set({ currentStepIndex: sequenceNumber })
						.where(eq(pipelineSessions.id, context.sessionId));
			...
			// NOTE: currentStepIndex is intentionally NOT bumped before executeStep.
			// The bump happens AFTER the step completes (below). executeStep creates
			...
			// created leaves currentStepIndex pointing at the last completed step, so
			// deriveResumeResolution (which uses currentStepIndex + 1) retries the
			// correct next step rather than silently skipping it.
	...
	// Resolves the in-flight depth=0 step using the persisted resolution (re-attach to a
	// detached managed run via RunWaiter, or fail the step with onFailure) and then runs
	// the remaining steps via the normal iteration.
	async resumeRecipeSteps(
		...
		if (context.depth === 0) {
			await this.db
				.update(pipelineSessions)
				.set({ currentStepIndex: inFlightStep.sequenceNumber })
				.where(eq(pipelineSessions.id, context.sessionId));
		}
		const inFlightResult = await resolveInFlightStep(
