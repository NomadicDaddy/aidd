...
export async function scanRoot(
	...
	const resolvedRoot = canonicalProjectPath(root);
	...
	async function visit(path: string, depth: number): Promise<void> {
		// The root itself is examined after the walk, never here. A fleet-wide task runs from the
		// applications root and the CLI stages a `.aidd/` scratch tree into whatever directory it runs
		// in, so claiming the root as a project the moment that directory appears would collapse
		// every project underneath it into one entry and empty the projects list.
		if (depth > 0 && (await hasAiddMetadata(path))) {
			projects.add(path);
			return;
		}
	...
	// Pointing a root straight at a single project is a supported setup, so a root that holds no
	// projects of its own is still read as one. A root that holds projects is a container, and its
	// own metadata directory belongs to fleet-wide work rather than to a project.
	if (projects.size === 0 && (await hasAiddMetadata(resolvedRoot))) projects.add(resolvedRoot);
	return {
