...
function usageValues(raw: RawRunLedgerEntry): LedgerUsageValues {
	...
	return {
		...
		// A zero in a ledger is ambiguous: it can mean a genuinely free/no-op run or a backend
...
export function finalizedRunLedgerEntries(entries: RawRunLedgerEntry[]): RawRunLedgerEntry[] {
	// A crash-fallback line can precede a real final summary for the same run id. Last entry
	// wins for finalized Run accounting; entries without run ids remain individually countable.
	...
	return [...legacyEntries, ...byRunId.values()];
}
...
export function projectUsageFromLedgerEntries(
	...
	const usageEntries = finalizedRunLedgerEntries(entries);
