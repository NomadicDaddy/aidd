...
export interface ContinuationLaunchDeps {
	db: WebDatabase;
	/**
	 * The initiator is a parameter rather than baked into the thunk because one wiring serves both
	 * consumers: a Continue click is 'operator', the auto-chain hook is 'automatic'. Collapsing
	 * them would file every auto-chained follow-up as work somebody asked for.
	 */
	launch(input: RunLaunchRequest, initiator: RunInitiator): Promise<WebRunRow>;
...
export async function continueRun(deps: ContinuationLaunchDeps, runId: string): Promise<WebRunRow> {
	...
	const launched = await deps.launch(buildContinuationLaunchRequest(row), 'operator');
...
export async function maybeAutoChainRun(
	...
	try {
		...
		const launched = await deps.launch(buildContinuationLaunchRequest(row), 'automatic');
		await deps.recordStart(launched);
