...
import { stopTail } from './heartbeatLiveness.ts';
import { prepareStaleRunRecovery, replayStaleRunArtifacts } from './staleAuditRecovery.ts';
...
export async function terminalize(
	...
	// transaction, so they cannot race a concurrent writer between read and write.
	...
	// The run reported its own exit, so both of its per-run signal files are spent: the heartbeat
	// file is this transition's retry signal, and the stop file is the request the run just
	// honored. annotateStopRequested only reads the stop file for 'running' rows and documents
	// that the flag self-heals once the run terminalizes, so clearing it here is what makes that
	// true for a run the backend never had to force. Only this path clears it — a stale or removed
	// heartbeat never confirmed the process read the request, and stopRun already clears the file
...
export async function markStale(
	...
	// The agent may have emitted its result in the seconds before the process died. Reading that
	// payload is safe here; replaying it is deferred until the write below is won.
	const plan = await prepareStaleRunRecovery(record);
	const staleRecord = plan.record;
	...
	// This writer owns the terminal transition, so the recovered payload — the whole product of a
	// run that died after finishing its work — is now replayed into real artifacts.
	const summary = await replayStaleRunArtifacts(ctx.db, plan, outcome.kind);
