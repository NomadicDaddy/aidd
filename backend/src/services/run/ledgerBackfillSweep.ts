 ...
 *
 * Also evaluates continuation_reason for rows still NULL there (force-failed or
 * terminalized by paths that skip evaluation), so the
 * Continue affordance converges for every row. 'none' is written for ineligible rows —
