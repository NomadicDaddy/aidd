...
export interface LaunchCommand {
	...
	entrypoint: string;
	/**
	 * Program tokens that precede the run flags in `args`. The Windows detached launcher reuses
	 * these to invoke the same program as the relauncher:
	 * `[...launcherPrefix, '--detached-spawn', payload]`.
	 */
...
function addModeArgs(args: string[], input: RunLaunchRequest, mode: WebRunMode): void {
	...
	if (mode === 'audit') {
		...
		args.push(
			'--audit',
			normalizeAuditNames(
				input.auditNames?.length ? input.auditNames : ['CODE_QUALITY'],
			).join(','),
		);
		return;
	}
...
export async function buildLaunchCommand(
	...
	// aidd ships as source, so a launch is always `bun <checkout>/cli/src/index.ts`.
	const entrypoint = join(rootDir, 'cli', 'src', 'index.ts');
	await access(entrypoint);
	const launcherPrefix = ['bun', entrypoint];
	const args = ['bun', entrypoint, '--project-dir', input.projectDir];
