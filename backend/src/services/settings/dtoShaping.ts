...
function directAiDto(config: PartialAiddConfig): DirectAiSettingsDto {
	...
	const apiKeyConfigured = Boolean(optionalString(providerConfig?.apiKey));
...
export function buildSettingsDto(
	...
	for (const [name, providerConfig] of Object.entries(config.providers ?? {})) {
		providers[name] = {
			apiKeyConfigured: Boolean(optionalString(providerConfig?.apiKey)),
	...
	return {
		...
		directorAutoCycleEnabled: resolved.director?.schedule?.enabled ?? false,
		directorAutoCycleIntervalHours:
			resolved.director?.schedule?.intervalHours ?? defaultDirectorIntervalHours,
		...
		initModel: config.initModel ?? null,
