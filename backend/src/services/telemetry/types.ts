import type { OutcomeRate } from 'aidd-shared/outcome-measures';
...
export interface RecordStartInput {
	...
	resourceId: string;
	resourceName: string;
	resourceSha256?: string | undefined;
...
export interface ProjectCostRow {
	/** Invocations whose run reported a positive cost, so unpriced work is not read as free. */
	costedInvocationCount: number;
	/** Sum of positive costUsd over the distinct runs the window's invocations point at. */
	costUsd: number;
...
// flag buckets where most runs captured no metrics instead of reading a low bar as low output.
