 ...
 *
 * The web backend serves `frontend/dist` itself — there is no nginx or other proxy in the
 * default loopback path, so anything a reverse proxy would normally add has to happen here.
 * Policy mirrors the template's nginx config (spernakit `docker/nginx.conf`) so behaviour is
...
/**
 * Pick the client's highest-quality encoding, preferring brotli on ties to match the build
 * budget's compression level. Entries refused with zero quality are skipped; a bare `*` is not treated as
...
export function negotiateEncoding(acceptEncoding: null | string): Encoding | null {
	...
	for (const part of acceptEncoding.split(',')) {
		...
		if (!name) continue;
		const qualityParam = params.find((p) => /^\s*q\s*=/i.test(p));
		const quality = qualityParam === undefined ? 1 : Number(qualityParam.split('=')[1]);
		if (Number.isFinite(quality) && quality >= 0 && quality <= 1) accepted.set(name, quality);
	}
	let selected: Encoding | null = null;
	let highestQuality = 0;
	for (const candidate of ['br', 'zstd', 'gzip'] as const) {
		const quality = accepted.get(candidate) ?? 0;
