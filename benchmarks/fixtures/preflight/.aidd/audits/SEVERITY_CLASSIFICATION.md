...
```json
{
	...
	"verificationEvidence": "Confirmed: path/to/file1.ts:42 — specific verified pattern or code behavior"
}
```
...
| `verificationEvidence` | String summarizing how the finding was verified against the current codebase. Must include a `file:line` reference and the specific pattern observed. |
...
- `verificationEvidence` MUST explain the concrete code evidence that justified creating the finding
