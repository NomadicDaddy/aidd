...
import { resolveExternalRunContext } from './orchestrator/active-run-heartbeat-support.ts';
...
export async function run(argv: string[]): Promise<number> {
	try {
		...
		const preparedSkill = await prepareSkillRun(args, config, rootDir);
		await assertProjectForRun(args);

		const plan = resolveRunPlan(args, config);
		const runDriver = runDriverForPlan(plan, preparedSkill?.driver);
		if (runDriver !== undefined) plan.driver = runDriver;
		...
		const heartbeat = await CliActiveRunHeartbeat.start(plan, {
			...
			...(external.initiator ? { externalInitiator: external.initiator } : {}),
			...(external.runId ? { externalRunId: external.runId } : {}),
