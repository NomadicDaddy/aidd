import { scrubSecretFields, scrubSecrets } from 'aidd-shared/lib/secretScrubber';
...
function cleanJsonDocument(raw: string): { changed: boolean; content: string } | undefined {
	...
	const content = `${JSON.stringify(scrubSecretFields(cleanValue(parsed)), null, 2)}\n`;
...
function cleanLogContent(raw: string): { changed: boolean; content: string } {
	...
	const cleaned = lines.map((line) => {
		...
		try {
			...
			const next = scrubSecretFields(cleanValue(parsed));
...
async function listLogFiles(iterationsDir: string): Promise<string[]> {
	...
	return entries
		.filter((entry) => /^(?:\d+\.(?:json|log)|run_\d+_[\w-]+\.log)$/.test(entry))
		.sort()
		.map((entry) => join(iterationsDir, entry));
}

// `undefined` means "scan everything": either no marker, an unreadable one, or one whose
// recorded version is not CLEANER_VERSION, so its sweep may lack passes this cleaner runs.
...
export async function cleanIterationLogs(
	...
	// A partial sweep must not mark skipped active logs as clean.
	if (scannedFiles > 0 && excludedFiles.size === 0) await touchMarker(markerPath);
