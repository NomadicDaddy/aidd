...
// The AIDD_RESULT marker (structuredResult) is a completion signal that may also carry the
// whole output. For large suggestion sets the model reliably writes the file but lets the
// marker's `suggestions[]` go empty, so trusting the marker over the file would silently drop
// every suggestion. Prefer whichever source carries more suggestions; the written file
