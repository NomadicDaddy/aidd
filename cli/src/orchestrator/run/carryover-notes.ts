...
// Raised when a feature record claimed by another LIVE run is gone from disk at the end of an
// iteration. The holder is still running and still working against that record: its agent will
// finish, write its completion, and find nothing there. This is the loudest aidd can be about a
// cross-run destruction it cannot undo — the deletion has already happened, and the bytes are
// gitignored in most projects, so there is nothing to restore from.
export function destroyedLeasedFeaturesNote(
	leases: readonly { featureId: string; runId: string }[],
): string {
	const lines = leases.map(
		(lease) =>
			`  - \`.aidd/features/${lease.featureId}/feature.json\` — claimed by live run \`${lease.runId}\``,
	);
	return (
		'**Feature records claimed by a concurrent live run no longer exist on disk.** They were ' +
		'present when this iteration started and are gone now:\n\n' +
		`${lines.join('\n')}\n\n` +
		'- If you removed, renamed, merged, or consolidated these records, restore them exactly as ' +
		'they were and leave them alone for the rest of this run. The run that claimed them is ' +
		'still working and will check them when it finishes.\n' +
		'- Redo the consolidation, prune, or rename against the records nobody has claimed, and say ' +
		'in your summary which ones you deliberately skipped.\n' +
		'- If you did not touch them, say so plainly rather than repairing state you did not break.'
	);
}
...
// tells the operator afterwards. This is the same check run mid-iteration, where the one party who
// can fix what it broke is still working. Capped, because a long-standing pre-existing
