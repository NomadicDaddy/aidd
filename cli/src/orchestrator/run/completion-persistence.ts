...
async function ignoredMetadataCompletionSatisfiesCommitGate(
	...
	for (const path of dirtySourcePaths) {
		...
		// it, and an operator editing an unrelated file mid-run must not sink the whole run by
		// demoting the completion to waiting_approval and exiting 7 over a file the agent
...
export async function parkCompletionsPendingCommit(input: {
	...
	for (const featureId of input.completedFeatures) {
		...
		// Deleted mid-iteration by a concurrent run: there is no record left to demote.
		const feature = await readFeatureIfPresent(input.store, featureId);
...
/** Stamp the run's completion instant onto features this iteration actually finished.
 *
 * `writeFeature` stamps `completedAt` for every write that flows through the store, but the normal
 * `→ completed` flip is the agent editing `feature.json` on disk — the orchestrator only observes
 * it — so on the common success path no store write happens at all and nothing would be stamped.
 * This reads each newly-completed record back and rewrites it through the store, which applies the
 * timestamp. Called only from the branch where completions survive the commit gate, so a completion
 * `parkCompletionsPendingCommit` demoted to `waiting_approval` is never dated as finished.
 *
 * Idempotent per feature: an already-stamped record is left alone, so a re-finalize does not
 * re-date it, and the whole batch shares one instant rather than drifting across the loop. */
export async function stampCompletedFeatures(
	store: AiddStore,
	featureIds: readonly string[],
	completedAt = new Date().toISOString(),
): Promise<void> {
	for (const featureId of featureIds) {
		const feature = await store.readFeature(featureId).catch(() => undefined);
		if (feature === undefined) continue;
		if (feature.status !== 'completed' || feature.passes !== true) continue;
		if (typeof feature.completedAt === 'string' && feature.completedAt.length > 0) continue;
		await store.writeFeature({ ...feature, completedAt });
	}
}
...
export async function applySimulatedFeatureCompletion(input: {
	...
	const feature = await readFeatureIfPresent(store, work.id);
