...
export function continuableBackendInterruptionRetryLimit(plan: RunPlan): null | number {
	...
	// (fall back to the iteration limit).
	const cap = plan.stopPolicy.maxConsecutiveTimeoutRetries;
	// 0 / null / undefined disable the dedicated cap (fall back to the iteration limit, so
	// continuable timeouts are bounded only by maxIterations, if set). A positive
