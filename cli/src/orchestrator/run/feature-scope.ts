...
export interface FeatureClaim {
	/** The selected feature's id, recorded for attribution even when the claim did not stick. */
	featureId: string | undefined;
	/** The record was gone by the time the claim ran — a concurrent run consolidated or pruned it
	 * between selection and here. There is nothing left to implement, so the caller must reselect
	 * rather than dispatch an agent at a feature directory that no longer exists. */
	vanished: boolean;
}
...
export async function claimSelectedFeatureForIteration(
	store: AiddStore,
	work: SelectedWork,
): Promise<FeatureClaim> {
	if (work.kind !== 'feature') return { featureId: undefined, vanished: false };
	const feature = await readFeatureIfPresent(store, work.id);
	// Deleted between selection and claim by a concurrent run (a consolidation directive, an
	// operator prune). Report it rather than throwing — aborting the run over someone else's edit
	// is too harsh — but do not let the iteration proceed either: its spec, notes, and directory
	// are gone, so the agent would be told to implement a feature that no longer exists.
	if (feature === undefined) return { featureId: work.id, vanished: true };
	if (feature.status === 'completed' && feature.passes === true)
		return { featureId: work.id, vanished: false };
	if (feature.status === 'in_progress') return { featureId: work.id, vanished: false };
	...
	if (feature.status === 'waiting_approval') return { featureId: work.id, vanished: false };
