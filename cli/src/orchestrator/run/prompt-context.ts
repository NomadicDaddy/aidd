...
import type { LeasedFeatureNotice } from '../../prompts/compile/leased-features.ts';
...
export class IterationPromptContext {
	...
	constructor(projectDir: string, rootDir: string, runId?: string) {
	...
	/** Feature records claimed by other live runs against this same working tree. This run's own
	 * lease is excluded — being told not to touch the feature it was given would be nonsense — and
	 * a run with no id (tests, embedded callers) filters nothing, so it can only over-report. */
	private async otherRunFeatureLeases(): Promise<LeasedFeatureNotice[]> {
		// Fail-soft: an unreadable lease directory must never be what stops an iteration from
		// compiling its prompt. Worst case the notice is absent, which is today's behavior.
		const leases = await readActiveFeatureLeases(this.projectDir).catch(() => []);
		return leases
			.filter((lease) => lease.runId !== this.runId)
			.map((lease) => ({ featureId: lease.featureId, runId: lease.runId }));
	}
	...
	async compile(promptPlan: PromptPlan): Promise<CompiledPrompt> {
		...
		const leasedFeatures = await this.otherRunFeatureLeases();
		return compilePrompt(promptPlan, {
			...
			...(leasedFeatures.length > 0 ? { leasedFeatures } : {}),
		});
	}
}
