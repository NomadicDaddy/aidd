...
// `audit`, `interview`, and a custom directive are compiled in code (compileBasePrompt returns
// before it reads the phase fragment) and have no file under `prompts/`. Advertising
// `prompts/<phase>.md` for them named a path that has never existed: readFragment swallows the
// ENOENT, so nothing failed, but the ref misreported where the prompt came from. Attach a path
// only for phases that really have a source file on disk.
export const filePhases = new Set([
	'coding',
	'director',
	'in-progress',
	'initializer',
...
function phaseFragment(phase: string): PromptFragmentRef {
	return filePhases.has(phase)
...
function promptFragments(
	...
	fragments.push(phaseFragment(phase));
	if (mode === 'audit') {
...
export function resolveRunPlan(args: ParsedArgs, config: ResolvedConfig): RunPlan {
	...
	const plan: RunPlan = {
		...
		stopPolicy: {
			...
			projectStopFile: `${projectDir}/.aidd/.stop`,
