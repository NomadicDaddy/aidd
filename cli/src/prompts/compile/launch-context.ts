...
//
// The scope sentence is load-bearing. Worded as a flat ban on starting a server, this block reads
// as a veto on the project's OWN gates: a fleet release would refuse to run its release script
// (which rebuilds and restarts the app as part of its own run, then stops it) and park the whole
...
// A run that inherits a dead app address would otherwise spend most of an iteration rediscovering
// that fact — probing, rebuilding, restarting — before parking. aidd already knows: it probed the address for
