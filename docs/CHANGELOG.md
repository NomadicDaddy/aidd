...
- The bash workspace policy no longer loses a command separator to a backslash-escaped quote. The tokenizer read `\"` outside quotes as a quote opener, so the span never closed and the rest of the line collapsed into one token: in `printf x \">/dev/null; cat ../outside.txt` the second command was never examined, and whether it was denied came down to how the host platform happened to resolve the swallowed blob. Both characters now stay in the token, so the separator survives and a Windows path keeps its backslash.
