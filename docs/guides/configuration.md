...
| `stream`              | boolean          | Set `false` to disable SSE streaming for an endpoint that is not streaming-compatible. Defaults to streaming for remote providers. |
| `streamIdleTimeoutMs` | positive integer | Stall timeout for a stream that stops sending bytes. JSON-only; there is no environment-variable equivalent.                       |

Recognized provider names and their defaults:

| Provider   | Base URL                              | Default model        | Key required |
| ---------- | ------------------------------------- | -------------------- | ------------ |
| `zhipu`    | `https://api.z.ai/api/coding/paas/v4` | `glm-5.2`            | yes          |
| `xai`      | `https://api.x.ai/v1`                 | `grok-4.5`           | yes          |
| `openai`   | `https://api.openai.com/v1`           | `gpt-5.6`            | yes          |
...
| `autoChainRuns`               | boolean          | `false`                                                                                  | Opt-in: auto-launches a follow-up run when a coding run ends continuation-eligible (wall-clock budget exhausted with selected features incomplete, or an initializer run completed cleanly). |
| `autoChainLimit`              | positive integer | `3`                                                                                      | Maximum auto-launched follow-up runs per chain, counted from the original run. Only meaningful when `autoChainRuns` is true.                                                                 |
