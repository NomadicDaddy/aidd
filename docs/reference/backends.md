...
was the native backend, Ollama or LM Studio through the native backend, or an external CLI process
(Claude Code, Cline, OpenCode, KiloCode, Codex, or Grok).
...
| `claude-code` | External Claude Code CLI process; `stream-json` output handled by the shared default parser (`parsers/plain.ts`).     |
| `cline`       | External Cline CLI process with dedicated parsing for both of Cline's NDJSON output formats.                          |
| `opencode`    | External OpenCode CLI process; JSON output handled by the shared kilo/opencode parser (`parsers/opencode-family.ts`). |
| `kilocode`    | External KiloCode CLI process; JSON output handled by the shared kilo/opencode parser (`parsers/opencode-family.ts`). |
...
rejected at input boundaries before a run plan is executed. `cline`, `codex`, and `grok` each use a
dedicated parser, `opencode` and `kilocode` share one, and `claude-code` falls back to the shared
default parser (`parsers/plain.ts`), which handles both tagged plain-text lines and generic JSON
envelopes.
...
(`NATIVE_PROVIDER`, `NATIVE_API_KEY` or the provider's own `OPENAI_API_KEY`/`ZHIPU_API_KEY`/
...
2. **Point aidd at it** with `cli` (or `--cli`), and optionally pin a model with `backends.<name>.model`:
    ...
    ```json
    { "backends": { "opencode": { "model": "zai-coding-plan/glm-5.3" } }, "cli": "opencode" }
    ```
...
> is `grok-4.6`; `--reasoning-effort` accepts `low`/`medium`/`high`/`xhigh`.
> Grok's headless output surfaces no tool-call events, so the Live Console shows its reasoning and
> final text but not intermediate tool steps.
...
> `zai-coding-plan/glm-5.3` (run `opencode models` to list valid ids). A bare or wrong-prefix id
...
unless you pass `--require-external`):

```powershell
bun run smoke:backends
bun run smoke:backends -- --require-external
```
