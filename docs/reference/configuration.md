...
| `web.maxConcurrentRuns`           | positive integer | `2`                                                                                      | Maximum concurrent _running_ managed web, scheduled, and Director launches. Overflow inserts as `queued` and is admitted FIFO when a slot frees. CLI-launched runs do not consume this ceiling.                                                                                     |
| `web.maxConcurrentRunsPerProject` | positive integer | `2`                                                                                      | Per-project cap on concurrent _running_ managed launches. A launch that would exceed it sits queued instead of failing. Mutating launches without a worktree still clamp to 1. CLI-launched runs do not consume this ceiling.                                                       |
