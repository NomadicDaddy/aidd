...
aidd is a Bun and TypeScript orchestration runtime. The high-level path is:
...
| ----------- | -------------------------------------------------------- |
| `director`  | `--director`                                             |
| `audit`     | `--audit NAMES` or `--audit-all`                         |
...
prompt rather than being appended. `--directive-readonly` forbids the run from touching code,
metadata, or git state even when the directive asks for changes; the agent describes the change it
would make instead. `--skill` runs default to read-only for the same reason and need
`--skill-intent apply-changes` to write. Director mode also accepts
...
- dependency gating, where every dependency must resolve to a known feature that is `completed`
  with `passes: true`; a dependency reference that matches no record blocks the feature
- status gating, where `completed`, `passes: true`, and `waiting_approval` records are skipped

Eligible candidates sort `in_progress` first, then by ascending `priority` (records without one
sort last), then by feature directory. Audit findings are skipped in normal coding unless
...
- `openai` uses native backend machinery with OpenAI provider defaults.
- `claude-code`, `cline`, `opencode`, `kilocode`, `codex`, and `grok` spawn external CLIs and
  parse their output.

All four native-machinery backends share one prompt fragment, `prompts/_cli/native.md`; there is no
per-provider file.
...
- `--quit-on-abort`, `--stop-when-done`, and `--no-stop-when-done` configure the remaining stop
  policy.
- Every mode except `director` is gated by a dirty-tree preflight: when the working tree carries
  more dirty files than `dirtyTreeThreshold`, the run stops before work selection and backend
  execution, records a `dirty_tree_too_high` iteration, and exits `0`. `.aidd/` metadata paths are
  excluded from that count, so feature/status bookkeeping never trips the gate.
