...
| `shippedVersion`      | `string`           | Internal project version this feature's current state shipped in. Stamped by the `document-changes` skill alongside a dated revision note in `notes`.                                                                |
