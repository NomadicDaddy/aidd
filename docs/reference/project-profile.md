...
| `single_user_local`   | Single-user local | One person runs it on their own machine; no other users. ⊖ low-exposure. Inferred fallback when no other signal matches.       |
...
| `internet_single_org` | Internet org      | Internet-reachable but serving a single organization's users. Inferred when Convex is detected.                                |
...
| `local_only` | Built locally | A `Dockerfile` exists and images are built locally or in CI, but nothing is pushed to a registry. |
| `published`  | Published     | Images are pushed to a registry (ghcr, Docker Hub, a private registry) and consumed from there.   |
...
`source: "explicit"` and persists the canonical file. The Stage 2 (`structured`) maturity action
`project-profile.json` promotes it the same way without opening the UI: it persists the inferred
facets unchanged as an explicit file.
