...
A coding step whose config sets `skipRemainingOnNoWork: true` ends its recipe when the run finds nothing to work on (`stopReason: no_work`, for example because every open feature is blocked by a dependency or waiting for approval). The steps after it are recorded as skipped and the session completes normally; the session counts them apart from completed steps, so it reads "1 of 4 steps completed · 3 skipped". A web restart at any point after the coding step finishes the remaining skips rather than resuming the review, because the run it launched keeps its `no_work` stop reason. `coding` and the other coding-first recipes set it, because their later steps review, remediate, and document what the coding step built, and with nothing built they would review older commits again. Recipes that follow coding with a validate pass leave it unset, since that pass checks completed features whether or not this run changed anything. Whether or not the flag is set, a no-work step shows the run's own explanation as its output instead of an empty transcript.
