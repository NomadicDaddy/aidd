import type {
	...
	TelemetryProjectCostRow,
	TelemetryResourceDetail,
...
export type ProjectCostQuery = ResourceUsageQuery;
...
export async function listProjectCosts(
	query: ProjectCostQuery = {},
	signal?: AbortSignal,
): Promise<TelemetryProjectCostRow[]> {
	const response = await apiGet<{ projects: TelemetryProjectCostRow[] }>(
		`/api/v1/telemetry/projects${buildQuery({ ...query })}`,
		{ signal },
	);
...
export async function getTelemetryResourceDetail(
	type: TelemetryResourceType,
	id: string,
	signal?: AbortSignal,
): Promise<TelemetryResourceDetail> {
	const response = await apiGet<{ detail: TelemetryResourceDetail }>(
		`/api/v1/telemetry/resource/${encodeURIComponent(type)}/${encodeURIComponent(id)}`,
		{ signal },
	);
