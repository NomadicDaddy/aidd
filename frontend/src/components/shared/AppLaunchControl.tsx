...
export function AppLaunchControl({
	...
	if (unavailable) {
		...
		// One control, both densities. The full-size branch says the sentence once, in the tooltip:
		// repeating it as visible prose beside the button reads in the project header as a loose
		// unattributed line next to a button that already says it.
		//
		...
		if (compact) {
			return (
				<span
					...
					<span className="sr-only">{withContext(unavailableLabel)}</span>
				</span>
			);
		}
		return (
