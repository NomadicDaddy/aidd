...
import { Button, buttonClassName, type ButtonSize } from '../ui/button.tsx';
...
export type LaunchTargetControlSize = 'chip' | Exclude<ButtonSize, 'icon'>;
...
export interface LaunchTargetControlProps {
	...
	role?: LaunchRole;
	/**
	 * `'chip'` (default) is the compact table/card summary. The remaining values use the shared
	 * button scale so a launch row can choose the same 32px, 36px, or 40px desktop step as its
	 * neighbouring controls.
	 */
	size?: LaunchTargetControlSize;
...
export function LaunchTargetControl({
	...
	const panelId = useId();
	const titleId = useId();
	const defaults = useLaunchDefaults(projectDir || undefined, mode, value.backend);
	...
	const dialogTitle = role
		? `${roleLabels[role]} launch target`
		: label && !label.toLowerCase().endsWith('launch target')
			? `${label} launch target`
			: (label ?? 'Launch target');
			...
			<button
				...
				className={cn(
					size === 'chip'
						...
						: buttonClassName('secondary', 'max-w-full flex-wrap', size),
					// Every size keeps the mobile floor: this is a bordered control that opens a
					...
					// at 28-40px depending on the selected desktop step or whether the summary wrapped.
					'max-sm:min-h-11',
					display.custom
				...
				{display.custom && customBadge ? (
					<Badge className="px-1.5 py-0.5" tone="teal">
						Custom
					</Badge>
				) : display.custom ? (
