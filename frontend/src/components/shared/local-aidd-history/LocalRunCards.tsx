...
export function RunExecutionTarget({ run }: { run: ProjectLocalRun }) {
	return (
		<div className="flex min-w-0 flex-wrap items-center gap-1.5">
			...
			{/* Mono, because this line is `aidd 3.0.0 · 716c96c7 · dirty`: a version, a commit
			    SHA and a tree state, all of them machine strings, set directly beneath the mono
			    backend and model chips above them. In proportional type it would be the one half
			    of an execution identity set in the wrong face. */}
						...
						<div className="border-t border-border">
							...
							{isOpen && runIterations.length > 0 ? (
								<div className="bg-muted/50">
									<LocalIterationsTable iterations={runIterations} now={now} />
								</div>
							) : null}
						</div>
					</div>
