...
export function TerminalTabView({ active, sessionId }: { active: boolean; sessionId: string }) {
	...
	const fitAndReport = useCallback(() => {
		...
		if (lastSentSize.current?.cols !== size.cols || lastSentSize.current?.rows !== size.rows) {
			lastSentSize.current = size;
			sendTerminalResize(sessionId, size.cols, size.rows);
		}
	}, [sessionId]);
	...
	}, [sessionId, fitAndReport, writeAcked]);
	...
	}, [open, active, maximized, heightPx, fitAndReport]);
	...
	const resolvePaste = useCallback((text: null | string) => {
	...
	return (
		<div
			...
			{pendingPaste !== null && (
				<div className="absolute inset-x-0 top-0 z-10 flex items-center gap-2 border-b border-amber-300/60 bg-amber-50 px-3 py-1.5 text-xs text-amber-900 dark:border-amber-500/30 dark:bg-amber-950/70 dark:text-amber-200">
					<span>
						Paste {pasteLineCount} lines? Each line may run as a separate command.
					</span>
					<button
						autoFocus
						className="rounded border border-amber-400/60 px-2 py-0.5 font-medium hover:bg-amber-100 dark:hover:bg-amber-900/40"
