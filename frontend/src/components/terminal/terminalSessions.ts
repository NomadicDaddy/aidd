...
import { unavailableReasonForStatus } from './terminalUnavailable.ts';

/** An ApiError carries the server's answer; anything else means no answer arrived at all. */
function reasonFor(error: unknown) {
	return unavailableReasonForStatus(error instanceof ApiError ? error.status : null);
}
...
export async function ensureTerminalReady(): Promise<void> {
	// Flagged around the whole sequence, so the pane can say it is working rather than render the
	// same empty region it shows when a terminal could not be started at all.
	setStarting(true);
	try {
		initPromise ??= initialize();
		await initPromise;
		// A creation kicked off elsewhere (openTerminalPaneAt, a concurrent effect) counts — wait
		// for it before concluding the pane needs a default tab.
		if (activeCreate) await activeCreate;
		if (getTabOrder().length === 0 && !isPtyUnsupported()) await createTerminalTab();
	} finally {
		setStarting(false);
	}
}
...
async function performCreate(options: {
	...
	try {
		...
		// A toast was the whole notice the operator got, and the pane behind it stayed blank.
		// Recording the reason gives that region something to say and a control to retry from.
		setUnavailable(reasonFor(error));
...
/** Retry after 'unavailable' (e.g. the backend came back, with or without PTY support). */
