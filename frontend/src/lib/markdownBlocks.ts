...
/**
 * The inline pass's grammar: `**bold**`, `*italic*`, `` `code` ``, `[text](href)`, and the explicit
 * `{{kbd:Ctrl+K}}` shortcut convention.
 *
...
export function plainInlineText(text: string): string {
	return text.replace(
		...
		(
			...
			_href?: string,
			shortcut?: string,
		) => bold ?? italic ?? code ?? linkText ?? shortcut ?? '',
	);
}

/** The first prose paragraph after an optional document title, preserving its inline markup. */
export function markdownSummary(markdown: string): string | undefined {
	const blocks = parseMarkdownBlocks(markdown);
	const firstContent =
		blocks[0]?.type === 'heading' && blocks[0].level === 1 ? blocks[1] : blocks[0];
	return firstContent?.type === 'paragraph' ? firstContent.text : undefined;
}
