...
import { auditCatalogHeadingId, auditCatalogListId } from '../auditsUtils.ts';
import {
	type CatalogSort,
	type CatalogSortKey,
	defaultCatalogSort,
...
import { useCatalogFilters } from './useCatalogFilters.ts';
...
export function CatalogTab({ onJumpToMatrix }: { onJumpToMatrix: () => void }) {
	...
	const filteredDefinitions = filterAndSortCatalog(manager.data?.definitions ?? [], {
		...
		healthFilter: catalogFilters.healthFilter,
		query: catalogFilters.query.trim().toLowerCase(),
		sort,
	});

	const enabledDefinitionNames = new Set(
		(manager.data?.definitions ?? []).filter((item) => item.enabled).map((item) => item.name),
	);
	const visibleEnabledNames = filteredDefinitions
	...
	const runAllDisabledReason = !auditsEnabled
		...
		: selectedProjectCount === 0
			? 'Select at least one launch target above to enable run actions.'
			: launch.isPending
	...
	return (
		<div className="space-y-5">
			<CatalogToolbar
				...
				onEnabledFilterChange={catalogFilters.setEnabledFilter}
				onHealthFilterChange={catalogFilters.setHealthFilter}
				onQueryChange={catalogFilters.setQuery}
				onReset={catalogFilters.reset}
				onRun={runAudits}
			...
			<div
				aria-labelledby={auditCatalogHeadingId}
				className={showEditor ? 'hidden xl:block' : undefined}
				id={auditCatalogListId}
				ref={catalogRef}
				role="region"
				tabIndex={-1}>
				{/* Three stacked cards, and the one carrying the data was the only one that never
				    said what it was — the editor even refers the reader back to "the catalog
				    above", which had no name on screen. Outside the p-0 table Card so the table
				    and the card list below xl share it, and it names the region itself now. */}
				<CardHeader
					className="mb-3 px-4"
					description="Every audit definition aidd can run, with its recent outcomes. Select a row to edit its definition below."
					id={auditCatalogHeadingId}
					title="Audit Catalog"
				/>
				<CatalogTable
					allSelected={allVisibleAuditsSelected}
					definitions={filteredDefinitions}
					filters={catalogFilters.register}
