...
import type { DashboardCardPlacement } from './dashboard-shared.ts';
...
const pillClassName =
	'inline-flex min-h-11 min-w-11 items-center justify-center rounded-full border border-accent/50 bg-card px-2 py-0.5 text-accent shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/80 sm:min-h-0 sm:min-w-0';
...
export function SortableDashboardCard({
	...
	locked: boolean;
	onHeightChange: (id: DashboardCardId, height: number) => void;
	...
	const cardStyle: DashboardCardStyle = {
		'--dashboard-card-column': placement?.column,
		'--dashboard-card-row-span': placement?.rowSpan,
		'--dashboard-card-row-start': placement?.rowStart,
	...
	useLayoutEffect(() => {
		...
		function reportHeight() {
			if (!wrapper) return;
			onHeightChange(card.id, Math.ceil(wrapper.getBoundingClientRect().height));
		}
	...
	return (
		<div
			className={cn(
				...
				placement &&
					'xl:[grid-column:var(--dashboard-card-column)] xl:[grid-row:var(--dashboard-card-row-start)/span_var(--dashboard-card-row-span)]',
				!placement && (isFull || stretch) && 'xl:col-span-2',
				!locked &&
