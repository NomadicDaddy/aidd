...
export function TokenSparkline({ points }: { points: ProjectUsageDailyTokens[] }) {
	...
	return (
		<svg
			...
			className={`block h-[18px] w-16 ${total > 0 ? toneText.teal : 'text-muted-foreground'}`}
			role="img"
...
function PortLabel({
	...
	const cls = collision ? `font-semibold ${toneText.amber}` : '';
	if (listening === true) {
		return (
			<a
		...
		<span className="inline-flex flex-wrap items-center gap-1.5 font-mono text-xs whitespace-nowrap">
			...
			{hasCollision ? (
				<span aria-label={collisionLabel} role="img" title={collisionLabel}>
					<AlertTriangle aria-hidden="true" className={`h-3.5 w-3.5 ${toneText.amber}`} />
				</span>
			) : null}
		</span>
	);
