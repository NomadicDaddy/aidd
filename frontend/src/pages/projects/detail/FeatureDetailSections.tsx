			...
			<ul className="mt-2 space-y-1.5">
				{items.map((item) => (
					<li
						className={
							mono
								? 'min-w-0 rounded-md border border-border bg-muted px-3 py-2 font-mono text-xs break-all text-foreground'
...
export function FeatureApprovalSection({
	...
	return (
		<section>
			<h3 className={sectionCaptionClass}>Approval</h3>
...
export function FeatureBlockingContextSection({
	...
	return (
		<section>
			<h3 className={cn(sectionCaptionClass, toneText.amber)}>Blocking context</h3>
			...
			<dl
				className={cn(
					'mt-2 divide-y divide-border overflow-hidden rounded-md border text-sm',
					toneBorder.amber,
				)}>
				{rows.map((row) => (
					<div
