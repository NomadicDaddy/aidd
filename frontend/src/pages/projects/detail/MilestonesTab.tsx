...
const AUTO_PLACE_HELP =
	"Repair placement: push features after their dependencies, pull completed features back to the milestone matching their shipped version, and backfill missing shippedVersion from the app's current version";
...
export function MilestonesTab({ projectId }: { projectId: string }) {
	...
	const intro = (
		<TabIntro
			description={
				<>
					Coding runs walk these in order and admit only the first incomplete one. Every
					change is previewed against the dependency graph before it is written to{' '}
					<span className="font-mono text-xs">roadmap.json</span>.
				</>
			}
			title="Milestones"
		/>
		...
		<div className="space-y-4">
			...
			<Card className={`overflow-hidden p-0 ${tableColumnClass}`}>
				<CardHeader
					action={
						<div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap">
							<div className="grid min-w-0 grid-cols-[minmax(0,1fr)_2.75rem] items-center gap-1 sm:flex sm:flex-none">
								<Button
								...
								<Tooltip
									content={AUTO_PLACE_HELP}
									disclosure
									disclosureLabel="Explain auto-place features"
									maxWidth="sm">
				...
				{view.lifecycle === 'locked' ? (
					<p className={`border-b border-border px-4 py-3 text-xs ${toneText.amber}`}>
