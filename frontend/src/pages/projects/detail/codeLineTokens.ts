...
export function isCommentLine(line: string, language?: null | string): boolean {
	const trimmed = line.trim();
	if (trimmed.length === 0) return false;
	if (language?.toLowerCase() === 'markdown') return trimmed.startsWith('<!--');
...
export function splitStrings(line: string): CodeSegment[] {
	...
	const nonWhitespace = line.replaceAll(/\s/gu, '').length;
	const stringGlyphs = segments
		.filter((segment) => segment.kind === 'string')
		.reduce((total, segment) => total + segment.text.replaceAll(/\s/gu, '').length, 0);
	if (nonWhitespace > 0 && stringGlyphs > nonWhitespace / 2) {
