 ...
 *
 * Hiding nodes without changing the full graph's 2,000px-plus shell would leave a three-node result
 * stranded at its original coordinates. The full relationship data stays on the source graph for
 * diagnostics; this view copy narrows the links used by the visible layout and selection panel
...
export function fitFeatureDependencyGraph(
	...
	const positionedNodes = positionNodes(visibleNodes, maxWidth, order);
	const size = graphSize(positionedNodes);
	return {
		...graph,
