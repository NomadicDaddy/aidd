...
import { featureMatchesPriorityFilter } from './featurePriorityFilters.ts';
...
export function featureMatchesSearch(feature: ProjectFeature, query: string): boolean {
	...
	// `summary`, not `spec`. The project-detail response does not ship `spec` with every row — it
	// would be a third of a two-megabyte payload — and a filter cannot match text the client does
	// not hold. `summary` is the one-line statement of the same thing, so a search for what a
	// feature does still finds it; searching the full acceptance criteria requires opening the
	// feature.
...
export function featureMatchesFilters(
	...
	if (!featureMatchesPriorityFilter(feature, filters.priorityFilter)) return false;
	const status = stringValue(feature, 'status') || 'unknown';
	if (
		filters.statusFilter === 'incomplete' &&
		(status === 'completed' || feature.passes === true)
	)
		return false;
	if (
