...
export function WorkingTreeCard({ projectId }: { projectId: string | undefined }) {
	...
	if (!query.data || query.data.state !== 'ok') {
		return (
			<EmptyState>
				{query.data ? STATE_MESSAGE[query.data.state] : STATE_MESSAGE.error}
			</EmptyState>
		);
	}
