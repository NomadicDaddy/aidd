...
export function ActiveRunRow({
	...
	// `?? null` guards a payload that omits the field (undefined).
	...
	return (
		<tr
			...
			<td className="max-w-0 py-3 pr-3 pl-4">
				...
				{/* One meta line, because a session row has one: a summary adding a third and
				    fourth line that only run rows carry would stand a Run row 100px tall against the
				    65px Skill row beneath it, and the table would have no row rhythm at all. The full
				    summary is available through the shared tooltip here and rendered whole in the
				    detail panel. */}
				{run.aiSummary ? (
					<Tooltip content={run.aiSummary}>
						<div
							className="mt-1 truncate rounded-sm text-xs text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring/80 focus-visible:ring-offset-2 focus-visible:ring-offset-background focus-visible:outline-none"
							tabIndex={0}>
			...
			<td className="px-3 py-3">
				...
				<div className="flex flex-nowrap items-center gap-1.5 whitespace-nowrap">
					...
					<RunInitiatorBadge run={run} />
				</div>
			</td>
			<td className="px-3 py-3">
				{/* No `max-w` here: the column budget is the colgroup's job, and capping the
				    cell below it would make the model segment ellipsise to two characters while
				    the column still has room. */}
