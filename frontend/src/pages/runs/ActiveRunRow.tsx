...
export function ActiveRunRow({
	...
	const disabledControlsHintId =
		stopDisabled || killDisabled ? `run-controls-disabled-${run.id}` : undefined;
	...
	return (
		<tr
			...
			<td className="px-4 py-3">
				...
				<div className="mt-1 flex max-w-[22rem] flex-wrap items-center gap-1.5 text-xs text-neutral-500">
				...
				{run.aiSummary ? (
					<div className="mt-1 line-clamp-2 max-w-[22rem] text-xs text-neutral-400 dark:text-neutral-500">
						{run.aiSummary}
					</div>
				) : null}
			</td>
			...
			<td className="px-4 py-3">
				...
				{controlHint && (
					<p className="mt-1 text-xs text-neutral-500" id={disabledControlsHintId}>
						{controlHint}
					</p>
				)}
			</td>
