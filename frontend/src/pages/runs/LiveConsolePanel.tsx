...
export function LiveConsolePanel({
	...
	// Ask the output endpoint for every selected record, including one whose listing says no
	// readable log is known. The endpoint owns the exact unavailable reason (missing file, absent
	// path, unknown run); suppressing this request would replace that evidence with stale UI copy.
	const output = useRunLiveOutput(selectedRunId, selectedRun?.status);
	...
	const liveConsoleMessage = ((): string => {
		...
		if (output.state === 'unavailable') return 'Run output is unavailable.';
		if (output.state === 'cli-only') {
			return 'No readable console output is available for this run.';
		}
	...
	return (
		<LiveConsole
			...
			key={selectedRunId ?? 'no-selection'}
			message={liveConsoleMessage}
			selectedRun={selectedRun}
			sourceEndByte={output.endByte}
			sourceStartByte={output.startByte}
			sourceTotalBytes={output.totalBytes}
			stopDetail={stopDetail}
			windowLimitBytes={output.windowLimitBytes}
		/>
	);
}
