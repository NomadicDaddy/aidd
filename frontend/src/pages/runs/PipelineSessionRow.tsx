	...
	return (
		...
		<div className="flex flex-nowrap gap-2">
			<Link
				...
				// An unscoped `h-8 w-8` override would strip the `icon` size's `h-11 w-11` and leave
				// 32x32 on a phone — the smallest control in the app that is a real navigation
				// action. The override is `sm:` scoped, so the ACTIONS cell keeps its 32px pair at
				// the widths the reasoning below is about, and the floor applies where there is no cell.
		...
		<tr
			...
			<td className="px-3 py-3">
				<div className="flex flex-wrap items-center gap-1.5">
					...
					<ParkedWorkBadge parkedWorkRuns={session.parkedWorkRuns} />
				</div>
