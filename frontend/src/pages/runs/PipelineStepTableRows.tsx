 ...
 *
 * They used to be one `colSpan={7}` cell containing a private CSS grid whose columns restated the
 * parent's `<colgroup>` percentages as `fr` units. Restating them is the bug: the parent is
 * `table-fixed` and its cells carry their own asymmetric padding, so the two column systems agreed
 * only by hand and drifted apart the moment either side was touched — a step's KIND badge sat over
 * the gap between the parent's KIND and MODEL columns. Real `<tr>` elements take the parent's
 * widths because they are in the parent's table; there is nothing left to keep in sync.
 *
