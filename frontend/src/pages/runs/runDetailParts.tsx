...
export const detailPreClass = `overflow-auto rounded-md border border-border bg-background p-2 text-xs whitespace-pre-wrap text-foreground ${machineTextBreakClass}`;
...
export function MetadataItem({
	...
	return (
		<div className={cn('flex min-w-0 flex-col', className)}>
			<dt className={cn(microLabelClass, 'text-muted-foreground')}>{label}</dt>
			<dd
				className={cn(
...
export function RunCommandBlock({
	...
	const setPre = (node: HTMLPreElement | null) => {
		cleanupRef.current?.();
	...
	return (
		<div className={monoEditorMeasureClass}>
			<div
				className={cn(
					'mb-1 flex items-center gap-1.5 text-muted-foreground',
					microLabelClass,
				)}>
				Command
				<RunCommandInfo command={command ?? null} runId={runId} />
			</div>
			{/* Six explicit 16px lines plus 16px padding and 2px border. Keeping both the line-height
			    and the 114px border-box cap explicit prevents a clipped partial line. */}
			<pre
				className={cn(
					'max-h-[114px] w-full font-mono leading-4',
					detailPreClass,
					expanded ? 'max-h-none overflow-auto' : 'overflow-hidden',
				)}
				id={commandId}
				ref={setPre}>
...
export function RunCommitsSection({ run }: { run: RunRecord }) {
	...
	return (
		<div className="space-y-2">
			...
			{hasCommits ? (
				<div>
					<div className={cn('mb-1 text-muted-foreground', microLabelClass)}>Commits</div>
