...
// column — the narrowest column that carries free text. A single `truncate` line capped at 16rem
// with the full text nowhere would be roughly a ninth of a sentence, with no tooltip in the
...
// already carry for payloads that omit the field.
export function consoleSelectionLabel(run: {
	...
	return `Show ${runDisplayName(run)} for ${run.projectName} in Live Console`;
}
...
export function runDisplayName(run: {
	...
	if (run.source !== 'director') return run.mode ?? 'Run';
	return run.summary ?? (run.id ? `Director cycle ${run.id.slice(-8)}` : 'Director cycle');
}
