...
import { presentRunRecordInitiator, type UnifiedInitiatorFilter } from './runInitiator.ts';
import { normalizePathForFilter, type UnifiedKindFilter } from './runsUtils.ts';

export type { UnifiedInitiatorFilter } from './runInitiator.ts';
export type { UnifiedKindFilter } from './runsUtils.ts';
...
// Initiator is filtered on runs only, exactly as mode is: a recipe session stores an initiator so
// the runs it launches inherit it, but the session API does not expose one, so narrowing to an
// initiator hides pipeline and skill entries rather than guessing for them.
export function entryMatchesInitiator(
	entry: UnifiedEntry,
	filter: UnifiedInitiatorFilter,
): boolean {
	if (filter === 'all') return true;
	if (entry.kind !== 'run') return false;
	return (entry.run.initiator ?? 'unknown') === filter;
}

export interface UnifiedEntryFilters {
	initiator: UnifiedInitiatorFilter;
...
export function entryMatchesFilters(entry: UnifiedEntry, filters: UnifiedEntryFilters): boolean {
	...
	if (!entryMatchesInitiator(entry, filters.initiator)) return false;
	...
	const haystack =
		entry.kind === 'run'
			? `${entry.run.id} ${entry.run.projectName} ${entry.run.projectPath} ${entry.run.mode} ${entry.run.status} ${entry.run.source} ${presentRunRecordInitiator(entry.run).label}`
