	...
	return (
		<>
			<Card className="flex flex-col gap-4" variant="sunken">
				...
				/>
				{dirty ? (
					<p className={`text-xs ${toneText.amber}`} role="status">
						Save or discard settings changes before restarting or shutting down the
						control panel.
					</p>
				) : (
					<p className="text-xs text-muted-foreground">
						Running · backend listener active · actions require confirmation
					</p>
				)}
			</Card>
