...
export function createBlankSettings(): WebConfigSettings {
	return {
		...
		directorAutoLaunchEnabled: false,
		directorAutoLaunchMaxPerCycle: 1,
		directorAutoLaunchMaxRank: 1,
		directorAutoLaunchRiskCeiling: 'LOW',
...
export function normalizeIgnoredFolders(settings: WebConfigSettings): WebConfigSettings {
	return {
		...
		// The backend drops blank entries on save, so a blank row left in this list would keep the
		// page dirty forever against a saved value that can never contain one.
		directorAutoLaunchAllowedRecipes: cleanIgnoredFolders(
			settings.directorAutoLaunchAllowedRecipes,
		),
