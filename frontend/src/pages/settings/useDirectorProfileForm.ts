 ...
 *
 * It used to live inside the section component, which is a child of a `TabPanel` — and a `TabPanel`
 * unmounts when you leave it. So the card was a save island with no unsaved vocabulary of any kind:
 * Save Profile looked identical dirty or clean, the AI & Director tab trigger carried no dot, and
 * switching tabs silently discarded the edit that the absent dot had just failed to mention. The
 * state has to outlive the panel for the dot to be true, which is why it is a hook called by the
 * page rather than `useState` in the section.
 *
