 ...
 *
 * Ticks at the raw data max and half of it would label a whole-number invocation axis `20.5`.
 * Rounding to a step first means the label is always a number a reader would have chosen.
 */
...
/** Ticks for a diverging chart whose two arms share one symmetric domain. */
export function divergingTicks(max: number, options: AxisScaleOptions = {}): AxisTick[] {
	const domain = niceAxisMax(max, options);
	return [
		{ label: formatCompactNumber(domain), offsetPct: 0 },
		{ label: formatCompactNumber(domain / 2), offsetPct: 25 },
		{ label: '0', offsetPct: 50 },
		{ label: `−${formatCompactNumber(domain / 2)}`, offsetPct: 75 },
		{ label: `−${formatCompactNumber(domain)}`, offsetPct: 100 },
	];
}
