...
This document defines the mandatory technology stack for Spernakit v3 applications. Agents must follow these standards to ensure consistency with the template architecture.
...
## Schema Change Coordination (CRITICAL)

When adding, removing, or renaming a column in a Drizzle schema file (`backend/src/db/schema/`):

1. **Run typecheck immediately** after the schema change: `bun run typecheck`
2. TypeScript will show every insert/update/select call that is now broken
3. **Fix ALL call sites** before proceeding: grep for the table name across `backend/src/services/` and `backend/src/routes/`
4. Run `bun run typecheck` again to confirm zero errors before moving on

**Common mistake:** Adding a required column to a schema table but only updating `seed.ts`, forgetting the service files that also insert into that table. TypeScript catches this if you run typecheck after the schema change.
...
```bash
# Development (agent-safe — non-blocking, managed via scripts/start.ts + scripts/stop.ts)
bun run start                  # Start both servers detached (use THIS in agent sessions)
bun run stop                   # Stop servers started by `bun run start`

# Development (human-interactive — blocking, foreground with aggregated logs)
bun run dev                    # Start both servers in foreground (humans only — blocks the terminal)
bun run dev:backend            # Backend only (foreground, blocking)
bun run dev:frontend           # Frontend only (foreground, blocking)
...
- [Spernakit STACK.md](https://github.com/NomadicDaddy/spernakit/blob/main/docs/template/STACK.md) - Full technical reference
- [Spernakit DEVELOPMENT.md](https://github.com/NomadicDaddy/spernakit/blob/main/docs/template/DEVELOPMENT.md) - Development guide
- [aidd Audit: SPERNAKIT.md](https://github.com/NomadicDaddy/aidd/blob/main/audits/SPERNAKIT.md) - Spernakit-specific audit
