...
**Carve-out for pre-existing suites:** if the project already has a test suite (including
Spernakit-scaffolded Vitest/bun:test suites), you may — and should — run and extend it. Only
installing a NEW test harness where none exists is banned.
...
1. **Launch browser** → `agent-browser open <app-url>` (the app's known URL — never guess a framework-default port)
