...
4. Select the next available feature from backlog based on priority and dependencies (skip `waiting_approval`; those are blocked on human decisions)
...
- **Autofix first, iterate fast (efficiency):** run `bun run format && bun run lint:fix` to clear
  ...
  **once** to confirm; do not re-run the whole slow gate after every single fix (see Step 9.2).
...
one final confirmation before commit. (Gates still run in the foreground; the change is how _often_
you run the slow one.)
...
#### 9.4 Post-Commit Metadata Check
