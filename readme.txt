...
A shell script that orchestrates autonomous development sessions using OpenCode or KiloCode. aidd supports both bash (aidd.sh) and PowerShell (aidd.ps1) environments.

## Specifications

aidd uses project specifications in Markdown format aligned with Spernakit template standards. All specs are located in the `specs/` directory with `.md` extensions.

## Usage

### Bash/Linux/macOS
```bash
./aidd.sh --project-dir <dir> [--spec <file>] [--max-iterations <num>] [--timeout <seconds>] [--idle-timeout <seconds>] [--model <model>] [--init-model <model>] [--code-model <model>] [--no-clean] [--quit-on-abort <num>]
```

### PowerShell/Windows
```powershell
./aidd.ps1 -ProjectDir <dir> [-Spec <file>] [-MaxIterations <num>] [-Timeout <seconds>] [-IdleTimeout <seconds>] [-Model <model>] [-InitModel <model>] [-CodeModel <model>] [-NoClean] [-QuitOnAbort <num>] [-Help]
```
...
### New Project Example
```bash
./aidd.sh --project-dir ../new-project --spec ./specs/todolist.md --max-iterations 5
```

### Existing Codebase Example
```bash
./aidd.sh --project-dir ../existing-app --max-iterations 10
```

### PowerShell Examples
```powershell
# New project
./aidd.ps1 -ProjectDir "../new-project" -Spec "./specs/todolist.md" -MaxIterations 5

# Existing codebase
./aidd.ps1 -ProjectDir "../existing-app" -MaxIterations 10
```
...
aidd automatically detects existing codebases by checking for files and directories (excluding common ignored items like `.git`, `.aidd`, `.DS_Store`, `node_modules`, `.vscode`, `.idea`). This enables the appropriate workflow for both new and existing projects.
