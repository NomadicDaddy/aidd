 ...
 *      point of having one.
 *
 * There is deliberately no `legacy` class. Rules for artifacts aidd no longer writes were removed
 * on 2026-07-16 along with the stale files themselves — a rule kept only to hide files nobody
 * produces is indistinguishable from drift, and it defeats the orphan-rule check above.
 *
