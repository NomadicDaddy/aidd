#!/usr/bin/env bun
import { existsSync, readFileSync, statSync } from 'node:fs';
import { resolve } from 'node:path';

import type { CrawlReport } from './crawltest-types.ts';
import type { MetricSnapshot } from './lib/crawltest/vitals-p75.ts';

import { distIndexHash } from './lib/crawltest/build-identity.ts';
import {
	evaluateCrawlVitals,
	percentilesByMetric,
	percentilesByPage,
} from './lib/crawltest/vitals-p75.ts';

const STALE_AFTER_MS = 24 * 60 * 60 * 1000;
const TOP_N = 5;
const RATING_SCORE: Record<string, number> = {
	good: 0,
	'needs-improvement': 1,
	poor: 2,
};

interface PageIssue {
	issues: MetricSnapshot[];
	page: string;
	worstScore: number;
}

function markerFor(rating: string): string {
	if (rating === 'good') return 'ok';
	if (rating === 'needs-improvement') return 'warn';
	return 'bad';
}

function formatValue(name: string, value: number): string {
	if (name === 'CLS') return value.toFixed(3);
	return `${Math.round(value)}ms`;
}

function printTopSlowest(
	byPage: Map<string, Map<string, MetricSnapshot>>,
	metricName: string,
): void {
	const entries: { page: string; rating: string; value: number }[] = [];
	for (const [page, metrics] of byPage) {
		const metric = metrics.get(metricName);
		if (metric) entries.push({ page, rating: metric.rating, value: metric.value });
	}
	if (entries.length === 0) return;

	entries.sort((a, b) => b.value - a.value);
	console.log(`\nSlowest pages by ${metricName} p75:`);
	for (const entry of entries.slice(0, TOP_N)) {
		console.log(
			`  ${markerFor(entry.rating).padEnd(4)} ${formatValue(metricName, entry.value).padStart(8)} ${entry.page}`,
		);
	}
}

/**
 * Group the crawl's Web Vitals by route, reduced to the nearest-rank p75 of each metric.
 * @param report The crawl report.
 * @returns The per-route p75 snapshots, and the routes whose p75 is not rated good.
 */
export function analyzeCrawlReport(report: CrawlReport): {
	byPage: Map<string, Map<string, MetricSnapshot>>;
	pagesWithIssues: PageIssue[];
} {
	const byPage = percentilesByPage(report);

	const pagesWithIssues: PageIssue[] = [];
	for (const [page, metrics] of byPage) {
		const issues = [...metrics.values()].filter((entry) => entry.rating !== 'good');
		if (issues.length === 0) continue;
		pagesWithIssues.push({
			issues,
			page,
			worstScore: issues.reduce(
				(score, entry) => Math.max(score, RATING_SCORE[entry.rating] ?? 0),
				0,
			),
		});
	}

	pagesWithIssues.sort((a, b) => b.worstScore - a.worstScore || a.page.localeCompare(b.page));
	return { byPage, pagesWithIssues };
}

function printBuildIdentity(report: CrawlReport): void {
	const identity = report.buildIdentity;
	if (!identity) {
		console.log('  Build: not recorded');
		return;
	}
	console.log(
		`  Build: v${identity.version} rev ${identity.revision} (${identity.mode}) built ` +
			`${identity.timestamp}`,
	);
	console.log(`  Origin: ${identity.origin}  index ${identity.indexHash ?? 'unknown'}`);
}

function printPercentiles(report: CrawlReport): void {
	const metrics = percentilesByMetric(report);
	if (metrics.length === 0) return;
	console.log('');
	console.log('Core Web Vitals p75 across all routes:');
	for (const metric of metrics) {
		console.log(
			`  ${markerFor(metric.rating).padEnd(4)} ${metric.name.padEnd(5)} p75 ` +
				`${formatValue(metric.name, metric.value).padStart(8)}  max ` +
				`${formatValue(metric.name, metric.max).padStart(8)}  n=${metric.sampleCount}`,
		);
	}
}

function printPageIssues(pagesWithIssues: PageIssue[]): void {
	console.log('');
	console.log(`Pages whose p75 is not good: ${pagesWithIssues.length}`);
	for (const pageIssue of pagesWithIssues) {
		console.log(`\n  ${pageIssue.page}`);
		pageIssue.issues.sort(
			(a, b) => (RATING_SCORE[b.rating] ?? 0) - (RATING_SCORE[a.rating] ?? 0),
		);
		for (const metric of pageIssue.issues) {
			console.log(
				`    ${markerFor(metric.rating).padEnd(4)} ${metric.name.padEnd(5)} ` +
					`${formatValue(metric.name, metric.value)} (${metric.rating}, n=${metric.sampleCount})`,
			);
		}
	}
}

/**
 * Print the analysis and report whether it is admissible as evidence of p75 compliance.
 * @param report The crawl report.
 * @param reportPath Path the report was read from, for provenance and staleness.
 * @param expectedIndexHash Hash of the production artifact on disk, or null when absent.
 * @returns The failures that disqualify the report; empty when it stands up.
 */
export function printAnalysis(
	report: CrawlReport,
	reportPath: string,
	expectedIndexHash: null | string,
): string[] {
	const ageMs = Date.now() - statSync(reportPath).mtimeMs;
	const { byPage, pagesWithIssues } = analyzeCrawlReport(report);
	const failures = evaluateCrawlVitals(report, expectedIndexHash);

	console.log('');
	console.log('Crawltest Web Vitals Analysis');
	console.log(`  Source: ${reportPath}`);
	printBuildIdentity(report);
	if (ageMs > STALE_AFTER_MS) {
		const ageHours = Math.round(ageMs / (60 * 60 * 1000));
		console.log(`  Warning: report is ${ageHours}h old; consider re-running crawltest.`);
	}

	console.log(`  Pages analyzed: ${byPage.size}`);
	console.log(`  Total metric samples: ${report.webVitals.length}`);

	printPercentiles(report);
	if (pagesWithIssues.length === 0) {
		console.log('');
		console.log('Every route p75 is rated good.');
	} else {
		printPageIssues(pagesWithIssues);
	}

	printTopSlowest(byPage, 'LCP');
	printTopSlowest(byPage, 'FCP');

	if (failures.length > 0) {
		console.log('');
		console.log('This report cannot substantiate p75 compliance:');
		for (const failure of failures) console.log(`  - ${failure}`);
	}
	console.log('');
	return failures;
}

if (import.meta.main) {
	const rootDir = resolve(import.meta.dirname, '..');
	const reportPath = resolve(rootDir, 'logs', 'crawltest.json');
	if (!existsSync(reportPath)) {
		console.log('No crawltest report found at logs/crawltest.json.');
		console.log('Run `bun run crawltest` first to generate it.');
		process.exit(1);
	}

	const report = JSON.parse(readFileSync(reportPath, 'utf8')) as CrawlReport;
	const failures = printAnalysis(report, reportPath, distIndexHash(rootDir));
	process.exit(failures.length > 0 ? 1 : 0);
}
