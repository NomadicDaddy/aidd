...
function buildLine(report: CrawlReport): string {
	const identity = report.buildIdentity;
	if (!identity) return '- Build: not recorded (the report names no artifact)';
	return (
		`- Build: v${identity.version} rev ${identity.revision} (${identity.mode}) at ` +
		`${identity.origin}, index ${identity.indexHash ?? 'unknown'}`
	);
}
