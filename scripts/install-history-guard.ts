 ...
 * appears.
 *
 * WHAT THIS SCRIPT DOES NOT DO. The file list, the copy, idempotence by content, the executable
 * bit, and the refusal to overwrite a target's uncommitted work belong to
 * `sync-shared-core.ts --write` for the `push-guards` group. The hook and every guard it chains are
 * installed as one unit — that is the manifest group, which is why the group and not this
 * script is the place a new guard has to be added. Under `set -euo pipefail` a hook installed
