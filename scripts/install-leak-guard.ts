 ...
 *
 * WHAT THIS SCRIPT DOES NOT DO. The file list, the two hook variants, the copy decisions, the
 * seeded-once files, idempotence by content, the executable bit, and the refusal to overwrite a
 * target's uncommitted work belong to `sync-shared-core.ts --write`, which this delegates to for
 * the `leak-guard-hooks` and `leak-guard-scripts` groups. Those are the sync's rules, not this
 * script's, and a second copy of a sync rule drifting out of step with the
...
/** The manifest groups that together hold everything this installer delivers. */
