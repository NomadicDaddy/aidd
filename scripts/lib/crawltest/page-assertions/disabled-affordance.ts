...
/* Both branches have to answer in the same unit, and the threshold below is stated in OKLCH
   chroma. The widest gap between two rgb channels divided by 255 is a different scale entirely:
   `#1e2330`, the app's dark `--muted`, reads 0.071 that way and 0.026 as real chroma, so every
   quiet surface in the dark theme would be judged a coloured one.
...
async function disabledActionState(
	page: Page,
	textIncludes: string,
): Promise<{ background: string; className: string; disabled: boolean; opacity: string } | null> {
...
export async function assertDisabledActionAffordance(page: Page, route: string): Promise<string[]> {
	...
	if (isProjectDetailRoute(route)) {
		...
		if (!move.disabled) {
			...
			const classes = new Set(move.className.split(/\s+/));
			if (!classes.has('bg-card') || !classes.has('border-control-border')) {
				errors.push(
					`${route} disabled Move project action lost its secondary variant classes`,
				);
			}
			const opacity = Number.parseFloat(move.opacity);
			if (Number.isNaN(opacity) || opacity > 0.55) {
				errors.push(
					`${route} disabled Move project action was not visibly dimmed (opacity ${move.opacity})`,
				);
			}
		}
		return errors;
