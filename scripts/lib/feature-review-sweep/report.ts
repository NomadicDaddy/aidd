...
export function buildPerAppReport(summary: AppSummary): string {
	...
	lines.push(
		`- Non-canonical acceptance_criteria before: ${summary.before.legacyAcceptanceCriteria}`,
	);
	lines.push(
		`- Non-canonical acceptance_criteria after: ${summary.after.legacyAcceptanceCriteria}`,
	);
	lines.push(`- Non-canonical file_locations before: ${summary.before.legacyFileLocations}`);
	lines.push(`- Non-canonical file_locations after: ${summary.after.legacyFileLocations}`);
