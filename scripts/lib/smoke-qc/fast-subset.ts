...
// than lint by 7s, so running it last would spend lint's full cost before reporting a formatting
// error that was already known. spernakit's numbers differ in magnitude (its lint is ~27s) but
// rank identically, so both repos run this same order.
//
