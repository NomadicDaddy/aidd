 ...
 *
 * The direct-dependency tables answer "what did we choose"; this answers "what does the runtime
 * dependency graph contain". aidd ships as source and does not redistribute this graph, but the
 * committed courtesy inventory still covers the whole closure rather than only package.json.
 *
...
/**
 * The runtime closure, taken from the lockfile, with each package's own license text read from disk.
 *
 * WHICH packages belong to the runtime graph comes from bun.lock, because that is identical on
 * every platform. WHAT each one says about its license has to come from the installed copy, because
 * a lockfile records no license and the inventory includes the package's own copyright line.
 * Walking the installed tree for both made the output depend on the install layout — isolated on
 * Windows, hoisted on Linux — and the drift gate could not be satisfied on both at once.
 */
