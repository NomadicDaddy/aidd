...
export function renderNotices(options: NoticesOptions): string {
	const { closure, intro, nonPackageNotices, title } = options;
	...
	const sections: string[] = [
		...
		intro.trim(),
		'',
		`This appendix covers **${closure.length}** third-party packages in the runtime closure.`,
		'',
	];
	...
	sections.push(nonPackageNotices.trim(), '', '## Package notices', '');
