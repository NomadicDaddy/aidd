...
async function runCoverageTests(projectRoot: string, args: string[]): Promise<number> {
	const coverageProcess = Bun.spawn(
		[
			...
			'--coverage-dir',
			TEST_COVERAGE_DIR,
			// Bun's parallel coverage output underreports executed lines, even after its LCOV
			// traces are merged. Keep the coverage authority in one process; ordinary test runs
			// remain parallel, and smoke:qc caches this slower authoritative result.
			'--serial',
			...args,
		],
		{
