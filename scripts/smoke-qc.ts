...
} from './lib/smoke-qc/build-diagnostics.ts';
...
import {
	...
	recordStepResult,
	recordStepSuccess,
	type SmokeCacheStatusEntry,
} from './smoke-cache.ts';

export const PROJECTED_WALL_TIME_WARNING_MS = 540_000;
...
export function projectUncachedWallTimeMs(entries: SmokeCacheStatusEntry[]): number {
	return entries.reduce(
		(total, entry) =>
			total + (!entry.valid && entry.durationMs !== null ? entry.durationMs : 0),
		0,
	);
}

export function shouldWarnProjectedWallTime(projectedMs: number): boolean {
	return projectedMs >= PROJECTED_WALL_TIME_WARNING_MS;
}
...
async function runCommand(step: SmokeQcStep, projectRoot: string): Promise<number> {
	// The build is the one step whose output cannot be recovered by rerunning it: its warnings are
	// emitted once and the step is then cached. Tee it to a retained artifact; every other step
	// keeps the cheaper inherited streams.
	if (capturesBuildDiagnostics(step.name)) {
		const exitCode = await runStepWithBuildDiagnostics(step, projectRoot);
		console.log(`[DIAG] ${step.label} output retained at ${BUILD_DIAGNOSTICS_RELATIVE_PATH}`);
		return exitCode;
	}
...
export async function runSmokeQc(args: SmokeQcArgs, projectRoot = cwd()): Promise<number> {
	...
	for (const step of steps) {
		...
		const exitCode = await runCommand(step, projectRoot);
		const durationMs = Date.now() - startedAt;
		// Commands may create artifacts or the operator may edit during a long-running step. Keep
		// shared reads only across consecutive cache checks, never across executed work.
		clearSmokeCacheEvaluationContext(cacheContext);
