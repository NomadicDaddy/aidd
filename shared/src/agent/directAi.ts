...
export function resolveDirectAiCall(
	...
	// Env beats file, the same precedence `resolveDefaultNativeClientConfig` applies and the same
	// one operator credentials follow. Reading the file alone would reject a provider whose key is
	// deliberately kept out of `config.json`, which is the arrangement `config/env-secrets.ts`
	// asks operators to adopt.
	const apiKey = providerEnvApiKey(provider, env) ?? cleanString(providerConfig?.apiKey);
	...
	if (defaults?.apiKeyRequired === true && !apiKey) {
		...
		throw new DirectAiConfigError(
			`Direct AI provider "${provider}" requires an API key. ` +
				`Set ${envVar ? `${envVar} (or NATIVE_API_KEY)` : 'NATIVE_API_KEY'} ` +
				`in the environment, or add providers.${provider}.apiKey to the config file.`,
		);
	}
