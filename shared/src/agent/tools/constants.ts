 ...
 */
export function resolveRealRoot(cwd: string): string {
 ...
 *
 * Exported: the bash workspace policy reuses this so `bash` and the structured filesystem
 * tools enforce one symlink boundary rather than two that drift apart.
 */
export function verifyRealPathWithinRoot(lexicalPath: string, realRoot: string): null | string {
