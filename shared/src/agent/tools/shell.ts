...
export async function runBash(args: Record<string, unknown>, cwd: string): Promise<string> {
	...
	try {
		...
		if (timedOut) {
			// The Windows taskkill tree sweep inside killProcessTree can take seconds to
			// enumerate; a short grace cap reaches its direct root-kill fallback quickly while
			// the sweep keeps running, and proc.exited resolves as soon as the root dies.
