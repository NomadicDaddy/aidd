...
import { applyEnvSecrets } from './env-secrets.ts';
...
export interface ResolveConfigOptions {
	...
	/**
	 * Source for the credential overlay, defaulting to the real environment.
	 *
	 * Injectable because the overlay makes the resolved config depend on the ambient shell: a
	 * test asserting the exact shape of `config.web` passes on a machine with no
	 * `AIDD_WEB_AUTH_TOKEN` and fails on the machine of anyone who took this indirection's
	 * advice. Pinning the source is what keeps the assertion about the resolver.
	 */
...
export async function resolveConfig(
	...
	// Environment-supplied credentials overlay the user config only. Project configs are
	// restricted below and must never carry one.
	const userConfig = applyEnvSecrets(
		await readConfig(options.userConfigPath ?? getUserConfigPath()),
		options.env,
	);
