...
export function dedupDirectorSuggestions(suggestions: DirectorSuggestion[]): DirectorSuggestion[] {
	...
	for (const suggestion of suggestions) {
		// Include the targeted artifact identity (filterValue / feature) so multiple
		// per-artifact suggestions in the same (project, taskType) bucket survive, while
		// true duplicates (same artifact, or same aggregate bucket) still collapse.
		const target =
			suggestion.suggestedArgs?.filterValue ?? suggestion.suggestedArgs?.feature ?? '';
		const key = `${suggestion.projectId ?? ''}\0${suggestion.taskType}\0${target}`;
