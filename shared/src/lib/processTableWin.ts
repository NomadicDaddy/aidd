...
/** szExeFile: WCHAR[260], null-terminated, and the tail of the struct (44 + 520 = 564, padded). */
const OFFSET_EXE_FILE = 44;
const MAX_EXE_FILE_CHARS = 260;
...
/** szExeFile as a JS string: UTF-16LE up to the first NUL. Free — it is already in the struct. */
function readExeName(view: DataView): string | undefined {
	const chars: number[] = [];
	for (let index = 0; index < MAX_EXE_FILE_CHARS; index += 1) {
		const code = view.getUint16(OFFSET_EXE_FILE + index * 2, true);
		if (code === 0) break;
		chars.push(code);
	}
	return chars.length > 0 ? String.fromCharCode(...chars) : undefined;
}
