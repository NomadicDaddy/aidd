...
async function listLeaseFiles(
	leaseDir: string,
): Promise<{ path: string; record: FeatureLeaseRecord }[]> {
	let names: string[];
	try {
	...
	const files: { path: string; record: FeatureLeaseRecord }[] = [];
	for (const name of names) {
		if (!name.endsWith('.json')) continue;
		const path = join(leaseDir, name);
		const record = await readLeaseRecord(path);
		if (record !== null) files.push({ path, record });
	}
	return files;
}
...
async function removeRunLeases(leaseDir: string, runId: string): Promise<number> {
	let removed = 0;
	for (const { path, record } of await listLeaseFiles(leaseDir)) {
...
/**
 * Every feature lease currently held by a LIVE run, read through the same shared lease directory
 * acquisition writes to. Leases whose recorded pid is provably dead are omitted: the next
 * acquisition reclaims them, so reporting one as held would name a run that is already gone.
 *
 * This is the read side of the lease that non-coding runs need. A coding run's lease stops another
 * *coding* run from selecting the same feature, but a directive or skill run selects nothing and
 * therefore holds no lease — so nothing stopped one from consolidating, retitling, or deleting a
 * feature another run had claimed and was actively working on. Callers use this to tell such a run
 * which records are off-limits, and to report it when one is destroyed anyway.
 *
 * Returns `[]` when there is no lease directory (not a git repository, or nothing has ever leased
 * against this project).
 */
export async function readActiveFeatureLeases(projectDir: string): Promise<FeatureLeaseRecord[]> {
	const leaseDir = await resolveFeatureLeaseDir(projectDir);
	if (leaseDir === null) return [];
	return (await listLeaseFiles(leaseDir))
		.map(({ record }) => record)
