...
/** Index accepting either identifier a dependency ref may legitimately use. Feature deps are
 * written by id, roadmap deps by directory, and derived projects have id !== directory — so both
 * keys must resolve. Single source for that rule: `dependenciesAreSatisfied` and the graph builder
 * share it, which is what keeps a satisfied dependency from rendering as unresolved. */
