...
export function summarizeFeatures(features: Feature[]): FeatureStats {
	const stats: FeatureStats = {
		byCategoryPriority: [],
		closed: features.filter(isFeatureComplete).length,
		...
		failing: features.filter(
			(feature) => !isFeatureComplete(feature) && feature.status !== 'waiting_approval',
		).length,
		open: features.filter(
			(feature) => feature.status !== 'completed' && feature.passes !== true,
		).length,
		passing: features.filter(isFeatureComplete).length,
...
/**
 * The dependencies that would stop selection picking this feature up: a ref nothing resolves to, or
 * a record that resolves but is not complete. Returned in declaration order and named exactly as the
 * record declares them, because a surface that reports a feature as blocked has to be able to say
 * which prerequisite the operator should go and look at.
 */
export function unsatisfiedDependencies(feature: Feature, allFeatures: Feature[]): string[] {
	const dependencies = feature.dependencies ?? [];
	if (dependencies.length === 0) return [];
	const byRef = indexFeaturesByRef(allFeatures);
	return dependencies.filter((dependencyId) => {
		const dependency = byRef.get(dependencyId);
		return dependency === undefined || !isFeatureComplete(dependency);
	});
}

/** The same rule as a yes/no, for callers that need the verdict rather than the reason. */
export function dependenciesAreSatisfied(feature: Feature, allFeatures: Feature[]): boolean {
	return unsatisfiedDependencies(feature, allFeatures).length === 0;
}
