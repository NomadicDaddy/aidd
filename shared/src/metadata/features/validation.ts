import { classifyDependencyCycles, findDanglingDependencies } from './graph.ts';
import { indexFeaturesByRef } from './indexByRef.ts';
import { isAuditFinding, isFeatureComplete, isRemediationFeature } from './query.ts';
import {
	type Feature,
	FEATURE_ID_PATTERN,
...
export function validateFeatureContract(feature: Feature): FeatureValidationIssue[] {
	...
	if (feature.passes === true && !isFeatureComplete(feature)) {
		issues.push({
			id,
			message: `Feature has passes=true but status is '${feature.status ?? 'missing'}'; only completed features may pass`,
		});
	}
...
export function validateFeatureCollection(features: Feature[]): FeatureCollectionValidationResult {
	...
	for (const feature of features) {
		...
		// Audit-derived records are exempt: their id is minted from the finding, not the directory.
		// Keyed on auditSource, not the `audit-` id prefix -- the prefix alone let any hand-written
		// feature opt out of the id/directory check just by naming itself `audit-...`.
		if (typeof feature.auditSource === 'string' && feature.auditSource !== '') continue;
		issues.push({
...
function validateDependencyEdges(features: Feature[]): FeatureCollectionValidationResult {
	...
	const isSettled = (nodeName: string): boolean => {
		const feature = byRef.get(nodeName);
		return feature !== undefined && isFeatureComplete(feature);
	};
