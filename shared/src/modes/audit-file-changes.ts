...
import { auditReportPath } from '../metadata/audit-freshness.ts';
...
export function recordFindingLedgerWrite(
	changes: ModeFileChanges,
	projectDir: string,
	existedBefore: boolean,
): void {
	const path = metadataPath(projectDir, 'findings-ledger.jsonl');
	(existedBefore ? changes.modeFilesEdited : changes.modeFilesCreated).push(path);
}

export async function snapshotFindingLedger(projectDir: string): Promise<boolean> {
	return await pathExists(metadataPath(projectDir, 'findings-ledger.jsonl'));
}
