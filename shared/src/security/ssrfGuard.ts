 ...
 * configuration. This enumerated list must be revisited when cloud credential endpoints change.
 */
export const BLOCKED_METADATA_HOSTS = new Set([
	...
	'metadata', // Search-domain shorthand for the metadata service
	'metadata.goog',
 ...
 */
export function agentBaseUrlValidationError(rawUrl: string, label: string): null | string {
	let url: URL;
	try {
		url = new URL(rawUrl);
	} catch {
		return `${label} base URL is not a valid URL: ${rawUrl}`;
	}
	if (url.protocol !== 'http:' && url.protocol !== 'https:') {
		return `${label} base URL must use http or https (got "${url.protocol.replace(/:$/, '')}").`;
	}
	...
	if (BLOCKED_METADATA_HOSTS.has(host) || host.endsWith('.metadata.google.internal')) {
		return `${label} base URL points at a cloud metadata endpoint (${url.hostname}), which is not permitted.`;
	}
	return null;
}

export function assertSafeAgentBaseUrl(rawUrl: string, label: string): void {
	const error = agentBaseUrlValidationError(rawUrl, label);
