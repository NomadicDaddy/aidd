...
const MAX_AGENT_SKILL_DESCRIPTION_CHARS = 1024;

export interface ImportedSkillRecord {
	category: SkillCategory;
	importedAt: string;
	sourcePath: string;
	/** Hash over every file copied from the import source (path, size, and bytes of each), not just SKILL.md; see hashSkillFiles. */
	sourceSha256: string;
}
...
export interface SkillDefinition {
	...
	contracts?: string[];
	/** Hash of the SKILL.md text alone, after newline normalization (sha256Text); the value telemetry records for a skill-driven run. Support files are not covered. */
...
export function parseSkillDefinition(input: {
	...
	// Agent Skills requires descriptions to contain 1-1,024 characters. This catalog validation is
	// independent of any provider-specific truncation applied when skill listings are displayed.
	...
	return {
		...
		...(contracts ? { contracts } : {}),
		definitionSha256: sha256Text(input.body),
		description,
		extensions,
		id: input.id,
