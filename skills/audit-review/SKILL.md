...
- **Reference audit document**: A catalog document with `type: reference`. The live audit discovery
  code excludes these documents from runnable audits, so review them against their reference purpose
  rather than expecting report history or treating them as executable methodologies.
...
- **Internally-authored audit**: All other runnable audits, written from scratch based on domain
  expertise
- **Audit report history**: Past reports in
  `{app}/.aidd/audit-reports/{AUDIT_NAME}-*.md` across discovered aidd-managed applications under
  the current workspace's configured or instructed application roots
...
4. **Parse YAML frontmatter**: extract `title`, `last_updated`, `version`, `category`, `priority`,
   `estimated_time`, `frequency`, and `lifecycle`; inspect and preserve every additional field that
   is present (for example `applies_to`, `consolidates`, `description`, `min_convex`,
   `prerequisites`, `source`, `trigger`, and `type`)
5. **Classify the audit**:
    - **Reference** if its frontmatter declares `type: reference`
    - **Vercel-derived** if it is runnable and `AUDIT_NAME` is `REACT_BEST_PRACTICES`,
      `COMPOSITION_PATTERNS`, or `WEB_DESIGN_GUIDELINES`
...
Scan across current aidd-managed applications for past reports. Skip this sub-phase for a
`type: reference` document because reference documents are not runnable and do not produce audit
reports.

1. Resolve the application roots at runtime from the invocation, workspace instructions, and
   configured aidd `applicationsRoot` / `web.allowedRoots` values when available. Discover project
   directories carrying `.aidd/` under those roots, then search each discovered project's
   `.aidd/audit-reports/{AUDIT_NAME}-*.md`. Cross-check `<spernakit-root>/spernakit.psd1` when
   present to verify or supplement the Spernakit-derived subset; do not treat that manifest as the
   complete aidd project fleet.
...
1. `.aidd/docs/template/STACK.md` when running from a staged project, otherwise
   `<spernakit-root>/docs/template/STACK.md` in the Spernakit repository: current technology
   versions and architectural rules
2. `.aidd/docs/template/DEVELOPMENT.md` when running from a staged project, otherwise
   `<spernakit-root>/docs/template/DEVELOPMENT.md` in the Spernakit repository: current coding
   conventions
...
For `type: reference`, judge structure against the document's declared reference purpose and do
not require runnable-only material that would misrepresent it as an executable audit. For runnable
audits, verify:
...
**Classification**: {Reference | Vercel-derived | Internally-authored}
**Report history**: {N} reports across {M} applications ({date range}) | N/A - reference document
**Date**: {today}
