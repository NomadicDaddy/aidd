...
`master` locally or as `origin/master`.

aidd's execution intent controls whether confirmed remediations are applied or only described.
The external review still uploads the diff under either intent.
...
- Determine the base ref: use the `[base-ref]` argument if provided; otherwise `main` (local or
  `origin/main`), else `master` (local or `origin/master`). If none exists, review only
  uncommitted changes.
...
- With a resolved base ref, run `coderabbit review --plain --base <base>`.
- With no base ref, run `coderabbit review --plain` so the review covers the uncommitted changes
  without passing a nonexistent ref.
...
- Otherwise run the project's relevant build, typecheck, lint, format-validation, and test scripts
...
- Findings fixed (or proposed under review-only intent), with the file and a one-line description
  for each
...
- This skill never pushes, opens PRs, or posts review comments. Apart from uploading review input
  to CodeRabbit as disclosed above, its only project mutation is editing the working tree.
