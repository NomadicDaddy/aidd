...
- **Same-target-date overwrite is allowed.** Unlike the cross-project `devdiary-update`, this
  skill regenerates the resolved date's entry on demand (the web "Write today's entry" button
  re-runs it for today). Read an existing entry before replacing it and retain accurate details
  that the current evidence does not supersede. Do not assume Git can recover it: repositories
  with their own push remote may ignore `.aidd/`. Never overwrite a date other than the resolved
  target date.

## Frontmatter Schema (write exactly this shape)

Use this exact authoring shape. The backend derives the indexed date and project identity from the
file path, rejects a conflicting frontmatter date, and can fall back to the H1 when reading an
older entry without a title. New entries still emit every key below. Quote the string values with
single quotes; escape an embedded single quote by doubling it (`''`).

```yaml
---
date: 2026-06-12 # required; must equal the date in the file path (YYYY-MM-DD)
project: 'aidd' # required; the project directory name
title: 'The Diary Lands' # required; evocative and specific
...
generatedBy: '' # optional; set only when the pipeline session id is explicitly available
---
```
...
3. **Run ledger**: `.aidd/runs.jsonl` (canonical). Select terminal runs whose `startedAt` or
   `endedAt` falls in the window; note mode, exit code, summary, stop reason, and duration.
...
10. Compose the entry body using this template:
    ...
    - **Prose style**: unlike the cross-project diary, these entries are the finished product (read
      ...
      `<aidd-root>/skills/humanize-docs/SKILL.md` in the aidd repo); read it before drafting.
      Minimum bar if unavailable: no aphorism or lesson lines, no "X isn't Y - it's Z"
      contrastive framing, at most one analogy, no punch fragments, no em-dashes in body prose,
      no AI filler (delve, leverage, robust, seamless), and let some bullets state facts without
      landing a point. Keep reflections tied to a concrete project, file, version, run, or decision;
      omit the section when there is nothing specific to say.
...
- The web UI uses its own Markdown parser. Prefer headings, paragraphs, lists, blockquotes, links,
  emphasis, and inline code; use tables or fenced code only when they materially help, and do not
  rely on raw HTML.
