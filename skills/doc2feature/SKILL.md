...
- `[app]`: Optional. If omitted, infer it from a document or directory path beneath a configured
  application root. If inference fails, return a usage error with the discovered candidate paths.
...
- **Issue feature**: A feature.json describing what must be verified to confirm the issue is fixed.
  Issue findings use the `remediation-{YYYYMMDD}-{slug}` process shape in any repository, derived
  applications included; clean slugs are for standalone capability work.
...
- Read the project's manifest (`package.json`, `pyproject.toml`, `Cargo.toml`, or equivalent) for
  its name, version, script names, and `spernakit_version` when present
- Read `.aidd/project.md` if present for project-specific directives, then read the architecture
  sources it names
- For a Spernakit-derived app, read `.aidd/docs/template/STACK.md` (staged;
  `<spernakit-root>/docs/template/STACK.md` in the Spernakit repository) for canonical
  architectural rules. Other projects use their own architecture sources.

#### 2b. Backend Topology

Map these common directories onto the target's actual structure, and skip a layer that the project
does not have.

- List the route directory (commonly `backend/src/routes/`) to map route files to domain areas
- List the service directory (commonly `backend/src/services/`) to map service files to domain areas
- List the schema directory (commonly `backend/src/db/schema/`) for the data model inventory
- Identify the actual route registration file. Common examples are
  `backend/src/create-api-app.ts`, `backend/src/app.ts`, and aidd's `backend/src/server.ts`.
- Note the actual API prefix convention (for example, `/api/v1/` or `/api/`)

#### 2c. Frontend Topology

Map these common directories onto the target's actual structure, and skip this subsection when the
project has no frontend.

- List the page directory (commonly `frontend/src/pages/`) to map pages to URL paths
- List the API-client directory (commonly `frontend/src/api/`) to map clients to domain areas
- Identify the actual route registration files. Current Spernakit apps import pages through
  `frontend/src/routes/lazyPages.ts`, define route objects in
  `frontend/src/routes/routeGroups.tsx` and `frontend/src/routes/settingsRoutes.tsx`, and assemble
  them in `frontend/src/routes.tsx`; aidd registers its routes in `frontend/src/App.tsx`. Do not
  assume either layout for other stacks.
- Identify navigation configuration (sidebar config, nav constants, or the project's equivalent)
...
When processing interview responses, check for `.aidd/response-review.md`. If present and it
assesses the source corpus, read it in full. This file is a prior critical review of the raw
...
If a response review was loaded and it assessed severity or priority for this claim differently
than the raw document implies, prefer the review's calibration when current code verification does
not contradict it; the review was produced after cross-referencing the full response set and the
codebase.
...
**Template upstream tagging**: If a claim passes the gate AND the affected code is covered by a
template feature (identified by `spernakit_version` in Phase 2d), the resulting issue feature.json
must include `"notes": ["UPSTREAM: root cause is in template feature {template-feature-id}. Fold
fix into Spernakit via the aidd-local consolidate-features skill after verification."]` so the
upstream debt is tracked and eventually synced back to the template. All newly written `notes`
values use arrays.
...
**Application**: {app-name}{optional template version}
**Source document(s)**: {document filename or corpus filenames}
**Response review**: {loaded | not found | not applicable}
...
| --- | ------------------ | ------------- | -------- | -------- | -------------------- |
| 1   | {issue-feature-id} | {issue title} | Security | 1        | claim-001, claim-003 |
...
Where `{feature-id}` is the full issue-finding ID (for example,
`remediation-20260403-license-no-signature` in either an application or Spernakit).
...
Feature persistence is **not complete** until every newly written feature.json is assigned in
`.aidd/roadmap.json`. After writing the files in Phase 8 and before the Phase 9 verification:

1. Read `.aidd/roadmap.json`. If it does not exist, **create it
   first**: a single `v1.0` milestone (priority 1) mapping every existing feature directory and
   preserving each feature's dependencies (keyed by directory). This is the shape the coding
   runtime auto-creates on first run. If the file exists but defines no milestones, initialize the
   same `v1.0` milestone and map any unmapped feature directories before continuing.
2. For each issue feature created in Phase 8, set
   `roadmap.features["{feature-id}"] = { "milestone": "{target}" }` (merge; preserve any existing
   `dependencies` on that key). The target is the current milestone: the first milestone in
   priority order with unfinished work, or the last existing milestone when all work is complete.
   Do not create a new milestone. The owner may re-target any feature afterward.
...
| ------------------ | ------------- | -------------------------------- |
| {issue-feature-id} | {issue title} | .aidd/features/{id}/feature.json |
