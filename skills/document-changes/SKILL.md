...
12. **Net-new functionality without a feature.json**: create
    ...
    - `completedAt`: the same current ISO timestamp when the new record is completed; omit it while
      the feature remains open.
...
13. **Modified existing feature**: update its `feature.json` in place:
    ...
    - When appending numbered `spec` criteria, continue from the number on the last numbered line in
      document order. Existing later blocks may restart at `1`; never use the global maximum.
    ...
    - Set `completedAt` to the current ISO timestamp only when this revision moves the feature into
      `completed`. Preserve an existing completion timestamp for an already-completed feature, and
      remove it when reopening the feature.
...
14. **Revision detail + version stamp** (both create and update): normalize `notes` at the write
    boundary — missing/null to `[]`, a string to `[existingString]`, an array to a copy — then append
    the structured entry and persist the result as an array. Preserve every existing string. Stamp
    the shipped version in the same edit:
