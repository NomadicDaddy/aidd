...
- `--include-completed`: include completed feature JSONs in spec-completeness review, not only
  coverage matching.
...
Read every `.aidd/features/*/feature.json` regardless of status. The current status vocabulary is
`backlog`, `in_progress`, `waiting_approval`, and `completed`; `completed` is the only completion
status, and a completed record is verified when `passes` is `true`.
...
- whether it carries `spernakit_version`, and whether the target is the Spernakit template
  repository itself
- whether the directory name matches `id`
- whether the spec is complete enough to reconstruct the implemented capability

A `spernakit_version` record is template-owned and read-only outside the Spernakit repository. In
the Spernakit repository itself, those records are the editable source of truth. Template-owned
copies can count as coverage but must not be modified.
...
```json
{
	"affectedFiles": ["<project-relative implementation path>"],
...
- Before appending `notes`, normalize missing/null to `[]`, a string to `[existingString]`, and an
  array to a copy; preserve existing entries and persist the appended result as an array.
- When appending numbered `spec` criteria, continue from the number on the last numbered line in
  document order. Later blocks may restart at `1`, so never use the global maximum.
- Preserve `id`, `createdAt`, `completedAt`, `justFinishedAt`, `spernakit_version`, and any other
  fields outside the confirmed tightening.
...
- template-owned feature JSONs outside the Spernakit repository
...
1. Add each new feature ID to the current milestone: the first milestone in priority order with
   unfinished work, or the last existing milestone when all work is complete. Do not create a new
   milestone; if the roadmap defines none, report the missing assignment instead.
