...
Run from within any project that has `.aidd/features/` with feature.json files.

## Workflow

### Phase 1: Discover & Load

1. Find all `feature.json` files under `.aidd/features/*/feature.json`
...
Check for `.aidd/remediation-review.md`. If present, read it in full. This file is a human-authored critical review of remediation features produced by the aidd-local `doc2feature` skill, containing per-feature assessments with:
...
- **Dependencies**: Must be an array of strings (or absent)
...
- Recommendation: {remove feature | narrow scope to net-new functionality | merge into existing}
...
1. **Missing required fields**: Add with sensible defaults derived from context (e.g., `priority` from dependency depth, `updatedAt` from current timestamp)
...
3. **Missing spec lines**: Add verification items for aspects the category requires but the spec omitted (e.g., backend CRUD missing route registration check, frontend page missing navigation entry)
...
8. **Codebase duplication - full overlap**: If the feature is entirely redundant (all specified functionality already exists), set `status` to `"completed"` and `passes` to `true`, and prepend to the description: `"[AUTO-CLOSED: functionality already exists in codebase]"`. Add a `notes` field listing the existing files that cover it.
9. **Codebase duplication - partial overlap**: If the feature partially overlaps, compute a narrowed
   spec that retains only net-new items. Apply it and clarify the description when at least two spec
   lines remain. Otherwise preserve the feature unchanged and report the unresolved boundary.
...
- **Do not change**: `id`, `status`, `passes`, `createdAt`, `justFinishedAt`, `spernakit_version`. Exception: Fix Rule 8 (full-overlap auto-close) is the ONLY case allowed to change `status`/`passes`, and every auto-closed feature must be itemized in the report.
...
1. Read `.aidd/roadmap.json` from the project root. If it does not exist (lite apps have no roadmap), skip this phase and note `no roadmap.json; assignment skipped` in the Phase 7 report.
...
4. If any assignment was added or changed in step 3, run `bun run aidd-tools -- roadmap:apply --project-dir <project-root>` from `<aidd-root>` and report the updated / unchanged / errors summary. If step 2 left every feature already correctly mapped, skip the command (nothing to propagate) and note `roadmap assignments already consistent` in the Phase 7 report.

### Phase 7: Post-Review Pipeline Handoff

After auto-fix completes:

1. **Count features closed as already-implemented** (fix rule 8)
2. **If any features were auto-closed**, recommend: `Run the aidd-local consolidate-features skill for {app} to clean up redundant features`
3. **Report final feature inventory health**: total features, backlog count, completed count, duplicates removed this session
...
- Do not remove spec lines that are correct even if they seem redundant, unless they duplicate existing codebase functionality (see fix rule 9)
