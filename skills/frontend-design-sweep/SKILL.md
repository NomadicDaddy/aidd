...
- `--features` → also file durable feature records (Phase 6). Default is report only.
...
| Screenshot id  | `{ID}` = `-HHmmss-frontend-design-sweep-{MODE}`           |
...
| --------- | ------------------------------------------------------ |
| `desktop` | `2250x1309`, `2560x1440`, `1920x1200`, `1440x900`      |
...
`{MODE}`. Resolve `{app}` to its absolute path before passing the screenshot destination to browser
...
overwritten report has no history to recover from and is simply gone.
...
```json
{
	...
	"source": "screen-map",
	"tab": { "id": "overview", "label": "Overview", "reach": "click:tab[Overview]" },
...
3. Write a baseline digest of 400 words or fewer to `{work}/baseline.md`: tokens in use, type scale
   ...
   app's existing aesthetic direction stated plainly. Record the design-review lens selected below.
...
Run one subagent per surface in batches sized to the runtime's available concurrency. Dispatch each
batch together so its reviewers run concurrently. If the runtime has no subagents, walk the same
procedure sequentially and say so in the report.

Give each subagent its own browser session (`ds-{RUN}-{MODE}-{n}`) and require it to close that
session.
...
Surface: {label} — {url}
Tab to activate for this surface: {tab label and click path, or "none"}
...
Design-review lens: {frontend-design review instructions only, or ui-redesign-planner
Phase 2 fallback}. How could this look cleaner, sleeker, and more consistent across the UI?
...
## Phase 6: Feature records (opt-in)

Only with `--features`, or when the user asks. Hand the consolidated findings to
`ui-redesign-planner {app} --source {work}/ui-design-sweep-{MODE}.md` and let it own feature
metadata, milestone assignment, and validation. `frontend-design` may inform the visual review but
does not replace this aidd-owned metadata handoff. Do not restate those rules here and do not write
feature JSON directly from this skill.
...
6. **Coverage honesty beats coverage.** Naming the surfaces you could not reach is part of the
   result.
7. **A tab is a surface.** A sweep that stops at the router misses most of the application.
