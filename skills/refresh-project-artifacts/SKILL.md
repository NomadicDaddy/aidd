...
metadata:
    ...
    aidd-references: >-
        docs/reference/artifacts.md, docs/reference/external-skills.md,
        audits/SEVERITY_CLASSIFICATION.md
    spernakit-references: docs/template/STACK.md, docs/template/DEPLOYMENT.md
---
...
The sibling workflows named by the table are staged under `.aidd/skills/<id>/SKILL.md`. When
running inside the aidd repository and a staged contract is absent, read `skills/<id>/SKILL.md`
from the repository root. Read each relevant contract before handling its artifact. Apply only the
artifact-scoped portion of a contract: this umbrella run does not commit, tag, push, deploy,
release, bump a version, or open an interactive interview unless the user separately granted that
authority.
...
5. Refresh `screen-map.md` from reachable routes. Refresh `testing-scenarios.md` through
   `testing-scenarios {app} --mode refresh`; preserve its header, notes, and Post-Test Procedure as
   that contract requires.
6. Refresh `questions.md` through the interview contract, which preserves questionnaires that
   already have answers. Verify existing response artifacts, but never fabricate, rewrite, or
   reorder interview answers.
7. Refresh `deployment.md` and fill deploy-config gaps from the explicit or inferred deployment
   target resolved by the deployment-readiness contract. Do not deploy.
...
- Screenshots are created only when the corresponding browser workflow actually runs. Do not
  recapture or overwrite browser evidence merely to make the artifact catalog look current.
- `_common/`, `audits/`, `skills/`, and `docs/` are refreshed by aidd's normal staging for this
  run. Verify what was staged rather than hand-editing copied files.
...
Do not regenerate, normalize, truncate, delete, or backfill `findings-ledger.jsonl`, `runs.jsonl`,
`iterations/`, `active-runs/`, or `.stop`. They are run evidence or live control state. The
findings ledger is append-only: a legitimately run audit producer may append lifecycle events, but
the umbrella workflow must never hand-edit it. Do not read secret values into output and do not
edit or expose `.aidd/aidd.config.json`.
