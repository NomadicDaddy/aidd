...
- every repository owned by the active `gh` account, including forks;
- every Git checkout under the applications and public roots named by the applicable `AGENTS.md`;
...
Account-level GitHub Projects should also be absent. Query them separately: disabling the
repository Projects setting does not inventory or remove Projects owned by the account.
...
2. Query all owned repositories with pagination, including forks.
3. Query the account's Projects with GraphQL when the token has `read:project` or `project`; mark
   the control unavailable when neither scope is present.
4. Recursively find `.git` markers under both local roots.
5. Deduplicate local repositories by resolved worktree path.
6. Report counts by active/archived, public/private, fork/source, and local current branch.
...
- account-level Projects and repository Projects, which are independent surfaces;
...
- a local repository is absent from the nearest `AGENTS.md` application inventory, when that
  document maintains one;
...
Treat account-level Project deletion as destructive: include each title, number, and node ID in the
reported batch, then re-read that exact node before deletion.
...
- account-level and repository-level Projects were both checked or explicitly marked unavailable;
