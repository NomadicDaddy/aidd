...
Turn local work into a reviewable GitHub pull request. Invoking this skill authorizes its commit,
push, and pull-request publication steps. Preserve the user's work and never force-push.
...
- `[app]` - Application name beneath an applicable applications root, or an explicit repository
  path. If omitted, use the current working directory.

## Preconditions

- Resolve the target, confirm `git rev-parse --show-toplevel`, and work from that repository. Read
  its applicable `AGENTS.md` files and repository-native validation scripts before acting.
...
regroup, or combine them with other work. Treat a missing upstream as expected for a new branch.
Resolve the base branch from the remote's reported HEAD branch; do not assume `main` or `master`.
...
Check `gh pr list --head <branch> --state open --json number,url,state,isDraft`. If an open pull
request already uses the branch, stop and use an existing-PR workflow instead of creating a
duplicate.
...
Fetch the remote base branch, then synthesize from `git diff origin/<base>...HEAD`, using three-dot
diff semantics so the preview matches the commits the pull request will contain.
