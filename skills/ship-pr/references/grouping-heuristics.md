...
3. **Rename + edit pairs.** When `git status` shows `R foo.ts -> bar.ts` plus an edit, keep them together so `git log --follow` works.
...
- **By directory depth:** `src/` vs `tests/` vs `docs/` regardless of feature. Tests separated from implementation break `git bisect`.
