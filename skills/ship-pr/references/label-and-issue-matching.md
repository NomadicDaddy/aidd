...
Capture `gh label list --limit 200 --json name,description` during the inventory step. **This is
the only valid label source.** `gh pr create` resolves supplied label names against existing
repository labels and rejects unknown names. Do not respond by creating a missing label; that
pollutes the repository's taxonomy.
...
Capture `gh issue list --state open --limit 50 --json number,title,labels,body` during the inventory
step.
