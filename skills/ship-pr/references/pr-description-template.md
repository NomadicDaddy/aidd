...
- [x] <Name a concrete check that passed and is tied to changed behavior.>
- [ ] <Name a required check that remains; include only when applicable.>
```

Mark a check complete only when current evidence shows it passed. An unchecked required check is
missing readiness evidence, so retain it in the test plan and open the pull request as a draft.
