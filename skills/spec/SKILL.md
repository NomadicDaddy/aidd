...
```json
{
	...
	"category": "{established project category}",
	...
	"passes": false,
	"priority": {target milestone numeric priority},
	"spec": "1. Verify ...\n2. Verify ...\n3. Verify ...",
	"status": "backlog",
...
- New `notes` values are arrays. When revising an existing record, normalize missing/null to `[]`,
  a string to `[existingString]`, and an array to a copy before appending; preserve every existing
  string and persist the result as an array.
- Use the project's existing category vocabulary. If it has none, choose the primary delivery area
  (`UI`, `Backend`, `Database`, `Security`, or `Core`) instead of inventing a narrow one-off label.
- `priority` mirrors the numeric priority of the assigned roadmap milestone. Do not rank the record
  independently: aidd's run-end reconciliation writes the milestone priority back to
  `feature.json`.
- For a new record, do not add fields beyond those shown. When revising an existing record,
  preserve every field outside the confirmed change, including passthrough fields the template
  does not list.
...
- When appending numbered clauses to an existing `spec`, continue from the number on the last
  numbered line in document order. Later blocks may restart at `1`; never use the global maximum.
...
- Set `roadmap.features["<directory>"] = { "milestone": "<target>", "dependencies": [...] }`,
  merging with any existing entry rather than replacing it. Roadmap dependencies name feature
  directories; translate a feature-record dependency id to its directory when those values differ.
- An explicit milestone from the source or project owner wins when it exists in the roadmap.
  Otherwise place new capability work in the future backlog selected by current aidd policy: reuse
  an existing empty or later-priority milestone; on an active roadmap with no separate future
  bucket, create the next `vX.0` milestone after the highest versioned milestone. On `lts` or
  `locked` roadmaps, never create a milestone; use the first unfinished milestone in priority order,
  or the last existing milestone when all work is complete.
- If `.aidd/roadmap.json` does not exist, create it first with a single `v1.0` milestone at
  priority 1 mapping every existing feature directory and preserving its dependencies, then apply
  the same assignment policy to the new record.
...
- Re-read every file written and confirm it parses. Ids are unique, each directory holds a
  `feature.json`, every feature-record dependency resolves to an existing id or directory, roadmap
  dependencies name existing feature directories, and no dependency cycle was introduced.
...
- Check every written artifact against the prompt-guidelines pre-send checklist
  ...
  rides along.
- Do not shell into the aidd installation to validate. aidd validates feature contracts after each
  iteration and reconciles roadmap priorities and dependencies at run end; report any issues it
  returns with the run.
