...
Re-skin Spernakit without changing behavior; record net-new behavior as backlog stubs, not inert UI.
...
- Every route declared through `frontend/src/routes/routeGroups.tsx` and
  `frontend/src/routes/settingsRoutes.tsx` stays reachable with its existing path and role guard;
  `frontend/src/routes.tsx` remains the guarded router assembly, not a page registry
...
**Code + screenshots coexisting.** When a source has both `src/` and `screenshots/`, code governs
visual structure and component hierarchy; screenshots govern visual polish/tokens where they
disagree. The target remains authoritative for routing, state, data flow, and behavior. Catalogue
both and flag unresolved visual disagreements for user resolution.
...
- Routes in `frontend/src/routes/routeGroups.tsx` and `frontend/src/routes/settingsRoutes.tsx`,
  lazy imports in `frontend/src/routes/lazyPages.ts`, and guarded assembly in `frontend/src/routes.tsx`
  (`React.lazy` named-export adapter pattern must stay intact)
...
- **Source routing is discarded.** Source hash routers, state-machine `currentPage` switches, and
  prop-drilled `onNav` callbacks are never carried over. Keep only the JSX bodies; navigation is
  rewired to `useNavigate` against the existing route objects. Any source route with no target
  route becomes a net-new stub.
...
- Font changes: self-host exact-version font packages and import them in
  `frontend/src/tailwind.css`. Do not add remote font imports or preconnects; Spernakit's CSP
  permits fonts only from `self` and `data:`.
...
6. **Net-new elements**: load the staged `ui-parity` contract from
   ...
   Do not render controls backed by no-op handlers, mock data, or missing target behavior. For each
   deferred element, create `.aidd/features/<slug>/feature.json` using the contract's format:
...
- Update `spec` points whose described elements, names, or interactions materially changed, and
  refresh `updatedAt` when the record itself changes.
- In a derived app, do not edit template-owned records. Upstream required text changes to
  Spernakit and let the template-feature sync carry them back.
- In Spernakit itself, update the template record when appropriate but preserve
  `spernakit_version`: it is the version that introduced the record, not a revision stamp.
...
| Implies new data / new endpoint / backend change        | **Flag**: create a `.aidd/features/<slug>/feature.json` stub, do not silently add backend code |
...
- Spernakit canonical rules: `docs/template/STACK.md` and `docs/template/DEVELOPMENT.md`
- Architectural constraint: every feature wired end-to-end, every backend route registered through
  `backend/src/create-api-app.ts` or a registered domain aggregator, every frontend page imported through
  `frontend/src/routes/lazyPages.ts` and registered in `frontend/src/routes/routeGroups.tsx` or
  `frontend/src/routes/settingsRoutes.tsx`, and every route reachable through navigation or an
  in-page path
