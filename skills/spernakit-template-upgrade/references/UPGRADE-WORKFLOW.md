...
- **Default target**: Current Spernakit checkout version (from `<spernakit-root>/package.json`)
- **Explicit target**: Specify `--to 2.5.1` to upgrade to a specific version

The source must be v3.29.0 or later. The complete workflow also needs a Spernakit checkout at the
target version because template feature metadata is gitignored and has no tagged baseline. For an
older explicit target, the tag can drive code review but cannot reconstruct Phase 9a. Stop with a
blocker unless a preserved checkout at that version also carries its matching live `.aidd/` corpus;
never sync feature metadata from a newer checkout.
...
These tracked documents ship in each derived application and live at
`<target-template-root>/docs/template/` in Spernakit.

### Phase 0: Version Discovery

1. **Read source version** from `{app}/package.json` → `spernakit_version` field (e.g., `"2.4.0"`)
2. **Read target version** from `<spernakit-root>/package.json` → `version` field, or from `--to` argument
3. **Validate versions**:
    ...
    - Source must be v3.29.0 or later; older apps must be initialized from the current template
      because the sync packet has no supported public baseline for them
    - `<target-template-root>/package.json` must declare `{target}` and its live `.aidd/` corpus must
      belong to that version; a tag-only worktree is insufficient because the corpus is gitignored
...
6. **Generate a read-only sync packet** from the template repo when the target app already declares
   a concrete `spernakit_version`:
   `bun run template:sync-plan -- --app ../<app> --from <source-version> --to <target-version>`.
   Review `upgrade-review/<app>/summary.md`, `pure-copy.txt`, `branded-copy.txt`,
   `infrastructure-review.txt`, `infrastructure-diffs/`, and `blocked-app-owned.txt` before applying
   any file changes. The packet compares the whole app with the target; `--from` is validated and
   printed but does not filter its candidates. Intersect every apply candidate with
   `git diff --name-only v{source} v{target}` so pre-existing app drift is reviewed, not copied as
   part of this release.
7. **Review override deltas** from the app before copying:
   `bun run check:override-deltas -- --target-version {target}`. Account for what every `KEEP` or
   `SKIP` entry withholds and remove entries reported under `WITHHOLDING NOTHING`.
8. **Find retained template deletions** from the app:
   `bun run check:drift -- --target-version {target}`. Add its deletion candidates to the Phase 6
   set; the ordinary drift baseline and release packet cannot reveal files removed before
   `v{source}`.
9. **Capture override baselines** for every `.templateoverrides` path, including whether each path
...
Read `docs/template/CHANGELOG.md` from `v{target}` and extract all entries between the source and
target versions (inclusive of target, exclusive of source).
...
The review packet is deterministic and read-only; applying it still requires file-by-file judgment.
Plan for ~3-5 minutes per app for small deltas (<20 files), longer for large deltas. The drift checker
(`bun run check:drift`) is the source of truth for sync status; read its ordinary and
`--target-version` output before starting Phase 5.
...
#
# Actions:
#   DELETED: A one-sided absence is intentional: either the app dropped a file the template ships,
#            or the template dropped a file the app intentionally retains
...
`scripts/template-manifest.json` must be byte-identical to Spernakit's target checkout. The manifest
is fleet-wide ground truth; branded/infrastructure classifications never differ per app, only
per-app file _content_ differs.

```bash
cp "<target-template-root>/scripts/template-manifest.json" "<applications-root>/{app}/scripts/template-manifest.json"
```
...
2. **Copy the file directly** from the checkout at the target tag:
    ```bash
    mkdir -p "$(dirname "<applications-root>/{app}/{path}")"
    cp "<target-template-root>/{path}" "<applications-root>/{app}/{path}"
    ```
...
1. **Check `.templateoverrides`**: respect `KEEP`, `SKIP`, and `DELETED` entries by leaving the file
   in place. For a removal, `DELETED` acknowledges that the template dropped a file the app
...
3. **Diff against the Spernakit v{source} version** to check for local modifications:
    ...
    - If the diff contains domain extensions → **preserve the file**, record the unresolved
      app-owned behavior as a blocker, and continue with independent work. Do not silently delete or
      classify the conflict as resolved.
...
**Strategy**: Copy verbatim from `<target-template-root>`, then reapply branding by substituting
Spernakit's slug, name, ports, and cookie names with the derived app's equivalents from its
`config/*.json` and `package.json`.
...
After Phases 5-7 complete and before moving on to Phase 8, compare every `SKIP` and `KEEP` path with
the exact existence/content snapshot captured in Phase 0. For each `DELETED` entry, preserve its
recorded one-sided state: absent when the target still ships the path, or retained when the target
removed it. Do not infer upgrade touches from `git diff --name-only HEAD`: that conflates the
upgrade with staged or unstaged edits that existed before it began.

If any protected path differs from its snapshot or a `DELETED` path changes sides, restore it from
the Phase 0 snapshot, preserve the override entry, report the path, and rerun the gate. Never use a
destructive working-tree command or discard pre-existing user edits. This gate runs unconditionally;
with no `.templateoverrides` file it passes trivially.
...
3. **Run `bun run db:generate`** and review the generated SQL and migration journal; never patch
   generated migrations by hand
4. **Reset development data with `bun scripts/reset-database.ts --force`, then run
   `bun run db:setup`** so the generated migrations are applied immediately
5. **Note**: Do NOT touch app-specific schema files, only template-managed tables. Update both
   SQLite and PostgreSQL schema dialects together.

### Phase 9a: Template Feature Sync

Copy Spernakit's feature records (`.aidd/features/<dir>/feature.json` plus `.aidd/roadmap.json`) into
the derived app. This phase is now a script, not a judgment call — do not hand-copy directories, and
do not decide ownership by reading `spernakit_version`. Run it from **`<target-template-root>`**, in
push mode: the app has not been re-stamped yet (that is Phase 10), and the pull form deliberately
skips on version mismatch. The script reads gitignored feature metadata from the working tree, not a
tag, which is why Phase 0 requires this checkout to match `{target}`.

```bash
# from <target-template-root>
...
Do not restate the sync's rules from memory — the script is the definition, and `docs/template/DEVELOPMENT.md` § Template Feature Sync is the prose. Two properties are worth knowing because a hand-run sync gets them wrong:

- **Process records never sync.** `remediation-<date>-…` and `audit-<slug>-<digits>-…` are findings from the template's own development. Their content reaches apps by being folded into a durable feature upstream; the finding itself is deleted once folded. A copy of one found in an app is pruned — a _copy_, meaning a record carrying `spernakit_version`: `planPrunes` skips every unmarked directory, so a finding the app authored under the same naming shape is never deleted by the sync. The pattern is narrower than a bare `audit-` prefix on purpose — `audit-logs` is a durable capability record.
...
2. **Run `bun run smoke:qc`** from the application root
3. **Fix any failures**; iterate until the quality gate passes
4. **Run a targeted crawl test** if UI-facing changes were made:
    ```bash
    bun scripts/crawltest.ts --page /settings
    bun scripts/crawltest.ts --page /dashboard
    ```
5. **Run `bun run check:drift`** to verify no accidental drift was introduced during the upgrade. The report should show fewer or equal drifted files compared to the pre-upgrade baseline from Phase 0.
6. **Clean up `UPGRADE-PLAN.md`**: delete the temporary plan, or move it to `docs/` when it should
   remain as a tracked record.
7. **Commit** with `chore: sync spernakit template to v{target}` (or the short form
   `sv{target}`).
8. **Run the lost-lines audit against that commit**:
   `bun run audit:lost-lines -- --app-dir <applications-root>/{app} --rev <upgrade-commit>`. A
   non-zero result is blocking: restore each app-authored line or document the deliberate removal,
   amend the commit, and rerun the audit. A green `smoke:qc` does not replace this check.
9. **Run `bun run fleet-manifest:sync` from `<spernakit-root>` before the upgrade commit.** It
   restates the app's `spernakit.psd1` entry from `{app}/package.json` and runtime
   `config/<slug>.json`; do not edit the ignored manifest by hand. Include any tracked registry
   correction in the upgrade commit. For a dance, rerun the sync after each app is tagged, then
   require `bun run check:fleet-manifest` after all scoped apps are complete.

### Phase 10.5: Template Override Staleness Check

If `.templateoverrides` exists, rerun
`bun run check:override-deltas -- --target-version {target}` after the merge:

1. **For `DELETED` entries**: Confirm the intended one-sided state. A path the target still ships
   must remain absent from the app; a path the target removed may remain in the app when the reason
   explains that retention.
2. **For `SKIP` and `KEEP` entries with deltas**: Account for each withheld target line, re-merge
   applicable changes, and update the reason to say what the entry withholds at this version.
3. **For `WITHHOLDING NOTHING` entries**: Remove the obsolete override so future drift is visible.
4. **For `UNRESOLVED` entries**: Treat the non-zero result as a blocker; do not call the override
   clean when the tool could not compare it.

Include the report and each override decision in the summary.
...
- lost-lines audit: {PASS/FAIL/PENDING COMMIT}
```
...
5. **Domain code is sacred**: Never overwrite domain-specific logic. When template and domain
   ...
   independent changes
6. **Version stamp before the final gate**: Update `spernakit_version` only after file, schema, and
   feature sync, then run `smoke:qc` at version parity
...
- If `bun run smoke:qc` fails after stamping the template version, fix issues iteratively and rerun
  the gate; do not roll the stamp back to make parity checks skip
