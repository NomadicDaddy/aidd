...
metadata:
    ...
    aidd-contracts: tester, testing-scenarios, bug2feature
    spernakit-references: docs/template/TESTING.md, docs/template/STACK.md
---
...
- Select **Feature Request** when the required capability does not exist.
...
When the caller is `spernakit-dance`, finish the normal `tester` report and then return one JSON
object with this exact shape. The Dance establishes the baseline before exploration and provides
those identities to this run; the tester owns the post-run retrieval and handoff.

```json
{
	"app": "<app slug>",
	"baselineReportIds": ["<app>:<numeric id>"],
	"blockers": ["<exact blocker>"],
	"pagesVisited": ["<route or named surface>"],
	"sessionReports": [{ "<complete API report row>": "<value>" }],
	"submittedReportIds": ["<app>:<numeric id>"]
}
```

Use empty arrays when a category has no entries. `baselineReportIds` and `submittedReportIds` are
app-qualified strings; `sessionReports` contains complete rows from the stabilized post-test API
retrieval for every post-baseline identity. Do not include credentials, tokens, cookies, or CSRF
values. Outside the Dance, use the general `tester` output contract without this JSON appendix.
