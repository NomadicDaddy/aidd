...
| Browser session       | `{SESSION}` = `HHmmss-tester`                            |
| Viewport              | `2250x1309` unless the project or request overrides it   |
| Screenshot id         | `{ID}` = `-{SESSION}`                                    |
| Evidence directory    | `{APP_DIR}/.aidd/screenshots/{YYYYMMDD}{ID}/` (absolute) |
...
Resolve `{SESSION}`, `{YYYYMMDD}`, and `{ID}` once from the same local run-start time. Resolve
`{APP_DIR}` to its absolute path before passing `{EVIDENCE_DIR}` to browser automation; its daemon
may have a different working directory. If the evidence directory already exists, append `-2`, then
`-3`, and so on to `{SESSION}`, recalculate `{ID}` as `-{SESSION}`, and use that final session name.
...
4. **Defect intake.** Decide before testing starts where findings will be filed, in this order of
   preference: an in-app bug-report feature, the project's issue tracker, an aidd feature record
   through the `bug2feature` skill, or the run's final results. `bug2feature` files a defect under a
   `remediation-*` process ID in any repository, a derived app included; clean slugs are for
   feature requests.
   Never invent a storage file the project does not already use.
...
When testing more than one desktop viewport, also cover `2560x1440`, `1920x1200`, and `1440x900`.
Capture the viewport with each responsive finding so its evidence can be reproduced.
