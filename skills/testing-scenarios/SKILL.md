...
- `--count` → target number of scenarios to generate. Defaults are defined in Setup.
...
| **Mode**  | `augment`, or `seed` when the catalog is missing       | `seed`, `augment`, `refresh`            |
| **Count** | `5` (augment) / `10` (seed) / match existing (refresh) | `15`                                    |
| **Focus** | whole app                                              | `--focus "workflow builder, analytics"` |

### Modes

- **`seed`**: Select automatically when `.aidd/testing-scenarios.md` is missing. Create the file
  from scratch with the full template header, description paragraph, numbered scenarios, and
  Post-Test Procedure footer. If the catalog exists, use `augment` or explicitly select `refresh`;
  `seed` must not overwrite it.
...
6. Generate        Write N scenarios matching the house style
7. Merge           Append, create, or replace according to the selected mode
8. Report          List what was generated and the coverage rationale
```
...
`passes`, and `priority` fields. Treat roadmap items and incomplete features as planning context,
not testable behavior; generate scenarios only for capabilities confirmed in the current source.
...
Diff the target matrix against existing coverage. In `augment` mode, generate only the gap list. In
`seed` or `refresh` mode, generate the complete target matrix up to the selected count; `refresh`
replaces covered scenarios too.

If the user provided a **focus**, filter the applicable generation set to that area and generate
there, even if overall coverage is already high.
...
- Convert confirmed defects into reviewed issue features or authenticated reports
- Run the aidd-local `feature-review` skill for newly created issue features
- Implement and validate actionable issue features without changing completed feature history
...
- **Only author into `.aidd/testing-scenarios.md`** in the target app — it is the sole deliverable.
  Do not edit any other source or metadata file, and never edit another app's scenarios. On a
  mutation run, the orchestrator also requires its run-owned `.aidd/CHANGELOG.md` bookkeeping; that
  entry is expected, not a skill-scope violation. A review-only run must not write either file.
