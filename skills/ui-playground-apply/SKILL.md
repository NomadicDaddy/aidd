...
If `sourceChanges` reports committed changes in copied paths, stop and report the exact
`ui-playground-sync` update command. Do not run that mutating workflow during this read-only review.
Review only after the playground has incorporated those site changes and any conflicts are
resolved. A different site commit with no copied-path changes does not make the UI baseline stale.
