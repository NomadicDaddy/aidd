...
- Preserve an existing playground's package-manager, lockfile, and designer-launcher contract unless
...
may remain supported, but defaults must use the mapped ports. The helper validates ranges,
uniqueness, and overlap between supplied and mapped ports; check listener occupancy separately.
...
Accept a contract file that lacks `schemaVersion` or `portMap` when updating, then bring it to the
current shape after successful validation. Keep path values native to the host platform.
...
6. Verify every copied file's Git blob against the source commit and reject unexpected files in the
   copied trees.
7. Add the smallest root workspace needed to install, type-check, lint, format, test, and build the
   copied frontend. Preserve the source dependency versions and code-quality conventions, while
   selecting a package manager, lockfile, and launcher that fit the intended designer environment.
...
12. Install with the selected package manager, verify its frozen or clean-lockfile install, and run
...
- Regular install followed by the selected package manager's frozen or clean-lockfile install
