...
- **Route Map table**: Route → Screen → Primary Purpose → optional Min Role
- **Screen Details**: per-screen narrative (Purpose, Core Regions, Key Controls)
- **Screen-to-Feature Traceability table**: Screen → feature IDs in `.aidd/features/`
...
- **Route definitions** — wherever the router tree is declared. Commonly `frontend/src/routes.tsx`;
  also frequently `frontend/src/App.tsx`, `frontend/src/router.tsx`, a `routes/` directory, a
  framework's file-based `src/pages/` or `app/` tree, or server-rendered route/template modules.
...
- Page components or templates (`.tsx`, `.jsx`, `.astro`, `.html`, and equivalent server templates),
  whose rendered regions, actions, and component names define the narrative
...
2. Resolve an explicit path directly. Resolve a bare app name under the configured
   `<applications-root>`, matching aidd's `applicationsRoot` project-resolution rule. Read the
   applicable `<applications-root>/AGENTS.md` for repository instructions, not as an application
   registry.
3. Verify `.aidd/` exists.
4. **Discover the route and navigation sources by role**, in this order — do not require any exact
   filename or frontend framework:
    - Route definitions: try `frontend/src/routes.tsx`, `frontend/src/App.tsx`,
      `frontend/src/router.tsx`, then grep the frontend source for the router import in use
      (`createBrowserRouter`, `<Routes>`, `<Route `). If the project uses file-based routing,
      inventory its route tree (`src/pages/`, `pages/`, `app/`, or the framework-configured
      equivalent). For server-rendered or static projects, find the server route registrations,
      template-to-path mapping, or generated page entry points that determine reachable URLs.
    - Navigation items: try `frontend/src/components/layout/navConfig.tsx`, then `nav-items.ts(x)`,
      ...
      component, template, or content collection the menu renders. Navigation may legitimately be
      absent — an app can route without a menu.

    State the files you resolved in your final report, so the map is traceable to its sources.

5. Note whether `.aidd/screen-map.md` exists. If not, the skill will create it from scratch using the standard structure: Primary Navigation prose + Route Map table + Screen Details sections + Screen-to-Feature Traceability table. If another app in scope already has a `screen-map.md`, mirror its section order.
...
1. Read every route-definition source resolved in Phase 1 fully.
2. Inventory concrete user-facing page routes. In an authenticated SPA, walk the application route
   tree under its shell or protection boundary and exclude authentication/callback routes such as
   `/login`, `/register`, `/oauth/callback`, `/mfa/*`, `/reset-password*`, `/verify-email`,
   `/force-password-change`, `/shared/*`, plus the `*` NotFound catch-all. In a public, static, or
   server-rendered application, include its reachable content pages and exclude only non-page
   endpoints, redirects without their own screen, and catch-all/error handlers.
3. For each included route, capture:
    - `path` as written (keep `:id` and other params)
    - The nearest route guard or access declaration, such as
      `<ProtectedRoute requiredRole="X">`; that is the route-level min role when one exists
    - Page component, template, or entry source used to render the path
...
5. **Merge** by path. For each route:
    - Screen label ← nav label if present, else prettified page component, template, or entry name
      (e.g. `ProjectDetailPage` → `Project Detail`)
...
1. Extract rows from the Route Map table: `Route | Screen | Primary Purpose`, plus `Min Role` when
   present. Note which Primary Purpose cells are `TODO`-ish so Phase 5 can replace them with inferred
   content.
...
1. **Locate the page source**: follow the route's component, template, file-based page, or generated
   entry point to the source that renders it. In React apps this is commonly under
   `frontend/src/pages/`; tabs and nested layouts often live under `pages/{domain}/tabs/` or
   `pages/{domain}/sections/`.
2. **Read the page source** and any tab, section, template, or partial it composes that is material
   to describing what the screen does.
3. **Extract signals**:
    - Top-level section headings, `Card` titles, or H2/H3 text → Core Region names
    - Mutation hooks, form handlers, server actions, and endpoint calls → Key Controls
    - Dialog, modal, or confirmation components opened from the page → Key Controls
    - Queries, loaders, server handlers, and content sources → Purpose
    - Route-guard / role checks visible in the component → role-related notes
4. **Match feature drivers**: list all directories under `.aidd/features/` and, for each screen, identify feature slugs whose name semantically matches the screen's functionality. Read the candidate `feature.json`'s `title` and `description` fields to confirm. Preferred match sources, in order:
...
1. **New routes**: in code, missing from Route Map. For each: proposed Screen name, path, Min Role
   when applicable, and **inferred Primary Purpose** (one short sentence from Phase 4).
...
- **Route Map table**:
    ...
    - Append new-route rows in the order they appear in the resolved route source, with the Phase 4
...
`.aidd/features/{slug}/feature.json` using this schema (the standard aidd feature record).
...
```json
{
	"affectedFiles": ["<known project-root-relative page source>"],
	"category": "<existing project category, usually UI for a screen>",
	"completedAt": "<ISO-8601 timestamp of this run>",
	...
	"notes": [
		"Backfilled by update-screen-map from current implementation evidence on <YYYY-MM-DD>."
	],
	"passes": true,
	"priority": 3,
...
- **`id`**: use a clean descriptive slug (e.g. `bug-inbox`, `run-console-page`): no app prefix,
  no timestamp, no `feature-` prefix. This matches the house base-feature naming convention in
  `skills/consolidate-features/SKILL.md`. The directory name must match the `id`. Read all existing
  `id` fields in the app's `.aidd/features/**/feature.json` to avoid collisions.
- **`affectedFiles`**: JSON array of known project-root-relative file paths the screen comprises
  (page component, tabs/sections, dialogs, API modules). A completed backfill must include at least
  the page source found in Phase 4; an empty array produces a feature-validation warning.
- **`dependencies`**: must be a JSON array (even if empty). Each ref must resolve to another existing
  feature in the app. The live graph accepts either that feature's `id` or its directory name;
  prefer the `id` and confirm both values from the file and its containing directory. Common
  upstream dependencies for UI screens are the app-shell, API-client, and auth/router features
...
- **`passes`**: use `true` with `completed`; this is an implemented backfill, and the operational
  feature contract requires both fields to describe verified completion.
- **`category`**: choose from the vocabulary used by existing features in the same app (grep
  `"category":` to sample). Category is project vocabulary, not a fixed aidd enum.

If `.aidd/roadmap.json` exists, add every new feature directory to its `features` map and
assign it to a milestone already defined in `milestones`, following the target roadmap's placement
convention. Preserve milestone definitions and unrelated mappings. Write the roadmap entry's
`dependencies` using the dependencies' **directory names**; roadmap dependencies are directory-keyed
even when a feature record uses IDs. Set the new feature's `priority` to the selected milestone's
numeric priority. A feature left unmapped is a hard validation issue; end-of-run reconciliation
propagates mapped priority and dependency data but does not invent a missing assignment.
...
- Dependency refs that match neither a feature `id` nor a feature directory
- Invalid `status`, or a category outside the target project's established vocabulary
...
2. If any `.aidd/features/*/feature.json` or `.aidd/roadmap.json` was created or modified, confirm the
   Phase 7 validator output was clean. If no feature or roadmap files were touched, skip this step.
3. Report what changed as counts:
    ...
    - `S new feature.json blueprints created, T roadmap assignments added`
4. Remind the user:
...
5. If the app has `bun run smoke:qc`, note that it is not required; this skill only edits
   `.aidd/screen-map.md`, possibly `.aidd/features/*/feature.json`, and possibly
   `.aidd/roadmap.json`, none of which affects the code pipeline.
...
- **Reconstruction completeness.** The goal of `.aidd/features/` is to let an agent rebuild the app
  from blueprints. A route in the resolved route source with no matching feature.json is a gap in
  that blueprint. This skill closes the gap by proposing backfills, validated against the aidd
  feature checker.
