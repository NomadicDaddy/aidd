...
1. **Detect and run the repository's authoritative test entry.** Read the repository instructions,
   test configuration, and relevant project manifest. For a Bun project, inspect the `scripts`
   block of `package.json` and use the aggregate test script it defines, such as `bun run test` or
   `bun run test:all`. Do not infer that one of several narrowly named `test:*` scripts is the full
   suite, and do not substitute an end-to-end or crawler command unless the repository identifies
   it as the test authority.

    **If no standalone test suite exists** after checking the repository instructions, manifests,
    test configuration, and test files, run the documented aggregate gate that owns test coverage,
    such as `bun run smoke:qc`, or the documented crawler when that is the project's only test
    surface. Report standalone unit tests as not applicable. Spernakit-derived apps deliberately
    have no unit-test framework; their repository-defined quality gate is authoritative.
...
3. **If any tests fail**, analyze the failures:
    ...
    - Review both the diff and commits relative to the resolved branch, substituting the branch
      reported above:
        ...
        Inspect relevant untracked files separately because `git diff` does not include them.
...
6. **After making fixes**, re-run the same authoritative test entry. Do not substitute a narrower
   test for the command that failed.

7. **Report summary** with the files changed, what was fixed (tests updated versus code fixed), the
   exact test command, and its actual result. Include any skipped checks, coverage-depth gaps, or
   unresolved blockers.
