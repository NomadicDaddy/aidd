...
describe('audit service', () => {
		...
		try {
			...
			try {
				...
				await Bun.write(
					...
					[
						JSON.stringify({
							auditFindings: { SECURITY: 2 },
							driverId: 'SECURITY',
							driverKind: 'audit',
							stopReason: 'completed',
						}),
						JSON.stringify({
							auditFindings: { SECURITY: 1 },
							driverId: 'SECURITY',
							driverKind: 'audit',
							stopReason: 'completed',
						}),
						// A ledger entry that predates the per-audit breakdown is unattributed, never credited.
						JSON.stringify({
							driverId: 'SECURITY',
							driverKind: 'audit',
							stopReason: 'completed',
				...
				expect(security.changePotential.evidence.completedRunsWithFindings).toBe(2);
	...
	test('project audit response retains report state across project override changes', async () => {
		const rootDir = await testTempDir('aidd-audit-project-override-report-');
		try {
			await writeAuditDefinition(rootDir, 'SECURITY', 'Critical');
			const projectDir = join(rootDir, 'project');
			await mkdir(join(projectDir, '.aidd'), { recursive: true });
			...
			const beforeReport = await service.listProjectAudits(projectDir);
			const missingSecurity = beforeReport.entries.find((entry) => entry.name === 'SECURITY');
			expect(missingSecurity?.enabled).toBe(true);
			expect(missingSecurity?.missingReport).toBe(true);

			const store = new FileAiddStore(projectDir);
			const reportPath = await store.writeAuditReport('SECURITY', '# SECURITY done');
			await service.saveProjectAuditOverrides(projectDir, {
				audits: { SECURITY: 'disabled' },
				rules: [],
				version: 1,
			});

			const whileDisabled = await service.listProjectAudits(projectDir);
			const disabledSecurity = whileDisabled.entries.find(
				(entry) => entry.name === 'SECURITY',
			);
			expect(disabledSecurity?.enabled).toBe(false);
			expect(disabledSecurity?.overrideEffect).toBe('disabled');
			expect(disabledSecurity?.freshReport).toBe(true);
			expect(disabledSecurity?.missingReport).toBe(false);
			expect(disabledSecurity?.reportFreshness?.report).toBe(basename(reportPath));
			...
			const afterReenable = await service.listProjectAudits(projectDir);
			const enabledSecurity = afterReenable.entries.find(
				(entry) => entry.name === 'SECURITY',
			);
			expect(enabledSecurity?.enabled).toBe(true);
			expect(enabledSecurity?.overrideEffect).toBe('required');
			expect(enabledSecurity?.freshReport).toBe(true);
			expect(enabledSecurity?.reportFreshness?.report).toBe(
				disabledSecurity?.reportFreshness?.report,
			);
		} finally {
			await removeTempTree(rootDir);
		}
	...
	test('stamps the initiator it was handed, whichever door the audit came in by', async () => {
		// Not derived from source: reading 'scheduled' as automatic and anything else as operator
		// cannot see a Run now, which arrives with source 'scheduled' and a person standing in front
		// of it, and would file that person's work as aidd's. The surface and the decider are
		// separate questions, and both are carried.
		async function initiatorFor(
			initiator: 'automatic' | 'operator',
			source: 'scheduled' | undefined,
		): Promise<unknown> {
			const options: unknown[] = [];
			await launchAuditsImpl(
				{
					auditNames: ['SECURITY'],
					initiator,
					projectIds: ['p1'],
				...
				{
					auditsEnabled: true,
					launchRun: (async (_request: unknown, launchOptions: unknown) => {
			...
			return (options[0] as { initiator: unknown }).initiator;
		}

		// The Run now case, and the whole point: a scheduled surface with an operator behind it.
		expect(await initiatorFor('operator', 'scheduled')).toBe('operator');
		expect(await initiatorFor('automatic', 'scheduled')).toBe('automatic');
		expect(await initiatorFor('operator', undefined)).toBe('operator');
	});
	...
	test('auditAll launches all audits without requiring auditNames', async () => {
		const requests: Record<string, unknown>[] = [];
		const result = await launchAuditsImpl(
			{ auditAll: true, initiator: 'operator', projectIds: ['only'] },
			{
				auditsEnabled: true,
				launchRun: (async (request: Record<string, unknown>) => {
					requests.push(request);
	...
	test('audit-all review stays a directive run and never becomes a fresh --audit-all run', async () => {
		const requests: Record<string, unknown>[] = [];
		const result = await launchAuditsImpl(
			{ auditAll: true, initiator: 'operator', projectIds: ['p1'], review: true },
			{
				auditsEnabled: true,
				launchRun: (async (request: Record<string, unknown>) => {
					requests.push(request);
	...
	test('rejects a launch with neither auditAll nor audit names selected', async () => {
		await expect(
			launchAuditsImpl(
				{ auditNames: [], initiator: 'operator', projectIds: ['p1'] },
				{
					auditsEnabled: true,
