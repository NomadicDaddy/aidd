...
// The two fields `isPeerAuthorized` actually reads. `allowRemote` is spelled out at every call
// because it is now material: it decides what a panel with no token does with a request it cannot
// place on loopback.
const local = { allowRemote: false } as const;
const localWithToken = { allowRemote: false, authToken: TOKEN } as const;
const remote = { allowRemote: true } as const;
const remoteWithToken = { allowRemote: true, authToken: TOKEN } as const;

describe('isPeerAuthorized (direct, non-forwarded requests)', () => {
	test('keeps the zero-config local exemption when no token is configured', () => {
		// The default panel is loopback-bound and tokenless, and it has to answer callers whose
		// peer address is not knowable: `server.requestIP` returns nothing when the app is driven
		// through `app.handle()` with no listening socket. Nothing it serves leaves the machine.
		expect(isPeerAuthorized(local, '100.64.1.2', null, false)).toBe(true);
		expect(isPeerAuthorized(local, null, null, false)).toBe(true);
		expect(isPeerAuthorized(local, '127.0.0.1', null, false)).toBe(true);
	});

	test('holds a tokenless remote-bound panel to loopback instead of allowing everything', () => {
		// This is the case that must never fail open: the panel is on the network and has no
		// token to check, so a non-loopback (or unplaceable) peer gets nothing.
		// `assertWebAuthTokenPresent` stops such a panel from binding at all; this is the second
		// line, for a config that reaches the guard some other way.
		expect(isPeerAuthorized(remote, '100.64.1.2', null, false)).toBe(false);
		expect(isPeerAuthorized(remote, null, null, false)).toBe(false);
		expect(isPeerAuthorized(remote, '127.0.0.1', null, false)).toBe(true);
		expect(isPeerAuthorized(remote, '::1', null, false)).toBe(true);
	});

	test('exempts loopback peers without a token', () => {
		expect(isPeerAuthorized(remoteWithToken, '127.0.0.1', null, false)).toBe(true);
		expect(isPeerAuthorized(remoteWithToken, '::1', null, false)).toBe(true);
		expect(isPeerAuthorized(localWithToken, '127.0.0.1', null, false)).toBe(true);
	});

	test('requires a matching token for remote peers', () => {
		expect(isPeerAuthorized(remoteWithToken, '100.64.1.2', null, false)).toBe(false);
		expect(isPeerAuthorized(remoteWithToken, '100.64.1.2', 'wrong', false)).toBe(false);
		expect(isPeerAuthorized(remoteWithToken, '100.64.1.2', TOKEN, false)).toBe(true);
	});

	test('denies remote peers when the peer address is unknown', () => {
		expect(isPeerAuthorized(remoteWithToken, null, null, false)).toBe(false);
		expect(isPeerAuthorized(remoteWithToken, null, TOKEN, false)).toBe(true);
	});
});
...
describe('isPeerAuthorized (forwarded / reverse-proxied requests)', () => {
	test('voids the loopback exemption: a proxied loopback peer must present the token', () => {
		...
		expect(isPeerAuthorized(remoteWithToken, '127.0.0.1', null, true)).toBe(false);
		expect(isPeerAuthorized(remoteWithToken, '::1', null, true)).toBe(false);
		expect(isPeerAuthorized(remoteWithToken, '127.0.0.1', 'wrong', true)).toBe(false);
		expect(isPeerAuthorized(remoteWithToken, '127.0.0.1', TOKEN, true)).toBe(true);
	});

	test('denies a proxied request outright when no token is configured', () => {
		// An unauthenticated control plane cannot be safely exposed through a proxy, whichever
		// interface it is bound to.
		expect(isPeerAuthorized(remote, '127.0.0.1', null, true)).toBe(false);
		expect(isPeerAuthorized(remote, '127.0.0.1', TOKEN, true)).toBe(false);
		expect(isPeerAuthorized(local, '127.0.0.1', null, true)).toBe(false);
		expect(isPeerAuthorized(local, '127.0.0.1', TOKEN, true)).toBe(false);
	});
});
...
describe('createBearerTokenGuardPlugin (always mounted, no token configured)', () => {
	test('allows a direct request with no forwarding headers', async () => {
		...
		const app = guardedApp(webConfig({ allowRemote: false }));
		const response = await app.handle(new Request('http://localhost/api/v1/ping'));
		expect(response.status).toBe(200);
	});

	test('denies the same request once the panel is bound remotely', async () => {
		// Same request, same missing token, but this panel is on the network and the peer
		// cannot be placed on loopback. Allowing it would serve the network unauthenticated.
		const app = guardedApp(webConfig({ allowRemote: true }));
		const response = await app.handle(new Request('http://localhost/api/v1/ping'));
		expect(response.status).toBe(401);
	});
