...
describe('DB worker round-trip', () => {
	...
	test('terminalizeRun preserves launcher provenance when the heartbeat record carries none', async () => {
		await handle.db.insert(runs).values({
			aiddDirty: false,
		...
		expect(row?.driverId).toBe('demo-skill');
		expect(row?.driverKind).toBe('skill');
		expect(row?.driverSha256).toBe('d'.repeat(64));
	});
