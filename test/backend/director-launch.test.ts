...
describe('DirectorService.runCycle context', () => {
	...
	test('omits context for empty cycle requests and adds profile/chat context when supplied', async () => {
		...
		try {
			...
			expect(run1?.model).toBe('glm-5.3');
			...
			expect(messages.find((message) => message.role === 'system')?.content).toMatch(
				/^Director cycle `cycle_[^`]+` completed/u,
			);
		} finally {
			sqlite.close();
			await removeTempTree(workspace);
...
describe('director API routes', () => {
	test('accepts empty cycle bodies and exposes profile/chat endpoints', async () => {
		...
		const invalidProfile = await app.handle(
			new Request('http://localhost/api/v1/director/profile', {
				body: JSON.stringify({ backend: 'unknown-cli' }),
				headers: { 'content-type': 'application/json' },
				method: 'PUT',
			}),
		);
...
describe('DirectorService.launchSuggestion', () => {
	...
	test('launches project-scoped suggestions and records the run id', async () => {
		...
		try {
			...
			const outcome = await service.launchSuggestion('sug_proj');

			expect(outcome.claimedElsewhere).toBe(false);
			...
			expect(launchedRun?.projectPath).toBe(canonicalProjectPath(projectDir));
			// Launching a suggestion is somebody clicking Launch on it. The run has to say so even
			// though the suggestion itself was written by an automatic Director cycle: who wrote
			// the suggestion and who acted on it are different questions.
			expect(launchedRun?.initiator).toBe('operator');
	...
	// Item 5 of the auto-launch spec sets two obligations for this race, and they pull apart: the
	// atomic claim satisfies "exactly one run" by throwing at the loser, which is precisely how the
	// second obligation — "the loser MUST NOT surface an error to the operator" — gets broken. The
	// service throw is still the mechanism, and `director-suggestion-auto-launch.test.ts` asserts the
	// auto-launcher records it as an ordinary skip; what is asserted here is that no person is ever
	// shown it. Both operator launches resolve, and one of them says it started nothing new.
	test('fires two concurrent launches for one pending suggestion and spawns exactly one run', async () => {
		...
		try {
			...
			// Nobody is told off for clicking a button that was valid when they clicked it.
			expect(outcomes.filter((outcome) => outcome.status === 'rejected')).toEqual([]);
			expect(fulfilled).toHaveLength(2);
			expect(fulfilled.filter((outcome) => outcome.value.claimedElsewhere)).toHaveLength(1);

			const winner = fulfilled.find((outcome) => !outcome.value.claimedElsewhere)!.value;
			const loser = fulfilled.find((outcome) => outcome.value.claimedElsewhere)!.value;
			const winningLaunch = winner.launch;
			expect(winningLaunch?.kind).toBe('run');
			if (winningLaunch?.kind !== 'run') throw new Error('Expected a run launch.');
			// The loser is never pointed at a different run. Whether it can name the winner's run at
			// all depends on where in the winner's own launch the claim was lost: the claim is taken
			// before the run exists, so a loser arriving in that window correctly reports that
			// something is running without yet knowing what. `claimedElsewhere` is the part the UI
			// acts on, and it is true either way.
			if (loser.launch !== null) expect(loser.launch).toEqual(winningLaunch);
			const row = (
