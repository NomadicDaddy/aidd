...
describe('GET /api/v1/launch-defaults', () => {
	...
	test('resolves a requested backend through the shared override path', async () => {
		const config: ResolvedConfig = {
			...baseConfig,
			backends: { native: { model: 'native-model', reasoningEffort: 'high' } },
		};
		const { body, status } = await getDefaults(
			makeApp(config),
			'?backend=native&mode=interview',
		);
		expect(status).toBe(200);
		expect(body.effective).toMatchObject({
			backend: 'native',
