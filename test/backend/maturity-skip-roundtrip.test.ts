...
describe('maturity skip round-trip', () => {
	...
	test('a file named maturity-skip.json is NOT read', async () => {
		// Guards the reverse mistake: a loader that reads this name would make this pass silently.
		const projectDir = await makeProject('other-name');
		await writeFile(
