...
describe('project implementation readiness', () => {
	...
	test('treats template-owned records as the scaffolded shell, not product work', () => {
		const state = evaluateProjectImplementationState(
			'coding',
			[
				{
					directory: 'shell-feature',
					id: 'shell-feature',
					passes: true,
					priority: 1,
			...
			{
				...roadmap,
				features: { ...roadmap.features, 'shell-feature': { milestone: 'MVP' } },
			},
		);
