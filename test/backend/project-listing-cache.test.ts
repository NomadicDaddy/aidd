...
describe('ProjectListingCache stale-while-revalidate', () => {
	test('does not restore an invalidated entry from an older in-flight compute', async () => {
		const projectDir = await testTempDir('aidd-project-listing-cache-');
		const cache = new ProjectListingCache();
		const firstGate = createDeferred();
		const firstStarted = createDeferred();
		try {
			const firstRequest = cache.getOrCompute(projectDir, async () => {
			...
			expect(tagOf(await cache.getOrCompute(projectDir, async () => taggedValue(2)))).toBe(
				'proj-2',
			);
			firstGate.resolve();
			expect(tagOf(await firstRequest)).toBe('proj-1');

			let recomputes = 0;
			const current = await cache.getOrCompute(projectDir, async () => {
				recomputes += 1;
				return taggedValue(3);
			});
			expect(tagOf(current)).toBe('proj-2');
