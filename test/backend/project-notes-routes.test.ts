...
describe('project notes routes', () => {
	...
	test('PUT accepts the notes limit and rejects content one character over it', async () => {
		const dir = await makeProjectDir();
		const app = createApp(dir);
		const atLimit = 'x'.repeat(projectNotesMaxLength);
		...
		expect(((await accepted.json()) as { content: string }).content).toBe(atLimit);

		const rejected = await app.handle(
			new Request(`${BASE}/projects/anything/notes`, {
				body: JSON.stringify({ content: `${atLimit}x` }),
				headers: { 'content-type': 'application/json' },
				method: 'PUT',
			}),
		);
		expect(rejected.status).toBe(422);
		expect((await readProjectNotes(dir)).content).toBe(atLimit);
	});
});
