...
describe('bundled recipe consistency', () => {
	...
	// `coding-spirit-document-changes` was retired once it drifted into a byte-identical copy of the
	// system `coding` recipe: two catalog entries that launch the same pipeline, one of which the
	// director never suggests. Distinct names and descriptions hid it, so key off behavior instead.
	test('no two recipes launch an identical pipeline', async () => {
		const recipes = await readRecipes();
		const byBehavior = new Map<string, string>();
		for (const [id, recipe] of recipes) {
			const behavior = JSON.stringify({
				parameters: recipe.parameters,
				steps: recipe.steps,
			});
			const twin = byBehavior.get(behavior);
			expect(
				twin,
				`${id} has the same parameters and steps as ${twin} — retire one or make them differ`,
			).toBeUndefined();
			byBehavior.set(behavior, id);
		}
	});
