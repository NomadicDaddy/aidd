...
function fakeLaunch(
	...
	initiators: RunInitiator[] = [],
) {
	return async (input: RunLaunchRequest, initiator: RunInitiator) => {
		calls.push(input);
	...
	test('launches a follow-up under the recorded launch target and records telemetry', async () => {
		...
		try {
			...
			const launched = await continueRun(
				{
					db,
					launch: fakeLaunch(db, calls, initiators),
			...
			// The Continue button is a person; the run it launches says so, even though the parent it
			// copies its launch target from may itself have been automatic.
			expect(initiators).toEqual(['operator']);
...
describe('maybeAutoChainRun', () => {
	function autoDeps(
		...
		return {
			...
			db,
			launch: fakeLaunch(db, calls, overrides.initiators ?? []),
			recordStart: async () => {},
		};
	}

	test('launches a linked follow-up when opted in, without an account or key', async () => {
		const { db, sqlite } = makeDb();
		const calls: RunLaunchRequest[] = [];
		try {
	...
	test('records the auto-chained follow-up as automatic while its parent stays operator', async () => {
		// The whole point of the split: a chain that runs overnight must not read back as a person
		// clicking Continue five times. The parent keeps whatever launched it; every link aidd adds
		// on its own is 'automatic', and the two are recorded, never derived from the chain shape.
		const { db, sqlite } = makeDb();
		const calls: RunLaunchRequest[] = [];
		const initiators: RunInitiator[] = [];
		try {
			await seedRun(db, { id: 'run_parent_auto', initiator: 'operator', status: 'failed' });
			await maybeAutoChainRun(
				autoDeps(db, calls, { initiators }),
				'run_parent_auto',
				'wall_clock_timeout',
			);
			expect(initiators).toEqual(['automatic']);
			expect(
				(await db.select().from(runs).where(eq(runs.id, 'run_parent_auto')))[0]?.initiator,
			).toBe('operator');
		} finally {
			sqlite.close();
		}
