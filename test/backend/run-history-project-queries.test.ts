...
describe('project-scoped run history queries', () => {
	test('latestProjectAuditRun finds a project audit hidden beyond the old 50-row global window', async () => {
	...
	test('listRuns includes terminal direct director cycles for matching status filters', async () => {
		...
		expect(cycleRun).toMatchObject({
			...
			source: 'director',
			status: 'completed',
			summary: 'Director cycle completed with 2 suggestion(s).',
		});
	});
	...
	test('listRunsPage keeps direct director cycles when director has a run ledger', async () => {
		...
		try {
			...
			expect(cycleRun?.summary).toBe('Director cycle completed with 3 suggestion(s).');
		} finally {
