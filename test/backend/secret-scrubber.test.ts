...
describe('scrubSecrets', () => {
	...
	it('masks AWS access key ids (AKIA...)', () => {
		const awsId = `AKIA${'ABCDEFGHIJKLMNOP'}`;
		const out = scrubSecrets(`aws id ${awsId} done`);
		expect(out).toBe(`aws id ${SECRET_REDACTED} done`);
	});

	it('masks GitHub personal access token prefixes', () => {
		const classicToken = `gh${'p_ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'}`;
		expect(scrubSecrets(`token ${classicToken}`)).toBe(`token ${SECRET_REDACTED}`);
		const fineGrained = `github${'_pat_11ABCDEFG0abcdefghijklmnop'}`;
		expect(scrubSecrets(`${fineGrained} more`)).toBe(`${SECRET_REDACTED} more`);
	});
