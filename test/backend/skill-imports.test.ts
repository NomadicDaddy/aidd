...
describe('managed skill imports', () => {
	test('previews and copies a package unchanged with provenance and merged discovery', async () => {
		...
		try {
			const descriptionAtLimit = 'a'.repeat(1024);
			const sourcePath = await writeSkill(sourceRoot, 'demo', descriptionAtLimit);
			...
			const overLimit = await writeSkill(sourceRoot, 'over-limit', 'b'.repeat(1025));
			await expect(service.importSkill({ sourcePath: overLimit })).rejects.toMatchObject({
				message: 'Skill over-limit: description must be at most 1024 characters',
				status: 400,
			});
			expect((await listSkillDefinitions(appRoot, dataDir)).map((skill) => skill.id)).toEqual(
				['demo'],
