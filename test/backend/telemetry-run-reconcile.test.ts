...
interface RunSeed {
	id: string;
	driverId?: string;
	driverKind?: 'audit' | 'prompt' | 'recipe-step' | 'skill';
	driverSha256?: string;
...
function seedRun(seed: RunSeed): Promise<unknown> {
	return db.insert(runs).values({
		...
		driverKind: seed.driverKind ?? null,
		driverSha256: seed.driverSha256 ?? null,
...
function seedInvocation(seed: InvocationSeed): Promise<unknown> {
	return db.insert(invocationEvents).values({
		...
		resourceName: seed.resourceName ?? 'demo resource',
...
describe('reconcileInvocationFromRun', () => {
	...
	test('copies the driver hash onto the run row and refines a named audit batch', async () => {
		await seedRun({
			id: 'run_audit',
			status: 'completed',
			exitCode: 0,
			completedAt: 5_000,
			durationMs: 4_000,
			driverId: 'AI+SECURITY',
			driverKind: 'audit',
			driverSha256: 'd'.repeat(64),
		});
		await seedInvocation({ id: 'inv_audit', runId: 'run_audit', resourceName: 'SECURITY+AI' });
		...
		expect(await commands.reconcileInvocationFromRun({ runId: 'run_audit' })).toBe(2);

		const runRow = await getInvocation('inv_audit');
		expect(runRow?.resourceName).toBe('AI+SECURITY');
		expect(runRow?.resourceSha256).toBe('d'.repeat(64));
		// A nested skill row keeps its own resource identity; only the terminal facts are copied.
		const nested = await getInvocation('inv_nested');
		expect(nested?.status).toBe('completed');
		expect(nested?.resourceName).toBe('Review');
		expect(nested?.resourceSha256).toBeNull();
	});

	test('keeps an audit-all resource name recorded at launch', async () => {
		await seedRun({
			id: 'run_all',
			status: 'completed',
			exitCode: 0,
			completedAt: 5_000,
			driverId: 'AI+ARCHITECTURE+SECURITY',
			driverKind: 'audit',
			driverSha256: 'f'.repeat(64),
		});
		await seedInvocation({ id: 'inv_all', runId: 'run_all', resourceName: 'audit-all' });

		await commands.reconcileInvocationFromRun({ runId: 'run_all' });

		const row = await getInvocation('inv_all');
		expect(row?.resourceName).toBe('audit-all');
		expect(row?.resourceSha256).toBe('f'.repeat(64));
	});
...
describe('recordStart fast-run reconciliation', () => {
	test('a skill launch whose run already terminalized lands terminal, not stuck running', async () => {
		...
		expect(inv?.exitCode).toBe(0);
		expect(inv?.completedAt).toBe(5_000);
		expect(inv?.resourceSha256).toBe('e'.repeat(64));
	});
