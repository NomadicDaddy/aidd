...
async function readFeatureFile(
	dir: string,
	directory: string,
): Promise<{ notes?: string | string[]; priority?: number; shippedVersion?: string }> {
	...
	test('reports cross-milestone dependency violations', async () => {
		const dir = await seedProject(
			[
				...
				// Same broken ordering on a feature that already shipped: nothing is left for it to
				// strand, and the view has to carry that difference for the callout to grade it.
				{
					dependencies: ['beta'],
					directory: 'done',
					milestone: 'MVP',
...
describe('POST /:id/milestones/reassign', () => {
	...
	test('apply re-homes by shipped version, stamps backfills, and re-syncs priorities', async () => {
		...
		expect(backfilled.notes).toEqual([expect.stringContaining('backfilled')]);
	...
	test('preserves missing, string, and array notes when backfilling shipped versions', async () => {
		const dir = await seedProject(
			[
				{
				...
				{
					directory: 'string-notes',
					milestone: 'v1.0',
					notes: 'Original string note.',
					passes: true,
					status: 'completed',
				},
				{
					directory: 'array-notes',
					milestone: 'v1.0',
					notes: ['Original array note.'],
					passes: true,
					status: 'completed',
				},
			],
		...
		const response = await send(createApp(dir), 'POST', '/reassign', {});
		expect(response.status).toBe(200);
		expect((await readFeatureFile(dir, 'missing-notes')).notes).toEqual([
			expect.stringContaining('backfilled'),
		]);
		expect((await readFeatureFile(dir, 'string-notes')).notes).toEqual([
			'Original string note.',
			expect.stringContaining('backfilled'),
		]);
		expect((await readFeatureFile(dir, 'array-notes')).notes).toEqual([
			'Original array note.',
			expect.stringContaining('backfilled'),
		]);
	});
