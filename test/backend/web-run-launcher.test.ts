...
// The fake CLI drops one file per spawned child, keyed by run id, so a test can count the
// children a launch actually started instead of inferring it from process liveness. On Windows
// the relauncher shim exits before this block runs, so only the real run leaves a witness.
function spawnWitness(dir: string): string {
...
async function spawnWitnessCount(dir: string): Promise<number> {
	try {
...
async function waitForWitnessCount(dir: string, expected: number): Promise<void> {
	const startedAt = Date.now();
	for (;;) {
...
// spawnDetachedRun writes the Windows hand-off payload before it spawns anything, so a payload
// that never appeared is proof the detached hop was never entered. Vacuous on POSIX, which
// spawns the run directly.
async function detachedPayloadCount(dataDir: string): Promise<number> {
	try {
		const entries = await readdir(join(dataDir, 'run-logs'));
		return entries.filter((entry) => entry.endsWith('.launch.json')).length;
	} catch {
		return 0;
	}
}

async function runTranscriptCount(dataDir: string): Promise<number> {
	try {
		const entries = await readdir(join(dataDir, 'run-logs'));
...
describe('web run launcher', () => {
	...
	test('POSIX detached run spawn plan runs the CLI directly and appends stderr to the run log', () => {
		...
		expect(plan.options).toMatchObject({
			...
			env: {
				...
				AIDD_EXT_RUN_DRIVER_SHA256: 'c'.repeat(64),
			},
			stdin: 'ignore',
			stdout: 'ignore',
		});
	...
	test('a skill launched from a recipe step carries the step driver env beside the skill flag', () => {
		const plan = buildDetachedRunSpawnPlan({
			args: ['bun', 'cli/src/index.ts', '--project-dir', '/srv/aidd', '--skill', 'review'],
			driver: {
				driverId: 'step-review',
				driverKind: 'recipe-step',
				driverSha256: 'c'.repeat(64),
			...
			initiator: 'operator',
			launcherPrefix: ['bun', 'cli/src/index.ts'],
			logPath: '/srv/aidd/data/run-logs/run_skill.log',
			payloadPath: null,
			platform: 'linux',
			rootDir: '/srv/aidd',
			runId: 'run_skill',
		...
		// Both identities reach the CLI. cli/src/run-driver.ts resolves the precedence: the skill
		// becomes the run driver and the step identity stays in
		// pipeline_step_results.step_definition_id (see test/cli/run-driver.test.ts).
	...
	test('launchRun clears a stale stop file so a fresh run never reads as stopping', async () => {
		...
		try {
			...
			try {
				const run = await service.launchRun({ projectDir }, { initiator: 'operator' });
	...
	test('launchRun persists the initiator the caller passed and reads it back', async () => {
		const workspace = await testTempDir('aidd-web-initiator-');
		...
		try {
			...
			const { service, sqlite } = makeService({
				...
				dataDir: join(workspace, 'data'),
				rootDir,
				// Two live coding runs on one project need a checkout each: a non-isolated
				// mutating launch is clamped to one per project (see launchMutation.ts).
				web: { useWorktrees: true },
			});
			try {
				// Every launch site names its own initiator; the launcher stores what it was told
				// rather than deriving anything from `source`, which is why the same 'web' door can
				// produce both values.
				const byHand = await service.launchRun({ projectDir }, { initiator: 'operator' });
				const byAidd = await service.launchRun({ projectDir }, { initiator: 'automatic' });

				expect((await service.getRun(byHand.id))?.initiator).toBe('operator');
				expect((await service.getRun(byAidd.id))?.initiator).toBe('automatic');
				const listed = new Map(
					(await service.listRunsForProject(projectDir)).map((row) => [
						row.id,
						row.initiator,
					]),
				);
				expect(listed.get(byHand.id)).toBe('operator');
				expect(listed.get(byAidd.id)).toBe('automatic');
				await waitFor(
					() => service.getRun(byHand.id),
					(value) => value.status === 'completed',
				);
				await waitFor(
					() => service.getRun(byAidd.id),
					(value) => value.status === 'completed',
				);
			} finally {
				service.markDisposed();
	...
	test('a pending stop is scoped to the selected live run', async () => {
		...
		try {
			...
			try {
				...
				// pending: stopRun writes a run-specific stop file and leaves the row running.
				const serviceInternals = service as unknown as { db: WebDatabase };
				await serviceInternals.db.insert(schema.runs).values({
					backend: 'native',
					id: 'run_pending_stop',
				...
				expect(
					await Bun.file(runStopFilePath(projectDir, 'run_pending_stop')).exists(),
				).toBe(true);
				expect(await Bun.file(join(projectDir, '.aidd', '.stop')).exists()).toBe(false);
				expect(await runTranscriptCount(dataDir)).toBe(0);
			} finally {
				service.markDisposed();
				sqlite.close();
			}
	...
	test('readOutput returns the CLI active run transcript when the record has a logPath', async () => {
		...
		try {
			...
			try {
				...
				await writeCliActiveRunRecord(makeCliActiveRun(projectDir, 'cli_without_log'));
				expect(await service.readOutput('cli_without_log')).toMatchObject({
					output: '',
					reason: 'No log path was recorded for this run.',
					state: 'cli-only',
				});

				const missingLogPath = join(logDir, 'missing.log');
				await writeCliActiveRunRecord({
					...makeCliActiveRun(projectDir, 'cli_missing_log'),
					logPath: missingLogPath,
				});
				expect(await service.readOutput('cli_missing_log')).toMatchObject({
					output: '',
					reason: `Log file is missing on disk: ${missingLogPath}`,
					state: 'unavailable',
				});
			} finally {
				service.markDisposed();
	...
	test('readOutput pages every line of an oversized transcript through bounded windows', async () => {
		...
		try {
			...
			const bulkLine = `${'界'.repeat(42)}\n`;
			const bulk = bulkLine.repeat(
				Math.ceil((3 * 1024 * 1024) / Buffer.byteLength(bulkLine)),
			);
			const transcript = `HEAD_MARKER_FIRST_LINE\n${bulk}TAIL_MARKER_LAST_LINE\n`;
			await Bun.write(logPath, transcript);
			await writeCliActiveRunRecord({
			...
			try {
				...
				expect(result.output.startsWith('界')).toBe(true);
				expect(result.output).not.toContain('\uFFFD');
				expect(result.endByte).toBe(result.totalBytes);
				expect(result.startByte).toBeGreaterThan(0);
				expect(result.windowLimitBytes).toBe(2 * 1024 * 1024);

				const head = await service.readOutput('cli_big_log', {
					endByte: result.windowLimitBytes,
					startByte: 0,
				});
				expect(head.output.startsWith('HEAD_MARKER_FIRST_LINE\n')).toBe(true);
				expect(head.output).not.toContain('TAIL_MARKER_LAST_LINE');
				expect(head.startByte).toBe(0);
				expect(head.endByte).toBeLessThanOrEqual(result.windowLimitBytes);
				expect(Buffer.byteLength(head.output)).toBe(head.endByte - head.startByte);
				expect(head.output.endsWith('\n')).toBe(true);

				const next = await service.readOutput('cli_big_log', {
					endByte: result.totalBytes,
					startByte: head.endByte,
				});
				expect(next.output).toContain('TAIL_MARKER_LAST_LINE');
				expect(next.startByte).toBe(head.endByte);
				expect(head.output + next.output).toBe(transcript);

				await expect(
					service.readOutput('cli_big_log', { endByte: 10, startByte: 20 }),
				).rejects.toThrow(/ascending range/);
				await expect(
					service.readOutput('cli_big_log', {
						endByte: result.totalBytes + 1,
						startByte: result.totalBytes,
					}),
				).rejects.toThrow(/exceeds.*transcript/);
				await expect(
					service.readOutput('cli_big_log', {
						endByte: 2 * 1024 * 1024 + 1,
						startByte: 0,
					}),
	...
	test('run output route rejects malformed and incomplete byte ranges', async () => {
		...
		const malformed = await app.handle(
			new Request('http://localhost/api/v1/runs/run_1/output?startByte=nope&endByte=10'),
		);
		expect(malformed.status).toBe(400);
		const incomplete = await app.handle(
			new Request('http://localhost/api/v1/runs/run_1/output?startByte=0'),
		);
		expect(incomplete.status).toBe(400);
		expect(await incomplete.json()).toEqual({
			error: 'Run output requests must include both startByte and endByte.',
		});
	});
	...
	test('omits triumvirate role flags on non-triumvirate launches even when settings define them', async () => {
		...
		try {
			...
			const command = await buildLaunchCommand(
				rootDir,
				{ backend: 'native', mode: 'coding', model: 'glm-5.3', projectDir },
				{
					backend: 'native',
					model: 'glm-5.3',
					triumvirate: {
	...
	test('directive route maps intent, trims prompts, and records direct-run telemetry', async () => {
		...
		const post = (
			...
			prompt: string,
			launchTarget: { backend?: string; model?: string; reasoningEffort?: string } = {},
		) =>
			app.handle(
	...
	test('directive route rejects invalid bodies before launching', async () => {
		...
		const unsafeModel = await post({
			executionIntent: 'review-only',
			model: 'gpt-safe & calc.exe',
			projectDir: 'd:/applications/demo',
			prompt: 'Inspect',
		});
...
${heartbeatTerminator()}`,
	...
	test('web audit launches resolve the mode-scoped auditModel like the CLI', async () => {
		...
		try {
			...
			try {
				const auditRun = await service.launchRun(
					{ mode: 'audit', projectDir },
					{ initiator: 'operator' },
				);
				...
				const codingRun = await service.launchRun(
					{ projectDir },
					{ initiator: 'operator' },
				);
	...
	test('stop writes a run-specific stop file and clears it after exit', async () => {
			...
			`
{
	...
	while (Date.now() < __deadline) {
		try {
			await access(join(__projectDir, '.aidd', '.stop-' + process.env.AIDD_EXT_RUN_ID));
			break;
		} catch {
			await Bun.sleep(25);
		}
...
${heartbeatTerminator({ state: 'stopped', exitCode: 130 })}`,
		...
		try {
			...
			try {
				...
				// The row commits terminal before terminalize finishes its cleanup, so poll for the
				// file to go rather than reading it once on the heels of the status change.
				await waitFor(
					async () => await Bun.file(runStopFilePath(projectDir, run.id)).exists(),
					(exists) => !exists,
				);
			} finally {
				service.markDisposed();
	...
	test('a failed ceiling reservation starts no process and leaves no row behind', async () => {
		...
		try {
			...
			try {
				...
				// hop is one-way — on Windows the pwsh bridge has usually fired Start-Process and
				// exited before a kill could reach it — so a spawn-then-check launch that lost the
				// race left a live run behind with no row at all.
				expect(capturedPid).toBeNull();
				await wait(750);
				expect(await spawnWitnessCount(witnessDir)).toBe(0);
				expect(await detachedPayloadCount(dataDir)).toBe(0);
				expect(await runTranscriptCount(dataDir)).toBe(0);
				expect(await service.listRuns()).toHaveLength(0);
				expect(
					messages.some(
	...
	test('a transcript creation failure releases the admitted run reservation', async () => {
		const workspace = await testTempDir('aidd-web-transcript-failure-');
		const dataDir = join(workspace, 'not-a-directory');
		const rootDir = await makeLauncherRoot('console.log("ok");\n');
		try {
			await Bun.write(dataDir, 'file blocks the run-logs directory');
			const projectDir = await makeProject(workspace);
			const { service, sqlite } = makeService({ allowedRoot: workspace, dataDir, rootDir });
			try {
				await expect(
	...
	test('a detached spawn failure releases its row and removes its transcript', async () => {
		const workspace = await testTempDir('aidd-web-spawn-failure-');
		const dataDir = join(workspace, 'data');
		const rootDir = await makeLauncherRoot('console.log("ok");\n');
		try {
			const projectDir = await makeProject(workspace);
			...
			try {
				...
				try {
					process.env.PATH = join(workspace, 'missing-bin');
					await expect(
						service.launchRun({ projectDir }, { initiator: 'operator' }),
					).rejects.toThrow();
				} finally {
	...
	test('worktree-isolated coding launches fill the configured per-project ceiling', async () => {
		const workspace = await testTempDir('aidd-web-worktree-ceiling-');
		const witnessDir = join(workspace, 'spawn-witness');
		const rootDir = await makeLauncherRoot(
			`${spawnWitness(witnessDir)}\nawait Bun.sleep(1500);\n`,
		);
		try {
			...
			try {
				...
				expect(first.worktreePath).not.toBeNull();
	...
	test('recipe service stores pipeline JSON recipes and rejects path-like ids', async () => {
		...
		try {
			...
			await Bun.write(
				join(workspace, 'recipes', 'unknown_step_type.json'),
				JSON.stringify({
					name: 'Unknown Step Type',
					steps: [
						{
							configJson: { prompt: 'unknown-type prompt' },
							name: 'Unknown',
			...
			await expect(recipeService.readRecipe('unknown_step_type')).rejects.toThrow(
				'Recipe has invalid steps: unknown_step_type',
			);
			expect(
	...
	test('ingestCompletedCliRuns terminalizes a stranded running row from a completed heartbeat', async () => {
		...
		try {
			...
			try {
				...
				expect(row?.aiddVersion).toBe('3.0.0');
