...
import { providerCredentials } from 'aidd-shared/agent/client/providerKeys';
import { TELEGRAM_BOT_TOKEN_ENV, WEB_AUTH_TOKEN_ENV } from 'aidd-shared/config';
...
import { mergeProviders } from '../../backend/src/services/settings/mergeProviders.ts';
import type { StatusCommandRunner } from '../../backend/src/services/settings/status.ts';

import { testTempDir } from '../_helpers/temp.ts';
import { removeTempTree } from './_helpers/remove-temp-tree.ts';

// Settings resolution consults the ambient environment for operator credentials, which is the
// whole point of AIDD_WEB_AUTH_TOKEN, AIDD_TELEGRAM_BOT_TOKEN and the provider key variables. A
// real one exported in the developer's shell beats the fixture value these cases write, so the
// Telegram bridge would receive the operator's own bot token instead of the fixture's, and a
// provider whose key was deliberately left out of the fixture would still report itself
// configured. Scrub the whole operator credential set for this file so these assertions mean the
// same thing on a developer machine as they do in CI, and build the list from the same registries
// production reads so a newly added provider is covered without anyone remembering this file. The
// cases that are about the overlay itself set their own values through `withEnv` further down.
const operatorCredentialEnv = [
	WEB_AUTH_TOKEN_ENV,
	TELEGRAM_BOT_TOKEN_ENV,
	'NATIVE_API_KEY',
	...Object.values(providerCredentials({}))
...
beforeAll(() => {
	for (const key of operatorCredentialEnv) {
...
test('provider merge keeps unmanaged values while normalizing direct edits', () => {
	expect(
		mergeProviders(
			{
				openai: {
...
describe('settings DTO templates', () => {
	test('surfaces registered templates as slim summaries without leaking initCommand', async () => {
		const dto = await buildSettingsDto(
			{
				web: {
					templates: [
						{
...
describe('web settings config', () => {
	...
	test('round-trips run budgets and preserves unmanaged per-backend keys', async () => {
		...
		// settings UI does not manage. Rebuilding the entry from the managed fields alone would
	...
	test('rejects a Direct AI base URL pointing at a cloud metadata endpoint (SSRF guard)', async () => {
		...
		await expect(
			service.updateConfig({
				...baseUpdate,
				providers: { zhipu: { baseUrl: 'file:///etc/passwd', model: 'glm-5.3' } },
			}),
		).rejects.toThrow(/must use http or https/);
		await expect(
			service.updateConfig({
				...baseUpdate,
				directAi: { baseUrl: 'not a url', enabled: false },
			}),
		).rejects.toThrow(/Direct AI base URL is not a valid URL/);
		await expect(
			service.updateConfig({
				...baseUpdate,
				providers: { zhipu: { baseUrl: 'https://', model: 'glm-5.3' } },
			}),
		).rejects.toThrow(/Provider "zhipu" base URL is not a valid URL/);
		const https = await service.updateConfig({
			...baseUpdate,
			providers: { zhipu: { baseUrl: 'https://api.example.com/v1', model: 'glm-5.3' } },
		});
		expect(https.config.providers.zhipu?.baseUrl).toBe('https://api.example.com/v1');
	...
	test('persists director suggestion settings through the route body schema', async () => {
		...
		const written = JSON.parse(await readFile(configPath, 'utf8')) as {
			director?: {
				suggestions?: {
					autoLaunch?: Record<string, boolean | number | string | string[]>;
					granularity?: string;
					maxPerBucket?: number;
				};
			};
		};
		// The auto-launch bounds ride along even though this body never mentioned them: a partial
		// settings post must not be able to switch auto-launch on, or widen a bound, by omission.
	...
	test('settings status routes cache probes until a refresh is requested', async () => {
		...
		// A plain reload — the case that must not spawn the whole subprocess fleet again.
	...
	test('drops the deprecated director schedule block without clobbering chat file-edits', async () => {
		const workspace = await testTempDir('aidd-web-settings-autocycle-');
		const configPath = join(workspace, 'config.json');
		// A config that still carries the cadence keys; the settings form has no field that writes
		// them.
		await Bun.write(
			configPath,
			JSON.stringify({
				director: {
					chat: { allowFileEdits: true },
		...
		expect(result.config.directorSuggestionMaxPerBucket).toBe(5);

		// The cadence lives on the built-in scheduled task now, and it was seeded from these keys
		// at boot, so the first save after the upgrade is where they stop being written. Everything
		// else in the director block survives.
		const written = JSON.parse(await readFile(configPath, 'utf8')) as {
			director?: {
				chat?: { allowFileEdits?: boolean };
				schedule?: Record<string, unknown>;
				suggestions?: { maxPerBucket?: number };
			};
		};
		expect(written.director?.chat?.allowFileEdits).toBe(true);
		expect(written.director?.suggestions?.maxPerBucket).toBe(5);
		expect(written.director?.schedule).toBeUndefined();
	...
	test('round-trips applicationsRoot separately from applicationRoots', async () => {
		...
		await mkdir(join(workspace, 'extra'));
		const cleared = await service.updateConfig({
			applicationsRoot: null,
	...
	test('round-trips defaultProvider and provider config through settings', async () => {
		...
		expect(result.config.providers.zhipu!.model).toBe('glm-5.3');
	...
	test('round-trips telegram channel settings through settings config', async () => {
		...
		const reloadedAfterClear = await service.getConfig();
		expect(reloadedAfterClear.telegram).toEqual({
			allowedChatIds: [],
			botTokenConfigured: false,
		});
		expect('botToken' in reloadedAfterClear.telegram).toBe(false);
...
describe('environment-supplied credentials never enter the config file', () => {
	// The point of AIDD_WEB_AUTH_TOKEN / AIDD_TELEGRAM_BOT_TOKEN is that `~/.aidd/config.json`
	// can hold no credential at all -- an agent asked to read the config file then has nothing
	// to disclose. A Save that copied the environment's value into the file would undo that
	// silently, on the next settings change anybody made.
	...
	test('a save neither persists nor loses an environment-supplied web.authToken', async () => {
		const workspace = await testTempDir('aidd-web-settings-env-auth-');
		const configPath = join(workspace, 'config.json');
		const allowedRoot = join(workspace, 'apps');
		await mkdir(allowedRoot, { recursive: true });
		await Bun.write(configPath, JSON.stringify({ cli: 'native' }));

		await withEnv({ AIDD_WEB_AUTH_TOKEN: 'env-supplied-token' }, async () => {
			// Mirrors a started backend: app.ts resolves through `resolveConfig`, which applies
			// the overlay, so the running config already carries the environment's token.
			const service = new SettingsService(
				makeConfig({
					...runtimeWeb,
					allowRemote: true,
					allowedRoots: [allowedRoot],
					authToken: 'env-supplied-token',
					dataDir: join(workspace, 'data'),
					hostname: '0.0.0.0',
				}),
				configPath,
			...
			expect(written.web).not.toHaveProperty('authToken');
			// An environment token counts as configured, so the save must not decide the remote
			// listener has no token and mint a fresh one into the file.
			expect(JSON.stringify(written)).not.toContain('env-supplied-token');

			// ...and the running config keeps it. Dropping it here would leave a remote listener
			// authenticating against nothing until the next restart.
			expect(saved.resolvedConfig.web.authToken).toBe('env-supplied-token');
			expect(saved.config.authTokenConfigured).toBe(true);
			expect((await service.getConfig()).authTokenConfigured).toBe(true);
		});

		await removeTempTree(workspace);
	});

	test('a save neither persists nor loses an environment-supplied telegram botToken', async () => {
		const workspace = await testTempDir('aidd-web-settings-env-bot-');
		const configPath = join(workspace, 'config.json');
		const allowedRoot = join(workspace, 'apps');
		await mkdir(allowedRoot, { recursive: true });
		// allowedChatIds is access control, not a credential, and stays in the file.
		await Bun.write(
			configPath,
			JSON.stringify({ channels: { telegram: { allowedChatIds: [42] } } }),
		);

		await withEnv({ AIDD_TELEGRAM_BOT_TOKEN: 'env-supplied-bot-token' }, async () => {
			const service = new SettingsService(
				makeConfig({
					...runtimeWeb,
					allowedRoots: [allowedRoot],
			...
			expect((await service.getConfig()).telegram?.botTokenConfigured).toBe(true);
			...
			const saved = await service.updateConfig({
				...
				ignoredFolders: ['node_modules'],
				reasoningEffort: 'low',
				telegram: { allowedChatIds: [42, 99] },
			});

			const written = JSON.parse(await readFile(configPath, 'utf8')) as {
				channels?: { telegram?: Record<string, unknown> };
			};
			expect(written.channels?.telegram?.allowedChatIds).toEqual([42, 99]);
			// Not written as "" either: a blank credential field reads as configured-but-broken.
			expect(written.channels?.telegram).not.toHaveProperty('botToken');
			expect(JSON.stringify(written)).not.toContain('env-supplied-bot-token');

			expect(saved.resolvedConfig.channels?.telegram?.botToken).toBe(
				'env-supplied-bot-token',
			);
			expect(saved.config.telegram?.botTokenConfigured).toBe(true);
		});
