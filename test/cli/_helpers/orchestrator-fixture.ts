 ...
 * margin. The ordinary suite uses eight workers and the 15s default per-test ceiling from
 * DEFAULT_TEST_TIMEOUT_MS; coverage runs serially under instrumentation. Either mode can amplify a
 * run that takes ~1.8s alone into a several-fold tail. These tests are not slower than the ones
 * above — they have nothing left to trade. This only grants anything while it stays ABOVE that
 * default; test/scripts/test-run-arguments.test.ts asserts it does. */
 ...
 * leftover features. The root is also removed once at suite startup because an interrupted test
 * process never reaches `afterAll`; without that recovery, its first fixture can inherit a stale
...
export function createOrchestratorTestContext(scope: string): {
	...
	const initialCleanup = removeTempTree(tmpRoot);
	return {
		cleanup: async () => {
