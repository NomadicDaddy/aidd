...
describe('Native provider client', () => {
	...
	test('does not fold an ambient OPENAI_API_KEY into a non-openai provider', async () => {
		// OPENAI_API_KEY is a ubiquitous shell variable; it must authenticate only the
		// openai provider, never silently stand in for a missing zhipu/xai key.
		await expect(
			resolveDefaultNativeClientConfig(
