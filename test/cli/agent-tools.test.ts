...
describe('native tools', () => {
	...
	test('runs bash with a minimal environment', async () => {
		...
		try {
			const result = await executeTool(
				'bash',
				JSON.stringify({
					// Probed through `env` rather than `${VAR-unset}`: the policy denies parameter
					// operations and unbound references, so the test cannot expand the names itself.
					command:
						"if env | grep -q '^AIDD_SECRET_SENTINEL='; then echo secret=set; else echo secret=unset; fi; if env | grep -qi '^PATH='; then echo path=set; else echo path=unset; fi",
				}),
				process.cwd(),
			);
	...
	test('returns a bash timeout result instead of waiting for outer idle handling', async () => {
		...
		const result = await executeTool(
			'bash',
			JSON.stringify({ command: 'sleep 10', timeout_ms: 50 }),
			process.cwd(),
		);
