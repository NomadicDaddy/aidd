...
describe('parseArgs', () => {
	...
	test('accepts directives with a body from --prompt or --skill', () => {
		...
		const skillRun = parseArgs(['--directive', '--skill', 'review-or-create-doc']);
		expect(skillRun.skillId).toBe('review-or-create-doc');
	...
	test('rejects unknown command and catalog flags', () => {
