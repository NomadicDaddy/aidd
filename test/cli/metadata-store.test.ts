...
describe('FileAiddStore feature compatibility', () => {
	...
	test('counts only coherently completed features as passing', async () => {
		const store = await makeStore('coherent-stats');
		await store.writeFeature({ id: 'done', passes: true, status: 'completed' });
		await store.writeFeature({ id: 'active-conflict', passes: true, status: 'in_progress' });
		await store.writeFeature({ id: 'waiting', passes: false, status: 'waiting_approval' });
		await store.writeFeature({ id: 'open', passes: false, status: 'backlog' });

		const stats = await store.getFeatureStats();
		expect(stats.passing).toBe(1);
		expect(stats.failing).toBe(2);
		expect(stats.waitingApproval).toBe(1);
		expect(stats.passing + stats.failing + stats.waitingApproval).toBe(stats.total);
	});

	test('selects in-progress before backlog and skips unselectable statuses', async () => {
	...
	test('tolerates missing required artifacts on a pre-onboarding ingest', async () => {
		const store = await makeStore('fresh-ingest');
	...
	test('flags passes true for every non-completed status', async () => {
		const store = await makeStore('invalid-feature');
		for (const status of ['backlog', 'in_progress', 'waiting_approval'] as const) {
			await store.writeFeature({
				id: `feature-${status.replace('_', '-')}`,
				passes: true,
				status,
			});
		}
		...
		const validation = await store.validateFeatures();
		expect(validation.valid).toBe(false);
		for (const status of ['backlog', 'in_progress', 'waiting_approval']) {
			expect(
				validation.issues.some((issue) =>
					issue.message.includes(`passes=true but status is '${status}'`),
				),
			).toBe(true);
		}
	});
