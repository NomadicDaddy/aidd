...
} from '../../shared/src/modes/audit-parsing.ts';
...
describe('mode handlers', () => {
	...
	test('coding mode auto-creates a single-milestone roadmap when none exists', async () => {
		...
		const work = await createModeHandler(plan(projectDir)).selectWork({ projectDir, store });
		// A roadmap-less project is gated rather than skipped: only the
	...
	test('coding mode records an honest self-park when the agent emits no marker', async () => {
		...
		// The run still exits 0 with stopReason 'completed', so the summary marker is the only
		// thing that stops the runs row — and any pipeline session built from it — reading green.
		expect(result.summary).toContain(parkedWorkMarker);
	...
	test('audit mode skips duplicate open findings by source and title', async () => {
		...
		const result = await mode.processResult(
			{ projectDir, runId: 'run-audit-duplicate', store },
			{
				events: [],
				exitCode: 0,
				filesModified: [],
		...
		const suppressedEvents = (await store.readFindingEvents()).filter(
			(event) => event.event === 'suppressed-duplicate',
		);
		expect(result.artifacts?.findingsCreated).toBe(0);
		expect(result.artifacts?.suppressedDuplicateCount).toBe(suppressedEvents.length);
