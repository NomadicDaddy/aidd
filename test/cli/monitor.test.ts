...
describe('monitorBackend', () => {
	...
	test('keeps Claude Code tool-progress heartbeats verbatim in raw_log', async () => {
		...
		const events = await collect(
			runProcessBackend(
				{
					args: ['-e', `process.stdout.write(${JSON.stringify(`${heartbeat}\n`)})`],
					backend: 'claude-code',
					command: process.execPath,
				},
				{ cwd: '.', text: 'prompt' },
		...
		const rawTranscript = events
			.filter((event) => event.type === 'raw_log' && event.stream === 'stdout')
			.map((event) => (event.type === 'raw_log' ? event.chunk : ''))
			.join('');

		expect(rawTranscript).toBe(`${heartbeat}\n`);
