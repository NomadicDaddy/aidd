...
describe('prompt compiler', () => {
	...
	// Inheriting a dead address unannounced costs most of an iteration in rediscovery (probe,
	// rebuild, restart) before the agent finally parks, so the probe result is stated outright.
	...
	test('applies feature filter before CLI prompt', async () => {
		...
		const contractIndex = compiled.text.indexOf('## aidd RESULT CONTRACT');
	...
	test('adds coding result contract for feature completion marker', async () => {
		...
		expect(compiled.text).toContain('## aidd RESULT CONTRACT');
		expect(compiled.text).toContain(
...
describe('prompt compiler — feature.json write matrix', () => {
	...
	test('selected-feature writes satisfy the result contract resolution-note requirement', async () => {
		...
		// permit writing it on the selected feature — this pairing is the one that has to hold.
	...
	test('feature note appends normalize every accepted persisted shape', async () => {
		const text = await compileCoding();
		expect(text).toContain('a missing or null value becomes `[]`');
		expect(text).toContain('a string becomes `[existingString]`');
		expect(text).toContain('Preserve every existing string');
		expect(text).toContain('persist an array');
	});

	test('numbered spec amendments continue from the final numbered line', async () => {
		const text = await compileCoding();
		expect(text).toContain('continue from the number on the **last numbered line**');
		expect(text).toContain('never use the global maximum');
		expect(text).toContain('Preserve all existing block numbering');
	});
...
describe('prompt compiler — dependency graph', () => {
	...
	// shelling out over the feature files must be gone from the source prompt, not merely contradicted.
