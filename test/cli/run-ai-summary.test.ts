...
import {
	...
	hasDestroyedLeasedFeatureMarker,
...
describe('writeRunSummary with stub summarizer', () => {
	...
	// The offending run is typically a single-iteration directive: it declares itself complete, so
	// the carryover note the guard raised is never drained into another prompt. The run summary is
	// the only channel that still reaches the operator and the ledger.
	test('records feature records this run destroyed under another run’s lease', async () => {
		const store = await makeStore('destroyed-leases');
		const runtimePlan = plan(store.projectDir);
		const acc = {
			commitsCreated: [],
			completedFeatures: new Set<string>(),
			destroyedLeasedFeatures: [{ featureId: 'feature-claimed', runId: 'run_holder' }],
			auditFindings: {},
			commandsRun: new Set<string>(),
			filesCreated: new Set<string>(),
			filesEdited: new Set<string>(),
		...
		await writeRunSummary(
			...
			'completed',
			0,
			'consolidate-features finished with exit code 0',
		);

		// The directive itself succeeded, so the exit code stays 0 — this is observational, like the
		// dirty-source markers. What must not happen is the damage going unrecorded.
		expect(capturedSummary!.exitCode).toBe(0);
		expect(hasDestroyedLeasedFeatureMarker(capturedSummary!.summary)).toBe(true);
		expect(capturedSummary!.summary).toContain('feature-claimed');
		expect(capturedSummary!.summary).toContain('run_holder');
