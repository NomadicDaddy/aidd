...
const EXPECTED_BUNDLED_SKILL_CATEGORIES = {
	...
	refactor: 'runtime',
	'refresh-project-artifacts': 'recipe-maturity',
...
describe('skill catalog', () => {
	...
	test('enforces Agent Skills identifiers, matching names, and descriptions', () => {
		...
		expect(definition(withDescription('a')).description).toBe('a');
		expect(definition(withDescription('a'.repeat(1024))).description).toHaveLength(1024);
		expect(() => definition(withDescription('a'.repeat(1025)))).toThrow(
			/description must be at most 1024 characters/,
		);
		expect(() => definition(withDescription('   '))).toThrow(
	...
	test('reads support files and merges bundled and imported roots', async () => {
		...
		try {
			...
			await Bun.write(join(root, 'skills', 'alpha', 'SKILL.md'), alphaBody);
			await Bun.write(join(root, 'skills', 'alpha', 'templates', 'one.md'), 'template');
			await Bun.write(join(dataDir, 'skills', 'beta', 'SKILL.md'), betaBody);
			await Bun.write(
			...
			expect(alpha.definitionSha256).toBe(sha256Text(alphaBody));
			expect(alpha.supportPaths).toEqual(['templates/one.md']);
			const skills = await listSkillDefinitions(root, dataDir);
			expect(skills.find(({ id }) => id === 'beta')?.definitionSha256).toBe(
				sha256Text(betaBody),
			);
	...
	test('compiles invocation arguments and support paths into a directive', () => {
		...
		expect(directive).toContain(
			'If the work creates a skill package or edits any file in one, ensure that package has an aligned `agents/openai.yaml`.',
		);
		expect(directive).toContain(
			'Include quoted `interface.display_name`, `interface.short_description` (25-64 characters), and `interface.default_prompt` that mentions the skill using its actual `$skill-id` token.',
		);
		expect(directive).toContain(
			'Preserve existing icons, brand color, policy, dependencies, and other valid fields unless the requested change makes them stale.',
		);
		expect(directive).toContain(
	...
	test('tester stages the fallback used to convert findings into issue features', async () => {
