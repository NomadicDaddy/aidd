...
const view = createElement(
	...
	createElement(SidebarNav, { badges: {}, collapsed: true }),
);
...
describe('shell navigation landmarks', () => {
	...
	test('exposes the mobile overflow and keeps the group boundaries visible', async () => {
		...
		expect(nav).toContain('max-sm:data-[overflow-start=true]:[--fade-start:2rem]');
		expect(nav).toContain('max-sm:data-[overflow-end=true]:[--fade-end:2rem]');
		expect(nav).toContain('max-sm:[mask-image:linear-gradient(to_right,');
		expect(nav).toContain('scroller.dataset.overflowStart = String(flags.start)');
		expect(nav).toContain('scroller.dataset.overflowEnd = String(flags.end)');
	...
	test('renders direct routes through the measured mobile navigation strip', () => {
		for (const pathname of ['/', '/audits', '/about']) {
			const markup = renderSidebar(pathname);

			expect(markup).toContain('data-sidebar-nav-scrollport=""');
			expect(markup).toContain('max-sm:data-[overflow-start=true]:[--fade-start:2rem]');
			expect(markup).toContain('max-sm:data-[overflow-end=true]:[--fade-end:2rem]');
			expect(markup).toContain('aria-current="page"');
		}
	});

	test('clips a destination label inside the rail while its width animates', () => {
		// The rail's width transitions over 200ms but `collapsed` flips the labels back on at
		// once, so crossing the breakpoint lays a full label out in a 64px box for that window.
		// Measured before the fix, "Dashboard" painted ~57px past the rail's right edge, over the
		// page beside it. Each destination has to clip its own content from `sm` up.
		...
		for (const link of links) {
			expect(link).toContain('sm:overflow-hidden');
		}
	});
...
describe('command palette glyph parity', () => {
	test('names the search combobox independently from its placeholder', async () => {
		const palette = await readSource('components/shared/CommandPalette.tsx');

		expect(palette).toMatch(/<CommandInput\s+aria-label="Search commands"/);
		expect(palette).toContain('placeholder="Search navigation, projects, actions…"');
	});
...
describe('shortcut chord separators', () => {
	...
	test('marks the overlay navigation shortcuts as sequential', async () => {
		...
		const shortcuts = await readSource('lib/keyboardShortcuts.ts');

		expect(overlay).toContain('shortcuts: navigationShortcuts');
		expect(shortcuts.match(/sequential: true/g)).toHaveLength(3);
