...
function renderArtifactGroups(): string {
	const script = [
		...
		"const view = createElement(ArtifactGroups, { disabled: false, filter: 'all', maturity, onFilterChange: () => undefined, onOpen: () => undefined, onToggleSkip: () => undefined, records, skipSet: new Set() });",
...
describe('artifact inventory', () => {
	...
	test('renders deployment evidence through the real React component', () => {
		...
		expect(html).toContain('>Shipped</h4>');
		...
		expect(html).toContain('>Other artifacts</h4>');
