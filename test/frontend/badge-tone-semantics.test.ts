...
describe('badge tone semantics', () => {
	...
	test('keeps inert artifact and draft states neutral', async () => {
		...
		expect(artifact).toMatch(/<Badge tone="neutral">\s*Not applicable\s*<\/Badge>/u);
		expect(artifact).toMatch(
			/skipped \? \([\s\S]*?<Badge tone="neutral">\s*Not applicable\s*<\/Badge>[\s\S]*?\) : \([\s\S]*?<Badge tone=\{status\.tone\}>/u,
		);
