...
const exemptions: { file: string; why: string }[] = [
	...
	{
		file: 'projects/detail/FindingDismissalDialog.tsx',
		why: 'dialog title, wired to aria-labelledby',
	},
	{
	...
	{
		file: 'projects/detail/SelectedFeaturePanel.tsx',
	...
	{ file: 'dashboard/DirectorQueueCard.tsx', why: 'per-suggestion row title inside a list' },
	{
		...
		why: 'shared per-suggestion row identity in the two dashboard queues, not a card section',
	},
...
describe('CardHeader adoption', () => {
	...
	test('opens every Project Detail tab with the same header shape', async () => {
		...
		for await (const file of glob.scan({ absolute: false, cwd: tabRoot, onlyFiles: true })) {
			...
			// Code opens directly on its labelled browser Card. A second "Code" heading would
			// repeat the selected tab without adding context; the search, file count and current
			// path supply the opening surface's useful identity instead.
			if (file === 'CodeTab.tsx' && text.includes('<Card className="@container')) continue;
	...
	test('the tab intro names its panel without drawing a box around the name', async () => {
		...
		// A tab whose header carries controls keeps its Card below the shared opening line, because
		// there the panel is holding something.
		const milestones = await source('projects', 'detail', 'MilestonesTab.tsx');
		expect(milestones).toContain('<TabIntro');
		expect(milestones).toContain('<CardHeader');
	});

	test('routes the seven remediated opening lines through TabIntro first', async () => {
		for (const [file, title] of [
			['ArtifactsTab.tsx', 'Artifacts'],
			['ReportsTab.tsx', 'Reports'],
			['HistoryTab.tsx', 'History'],
			['MilestonesTab.tsx', 'Milestones'],
			['DiaryTab.tsx', 'Diary'],
			['ProfileTab.tsx', 'Profile'],
			['AuditsTab.tsx', 'Audits'],
		] as const) {
			const tab = await source('projects', 'detail', file);
			const introAt = tab.indexOf('<TabIntro');
			const contentAt = Math.max(
				tab.indexOf('<Card'),
				tab.indexOf('<DiaryFeed'),
				tab.indexOf('<FilterToolbar'),
			);

			expect(introAt).toBeGreaterThan(-1);
			expect(tab).toContain(`title="${title}"`);
			if (file === 'DiaryTab.tsx') {
				expect(tab).toContain('filterHeader={<TabIntro title="Diary" />}');
			} else {
				expect(contentAt).toBeGreaterThan(introAt);
			}
		}

		const profile = await source('projects', 'detail', 'ProfileTab.tsx');
		expect(profile.indexOf('<TabIntro')).toBeLessThan(profile.indexOf('<EditorActionBar'));

		const history = await source('projects', 'detail', 'HistoryTab.tsx');
		expect(history).not.toContain('title="Project history"');
		const reports = await source('projects', 'detail', 'ReportsTab.tsx');
		expect(reports).not.toContain('title="Project reports"');
		expect(reports).toContain('label="Kind"');

		const milestones = await source('projects', 'detail', 'MilestonesTab.tsx');
		expect(milestones).not.toContain('title="Roadmap actions"');
		expect(milestones).toContain('title="Milestones"');

		const diary = await source('projects', 'detail', 'DiaryTab.tsx');
	...
	test('keeps Project Detail card headings below their containing panel', async () => {
		...
		expect(repositoryRefs).toContain('<RepositoryPanelHeading');

		const repositoryPanelHeading = await source(
			'projects',
			'detail',
			'RepositoryPanelHeading.tsx',
		);
		expect(repositoryPanelHeading).toContain('headingLevel={4}');

		const overview = await source('projects', 'detail', 'OverviewTab.tsx');
		expect(overview).not.toContain('title="aidd activity"');
		const activity = await source('projects', 'detail', 'RecentActivity.tsx');
		expect(activity).toContain('label="aidd state"');

		const milestones = await source('projects', 'detail', 'MilestonesTab.tsx');
		expect(milestones).toContain('<TabIntro');
		expect(milestones).toContain('title="Milestones"');
		expect(milestones).toContain('title="Roadmap order"');
		expect(milestones).toContain('headingLevel={3}');

		const reports = await source('projects', 'detail', 'ReportsTab.tsx');
		expect(reports).toContain('<TabIntro');
		expect(reports).toContain('title="Reports"');
		expect(reports).toContain('<FilterToolbar');
	});
});
