...
describe('shared section caption', () => {
	...
	test('restates exactly those utilities behind the cmdk heading variant', async () => {
		...
		const variants = [...command.matchAll(/\[&_\[cmdk-group-heading\]\]:([\w./[\]-]+)/g)].map(
			(match) => match[1] ?? '',
		);

		// cmdk owns the heading element, so the shared string cannot be applied to it directly —
		// this list is the copy, and it has to stay a copy of the same utilities.
		for (const utility of utilities) {
			expect(variants).toContain(utility);
		}
		// Spacing is the palette's own; nothing else may drift in.
		expect(variants.filter((variant) => !utilities.includes(variant)).sort()).toEqual([
			'px-2',
			'py-1.5',
		]);
	});
