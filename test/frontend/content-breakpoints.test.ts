...
const contentBreakpointContracts = [
	...
	['pages/projects/detail/AuditsDesktopTable.tsx', ['@min-[60rem]:block']],
	['pages/projects/detail/AuditsMobileList.tsx', ['@min-[60rem]:hidden']],
	['pages/projects/detail/FeaturesDesktopTable.tsx', ['@min-[61rem]:block']],
	['pages/projects/detail/FeaturesTab.tsx', ['@min-[61rem]:hidden']],
	[
		'pages/projects/detail/workingTree/WorkingTreeCard.tsx',
		['@min-[48rem]:block', '@min-[48rem]:hidden'],
	],
...
describe('content-aware responsive breakpoints', () => {
	...
	test('gates and aligns the shared page header row on its own content width', async () => {
		const source = await readFile(
			join(frontendSource, 'components', 'shared', 'PageHeader.tsx'),
			'utf8',
		);

		expect(source).toContain('const rail = useContentRail();');
		expect(source).toContain(
			'<header className="@container relative z-10" data-content-rail={rail}>',
		);
		expect(source).toContain(
			'flex flex-col gap-2 @min-[61rem]:flex-row @min-[61rem]:items-start @min-[61rem]:justify-between @min-[61rem]:gap-3',
		);
		expect(source.indexOf('{breadcrumb !== undefined && (')).toBeLessThan(
			source.indexOf('className="flex flex-col gap-2'),
		);
