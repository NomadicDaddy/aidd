...
describe('diary polish', () => {
	test('states the mode and the project once, not twice', () => {
		...
		// The project is now its own fixed display-label column rather than a trailing chip.
		expect(html).toContain('truncate text-xs text-muted-foreground');
		expect(html).toContain('>aidd</span>');
	});
	...
	test('separates the meta parts and sets each in the face its content asks for', () => {
		...
		// The project occupies its fixed sans column; mode and duration remain structured metadata.
		expect(html).toContain('truncate text-xs text-muted-foreground');
		...
		// could see.
		expect(html.match(/aria-hidden="true" class="text-muted-foreground"/gu)).toHaveLength(2);
