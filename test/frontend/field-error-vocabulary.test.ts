...
/** Comments naming a literal a file no longer carries are the record of why it went, not the code. */
...
describe('one vocabulary for an invalid field', () => {
	test('FieldRow puts the message and the aria-invalid on the same decision', () => {
		...
		// As two props at every call site the two halves drift apart in both
	...
	test('an existing description survives a shared hint and error being added to it', () => {
		const script = [
			"import { createElement } from 'react';",
			"import { renderToStaticMarkup } from 'react-dom/server';",
			"import { FieldRow } from './src/components/ui/field.tsx';",
			"const child = createElement('input', { 'aria-describedby': 'hint-1' });",
			"const row = createElement(FieldRow, { error: 'Bad', hint: 'Expected format', label: 'Id' }, child);",
			'console.log(renderToStaticMarkup(row));',
		].join('\n');
		const result = Bun.spawnSync([process.execPath, '-e', script], {
			cwd: FRONTEND_ROOT,
		...
		// An explicit description remains first, followed by the shared hint and error. The error is
		// rendered first so it stays beside the invalid control, while all three ids still resolve to
		// content in the field.
		const describedIds = html.match(/aria-describedby="([^"]+)"/)?.[1]?.split(' ') ?? [];
		expect(describedIds).toHaveLength(3);
		expect(describedIds[0]).toBe('hint-1');
		expect(html).toContain(`id="${describedIds[1]}"`);
		expect(html).toContain('Expected format');
		expect(html).toContain(`id="${describedIds[2]}"`);
		expect(html).toContain('Bad');
		expect(html.indexOf(`id="${describedIds[2]}"`)).toBeLessThan(
			html.indexOf(`id="${describedIds[1]}"`),
		);
	});
	...
	test('the Skills project select is required, not invalid, on first paint', async () => {
		...
		expect(form).toMatch(/<FieldRow[\s\S]*label="Project"[\s\S]*required>/);
