...
describe('mobile metric summary grids', () => {
	...
	test('uses 390px container arrangements while retaining the 320px fallback', async () => {
		const [dashboard, interview, telemetry] = await Promise.all([
			readSource('pages/dashboard/DashboardMetrics.tsx'),
		...
		expect(interview).toContain('role="progressbar"');
		...
		expect(telemetry).toContain(
			'grid gap-3 @min-[20rem]:grid-cols-2 @min-[45rem]:grid-cols-4 @min-[61rem]:grid-cols-8',
		);
		expect(dashboard.match(/<Metric/gu)).toHaveLength(4);
		expect(interview).not.toContain('<Metric');
		// Three headline Metrics remain direct. The shared eight-outcome order maps through one
		// interactive Metric component so the tiles cannot drift from the canonical vocabulary.
		expect(telemetry.match(/<Metric/gu)).toHaveLength(4);
		expect(telemetry.match(/<OutcomeMetric/gu)).toHaveLength(1);
		expect(telemetry).toContain('telemetryOutcomeOrder.map((outcome) => (');
		expect(telemetry.match(/size="compact"/gu)).toHaveLength(1);
