...
function readNavItems(source: string): { icon: string; label: string }[] {
	const iconByPath = new Map(
		[...source.matchAll(/^\t'([^']+)': (\w+),$/gmu)].map((match) => [
			match[1] ?? '',
			match[2] ?? '',
		]),
	);
	return NAV_DESTINATIONS.map((destination) => ({
		icon: iconByPath.get(destination.to) ?? '',
...
describe('navigation glyph uniqueness', () => {
	...
	test('separates Diary from Docs, which both used BookOpen', async () => {
		...
		expect(source).toContain("lucide-react/dist/esm/icons/book-open'");
	});
	...
	test('demotes the shell preference controls to compact icon buttons', async () => {
		...
		// Both preferences are icon buttons: as full-width ghost `Button`s carrying a visible text
		// label they would have the same weight as the launch anchor and the report row above.
