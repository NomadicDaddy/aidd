...
describe('pipeline step duration stays live while a step runs', () => {
	...
	test('report-page step rows tick only while live and preserve terminal nulls', async () => {
		const row = await readSource('pages', 'pipelineSessions', 'StepRows.tsx');
		const duration = await readSource('pages', 'pipelineSessions', 'pipelineStepDuration.ts');
		expect(duration).toContain('formatActiveDuration(step.durationMs, step.startedAt, now)');
		expect(duration).toContain("step.status === 'queued' || step.status === 'running'");
		expect(duration).toContain(
			"step.durationMs === null ? '—' : formatDuration(step.durationMs)",
		);
		expect(row).toContain('pipelineStepDuration(step, now)');
	});
