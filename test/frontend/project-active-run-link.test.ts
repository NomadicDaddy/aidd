...
function renderActiveRunLink(
	activeRuns: { count: number; latestRunId: null | string },
	empty: 'hidden' | 'placeholder' = 'hidden',
): string {
	const script = [
		...
		`const link = createElement(ProjectActiveRunLink, { activeRuns: ${JSON.stringify(activeRuns)}, empty: '${empty}' });`,
...
describe('ProjectActiveRunLink', () => {
	...
	test('renders the table placeholder when a project has no active runs', () => {
		expect(renderActiveRunLink({ count: 0, latestRunId: null }, 'placeholder')).toBe(
