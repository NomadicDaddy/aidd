...
const common = {
	emptyFilters: { inForce: [{ label: 'Search', value: 'aidd' }], onReset: () => {} },
	gitStatus: undefined,
	onRefresh: () => {},
	onToggleSort: () => {},
	projectView: 'cards',
...
describe('row height is stable across the first edit', () => {
	...
	test('the facet selects take their width from the column, not their own content', async () => {
		const source = await read('pages/projects/profileMatrix/ProfileMatrixRow.tsx');
		expect(source).toContain('${quietSelectClass} h-8 w-full px-2 text-xs');
	});
});
...
describe('the resting Summary view renders no disabled control', () => {
	test('row commit controls stay in the pinned identity cell without adding a column', async () => {
		...
		expect(table).not.toContain('showActions');
		expect(table).not.toContain('pinnedRightEdgeClass');
		expect(row).toContain('className="relative min-h-10 pr-20"');
		expect(row).toContain('className="absolute top-0 right-0 flex items-center gap-1"');
	});
...
describe('the pinned seam is drawn from one token', () => {
	test('the shared class faces the scrolling side', async () => {
		const styles = await read('lib/tableStyles.ts');

		// A negative x-offset on an inset shadow lays it along the right inner edge.
		expect(styles).toContain(
			"export const pinnedLeftEdgeClass = 'shadow-[inset_-8px_0_8px_-8px_rgba(0,0,0,0.35)]';",
		);
		expect(styles).not.toContain('pinnedRightEdgeClass');
	});
...
describe('every filter can express every value its column renders', () => {
	...
	test('the control offers one option per posture the column renders', () => {
		for (const value of ['standard', 'low', 'full']) {
			expect(profileMatrixPostureOptions).toContainEqual(expect.objectContaining({ value }));
		}
	});
});
...
describe('the phone card is the table, not a subset of it', () => {
	...
	test('every column the table has beyond the facets is on the card too', async () => {
		...
		// columns. A card that stops after the audit counts silently loses the hardening triggers
		// and the last-written date on a phone, and the operator has no way to know a column
...
describe('project card heights converge', () => {
	test('the milestone run caps at three chips plus a +N badge', async () => {
		...
		expect(card).toContain('const visibleMilestones = milestoneNames.slice(0, 3);');
...
describe('the card attribute list forms a column', () => {
	test('every row sits on one label track instead of starting after its own label', async () => {
		...
		// The label must not sit inside the value cell as bare text.
...
describe('both views can order the same list the same way', () => {
	...
	test('the filter footer renders the card control with the table sort handler', async () => {
		const page = await read('pages/projects/ProjectsPage.tsx');
		const toolbar = await read('pages/projects/ProjectsToolbar.tsx');
		const control = await read('components/shared/CardSortControl.tsx');

		expect(page).toContain('<CardSortControl');
		expect(page).toContain('onToggleSort={toggleSort}');
		...
		expect(control).toContain('max-sm:mx-4');
		expect(control).toContain("phoneInset && 'max-sm:mx-4'");
		expect(control).toContain('max-sm:col-span-2 max-sm:w-full');
		expect(control).toContain('max-sm:col-start-2 max-sm:row-start-1');
	});
...
describe('Projects results use one explicit state', () => {
	...
	test('renders the named component for each result state', () => {
		...
		expect(rendered.noMatch).toContain('3 projects discovered.');
		// The narrowed state names what narrowed it while the adjacent toolbar owns the sole reset.
		expect(rendered.noMatch).toContain('<span>In force:</span>');
		expect(rendered.noMatch).toContain('<span>Search</span>');
		expect(rendered.noMatch).toContain('>aidd</code>');
		expect(rendered.noMatch).not.toContain('Reset filters');
		expect(rendered.ready).not.toContain('Sort by');
		expect(rendered.ready).toContain('grid-cols-[repeat(auto-fill,minmax(30rem,1fr))]');
