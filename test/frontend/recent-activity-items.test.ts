...
describe('recentMetadataActivity', () => {
	test('summarizes a run with human launch, runtime, and work details', () => {
		...
		expect(items[0]?.detailParts).toEqual([
			'Web launch',
			'2m',
			'2 files edited, 1 file created, 1 commit',
		]);
	...
	test('includes iteration activity when no run row represents it', () => {
		...
		expect(items[0]?.detailParts).toContain('1 feature touched, smoke:qc passed');
		expect(items[0]?.executionIdentity).toEqual({
			backend: 'native',
			model: undefined,
			provider: undefined,
	...
	test('prefers aiSummary over mechanical count parts in run detail', () => {
		...
		expect(items).toHaveLength(1);
		expect(items[0]?.detailParts).toContain('Implemented the login flow with error handling.');
		expect(items[0]?.detailParts).not.toContain('2 files edited, 1 file created, 1 commit');
	});
	...
	test('falls back to mechanical summary when aiSummary is absent', () => {
		...
		expect(items).toHaveLength(1);
		expect(items[0]?.detailParts).toContain('2 files edited, 1 file created, 1 commit');
	});
