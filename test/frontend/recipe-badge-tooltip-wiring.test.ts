...
// renders the interactive form: Tooltip wraps its native button child in `relative inline-flex`
// and returns the child untouched when content is empty
...
const WRAPPED_TRIGGER =
	/<span class="relative inline-flex max-w-full min-w-0"><button class="relative inline-flex [^"]*focus-visible:/g;
// The interactive trigger is a native `<button>`, not a tabbable `<span>`: the component that owns
// the action owns its focusability, while Tooltip only supplies disclosure behavior.
const WRAPPED_BADGE =
	/<button class="relative inline-flex [^"]*focus-visible:[^"]*"[^>]*><span class="inline-flex items-center gap-1\.5[^"]*">(.*?)<\/span><\/button>/g;
...
function renderMarkup(body: string): string {
	const script = [
		...
		"import { RecipeOverviewMode } from './src/pages/recipes/detail/RecipeOverviewMode.tsx';",
...
// where there is room to read it. One list, not two: a table showing the whole set capped at two
// plus a `+N` leads a row with `failure: stop (3)` — the default, true of every recipe — while the
// card beside it shows only what departed from that default.
...
function counts(markup: string) {
	return {
		...
		triggers: (markup.match(WRAPPED_TRIGGER) ?? []).length,
...
function renderOverview(recipeDefinition: RecipeDefinition): string {
	return renderMarkup(
		`console.log(render(h(RecipeOverviewMode, { onDelete: () => {}, onEdit: () => {}, onReload: () => {}, recipe: ${JSON.stringify(recipeDefinition)} })));`,
	);
}
...
describe('recipe badge tooltip wiring', () => {
	...
	test('the interactive form is still a real tooltip trigger', () => {
		...
		expect(markup).toContain('max-sm:before:-inset-2.5');
	});

	test('a recipe page renders the complete interactive policy summary', () => {
		// The counterpart to the two catalog assertions above: the catalog intentionally limits its
		// policy summary, but the read view must expose every policy before the operator launches it.
		const overview = renderOverview(recipe);
		for (const label of [
			'failure: stop (3)',
			'failure: continue (1)',
			'failure: auto-fix (1)',
		...
		expect(overview).not.toContain('title="Skill intent:');
	});

	test('the read view distinguishes review-only and apply-changes recipes', () => {
		const reviewOverview = renderOverview({ ...recipe, steps: [recipe.steps[3]!] });
		expect(reviewOverview).toContain('skills: review (1)');
		expect(reviewOverview).not.toContain('skills: apply');

		const applyOverview = renderOverview({ ...recipe, steps: [recipe.steps[4]!] });
		expect(applyOverview).toContain('skills: apply (1)');
		expect(applyOverview).not.toContain('skills: review');
	});
