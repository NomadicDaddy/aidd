 ...
 * flagged: a stretched grid row sized by the tall card, with the short one ending in a void.
 */
...
describe('recipe card surface', () => {
	...
	test('stretches to the row and pins its actions to the bottom', () => {
		for (const markup of [short, tall]) {
			expect(markup).toContain('flex h-full flex-col');
			// `mt-auto` is what absorbs the leftover height of a stretched row, so the void lands
			// above the footer instead of below it.
			expect(markup).toContain('mt-auto flex flex-wrap gap-2 border-t border-border pt-3');
		}
	});
