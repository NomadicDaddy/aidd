...
describe('final-check failure surfacing (ISS-001)', () => {
	...
	test('a successful iteration with a failed final check is flagged as a warning, not clean success', () => {
		...
		expect(outcome.label).toBe('Completed · check failed');
	...
	test('a completed/exit-0 run with a failed iteration final check categorizes as Warnings', () => {
		const run = makeRun();
		// classifyRun alone still reports the raw ledger fact (completed/exit 0 → Completed).
		expect(classifyRun(run).tone).toBe('emerald');
		// With its iterations, categorization downgrades to Warnings so it is not filtered as Completed.
		const failingIterations = [
			makeIteration({ finalChecks: { smokeQc: 'failed' }, status: 'success' }),
		];
		expect(categorizeRun(run, failingIterations)).toBe('Warnings');
		expect(categorizeRun(run, [makeIteration({ status: 'success' })])).toBe('Completed');
	});
});

describe('run-level warning downgrade (ISS-003)', () => {
	test('classifyRunWithWarnings presents no-work runs consistently with their filter', () => {
		const run = makeRun({ commitsCreatedCount: 0, completedFeatures: [] });
		...
		expect(categorizeRun(run, [])).toBe('No work');
	});
	...
	test('classifyRunWithWarnings does NOT downgrade for runLedgerDirty alone', () => {
		// Ledger dirt gets its own amber advisory because metrics may be incomplete, but it usually
		// reflects pre-existing operator dirt and pre-fix ledger entries had it unconditionally true.
		// The primary outcome therefore remains unchanged rather than duplicating that advisory.
	...
	test('unattributed concurrent dirt stays successful and renders its own informational badge', () => {
		...
		expect(outcome.tone).toBe('emerald');
		expect(outcome.label).toBe('Completed');
		expect(categorizeRun(run, [])).toBe('Completed');
	...
	test('categorizeRun keeps ledger-dirty runs as Completed', () => {
		const run = makeRun({ runLedgerDirty: true });
		expect(categorizeRun(run, [])).toBe('Completed');
	});
