...
describe('run stopping state', () => {
	...
	test('a payload without the field only stops via the local request', () => {
		const withoutField = makeRun();
		delete (withoutField as Partial<RunRecord>).stopRequested;
		expect(isRunStopping(withoutField, false)).toBe(false);
		expect(isRunStopping(withoutField, true)).toBe(true);
	});
