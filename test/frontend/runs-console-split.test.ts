 ...
 *
 * The threshold is the region's own width (`@min-[81.25rem]:`), not the window's, so the height
...
describe('the live console gets the bulk of its column', () => {
	test('the metadata block is the part that gives', async () => {
		...
		expect(source).toContain(
			'<div className="@min-[81.25rem]:min-h-0 @min-[81.25rem]:overflow-y-auto">',
		);
		const wrapper = source.slice(
			source.indexOf('@min-[81.25rem]:min-h-0 @min-[81.25rem]:overflow-y-auto'),
		);
	...
	test('the transcript has a floor, and the floor is twenty lines', async () => {
		...
		expect(source).toContain('@min-[81.25rem]:min-h-[27rem] @min-[81.25rem]:flex-1');
		expect(source).not.toContain('@min-[81.25rem]:min-h-0 @min-[81.25rem]:flex-1');
	});
	...
	test('the column that does the dividing is still the fixed-height sticky one', async () => {
		...
		expect(page).toContain('@min-[81.25rem]:h-[calc(100dvh-3rem)]');
		expect(page).toContain('@min-[81.25rem]:sticky');
	});
