...
function selectionContainerOpeningTag(source: string): string {
	const clickIndex = source.indexOf('onClick={containerSelectionHandler(');
	expect(clickIndex).toBeGreaterThan(-1);
	const startIndex = Math.max(
		source.lastIndexOf('<div', clickIndex),
		source.lastIndexOf('<tr', clickIndex),
	);
	const endIndex = source.indexOf('>', clickIndex);
	expect(startIndex).toBeGreaterThan(-1);
	expect(endIndex).toBeGreaterThan(clickIndex);
...
describe('Runs row consistency', () => {
	...
	test('run rows use the same Name, Project, and Kind contract on desktop and mobile', async () => {
		for (const file of ['ActiveRunRow.tsx', 'ActiveRunMobileCard.tsx']) {
			...
			const container = selectionContainerOpeningTag(source);
			expect(container).not.toContain('tabIndex={0}');
	...
	test('console selection label contains the visible button text', () => {
		...
		expect(
			consoleSelectionLabel({
				...
				source: 'director',
				summary: 'Director wrote 4 suggestions',
			}),
		).toBe('Show Director wrote 4 suggestions for Director in Live Console');
	});
	...
	test('execution selection uses a pressed native button with visible keyboard focus', async () => {
		...
		const mobileRuns = await readRunSource('ActiveRunMobileCard.tsx');
		...
		expect(mobileRuns).not.toContain('aria-selected');
		expect(pipelines.match(/aria-selected=/gu)).toHaveLength(1);
		const container = selectionContainerOpeningTag(pipelines);
		expect(container).not.toContain('tabIndex={0}');
		expect(container).not.toContain('onKeyDown=');
	});
	...
	test('Active and History share column widths and selected steps stay visible', async () => {
		...
		for (const file of ['ActiveRunRow.tsx', 'PipelineSessionRow.tsx']) {
			expect(await readRunSource(file)).toContain('<td className="max-w-0 py-3 pr-3 pl-4">');
		}
		expect(await readRunSource('PipelineStepTableRows.tsx')).toContain(
		...
		// A sub-row grid that mirrors the colgroup by hand is one rebalance away from leaving every
		// expanded step offset from the columns it is supposed to sit under. The desktop steps are
		// `<td>` in this table, so they take these widths by construction.
	...
	test('run origin labels use the current Director name from one shared helper', async () => {
		...
		// The detail a row opens is the fifth surface, and the one where a drifted label would be
		// hardest to spot: the row and the panel it opens would disagree about the same run. It
		// names its prop differently, so it is checked here rather than in the loop above.
		const detail = await readRunSource('RunDetailPanel.tsx');
		expect(detail).toContain('runSourceLabel(selectedRun)');
		expect(detail).not.toMatch(/Coordinator|'Coord'/);
	});
