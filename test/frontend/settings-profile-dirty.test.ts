...
describe('the Director Profile card has the surface dirty vocabulary', () => {
	...
	test('Save Profile stays secondary to the page-level save', async () => {
		...
		// The page-level save remains the one primary action for Settings.
	...
	test('the dot is announced, not only drawn', async () => {
		...
		// and the same words in the option text of the select the compact strip collapses to below
		// `lg`. A badge with no `badgeLabel` would delete the fact at exactly the width where the
		// triggers stop being on screen.
		expect(toolbar).toContain('<span className="sr-only">unsaved changes</span>');
		expect(toolbar).toContain("badge: <UnsavedDot />, badgeLabel: 'unsaved changes'");
	});
