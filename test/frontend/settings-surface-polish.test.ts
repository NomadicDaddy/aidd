...
function render(imports: string, expression: string): string {
	const script = [
		...
		imports,
		`console.log(renderToStaticMarkup(createElement(PageRail, { rail: 'bounded' }, ${expression})));`,
	].join('\n');
...
describe('settings dirty-tab locator', () => {
	...
	test('locates Spernakit scaffolding changes on Workspace', () => {
		const saved = createBlankSettings();
		const form = { ...saved, spernakitTemplateRef: 'v4.0.0' };

		expect([...dirtySettingsTabs(form, saved)]).toEqual(['workspace']);
	});
...
describe('backend defaults table alignment', () => {
	test('emits one cell per control so each sits under its own header', () => {
		// Four controls sharing a single `<td colSpan={4}>` with a private grid lay out independently
		// of the header row — every `<th>` sits left of the control it names.
		const html = render(
	...
	test('emits plain divs in the stacked mobile layout', () => {
		...
		expect(html.match(/<div class="min-w-0">/g)).toHaveLength(4);
	});
});
...
describe('settings toolbar unsaved locator', () => {
	...
	test('marks the dirty tab, once in each representation of the strip', () => {
		const html = renderToolbar(['run-engine']);
		const triggers = html.indexOf('role="tablist"');
		expect(triggers).toBeGreaterThan(-1);

		// The compact strip renders both forms and lets CSS pick one, so exactly one is ever on
		// screen and each has to carry the marker on its own. A select renders text and nothing
		// else, which is why the amber dot travels into it as words rather than being dropped at
		// the width the strip collapses at.
		for (const half of [html.slice(0, triggers), html.slice(triggers)]) {
			expect(half.match(/unsaved changes/g)).toHaveLength(1);
			expect(half.indexOf('unsaved changes')).toBeGreaterThan(half.indexOf('Run Engine'));
			expect(half.indexOf('unsaved changes')).toBeLessThan(half.indexOf('AI &amp; Director'));
		}
	...
	test('renders no dot when nothing is pending', () => {
		...
		expect(html).not.toContain('<span class="sr-only">unsaved changes</span>');
	});
});
...
describe('settings help text stays out of the accessible name', () => {
	test('associates the Applications Root hint by aria-describedby, not by nesting', () => {
		...
		const input = html.match(/<input[^>]*placeholder="\/path\/to\/applications"[^>]*>/)?.[0];
	...
	test('describes a configured bot token without folding the status into the label', () => {
		...
		settings.telegram = { ...settings.telegram, botTokenConfigured: true };
		const html = render(
		...
		const input = html.match(/<input[^>]*placeholder="Enter replacement token"[^>]*>/)?.[0];
		const hintId = input?.match(/aria-describedby="([^"]+)"/)?.[1];
	...
	test('omits the bot-token hint when there is nothing to describe', () => {
		...
		expect(html).not.toContain('A bot token is configured.');
	});

	test('uses one visible configured-state indicator for the bot token', () => {
		const settings = createBlankSettings();
		settings.telegram = { ...settings.telegram, botTokenConfigured: true };
		const html = render(
			"import { TelegramChannelSection } from './src/pages/settings/TelegramChannelSection.tsx';",
		...
		expect(html.match(/Configured/g)).toHaveLength(1);
		expect(html).toContain('placeholder="Enter replacement token"');
