...
describe('the skills catalog scans by name', () => {
	...
	test('a focus-restored row remains visibly active without claiming selection', () => {
		...
		expect(rendered.focusRestored).toContain('data-active="true"');
		expect(rendered.focusRestored).toContain('data-[active=true]:border-accent');
		expect(rendered.focusRestored).toMatch(
			/aria-label="Imported Skill" aria-selected="false"[^>]*data-active="true"[^>]*id="skill-option-imported-skill"[^>]*tabindex="0"/,
		);
		expect(rendered.filtered).not.toContain('data-active="true"');
	});
	...
	test('the catalog leaves result reporting to the shared filter toolbar', () => {
		expect(rendered.filtered).not.toContain('Showing 1 of 2 skills');
		expect(rendered.selected).not.toContain('Showing 2 of 2 skills');
	});
});
...
describe('the skills page puts the catalog first', () => {
	test('import is a header action opening a dialog, not the first card on the page', async () => {
		const [page, dialog] = await Promise.all([
			Promise.all([
				...
				skillsSource('SkillsHeaderActions.tsx'),
	...
	test('the category filter sits in a full-width card above the split', async () => {
		...
		const filter = await skillsSource('SkillsFilterToolbar.tsx');
		const filterCard = page.indexOf('<SkillsFilterToolbar');
		...
		expect(filter).not.toContain('rail="full"');
		expect(filter).toContain('<SegmentedControl');
		expect(page).toContain('<div className="@container space-y-5">');
		expect(page).toContain("className={showDetail ? 'hidden @min-[40rem]:block'");
	});
	...
	test('the definition block is read as a document, not as source', async () => {
		...
		// is the piece of it that came out. The page is still held to no raw-source rendering,
		// because moving a card into its own file is not the same as having stopped.
		const card = await skillsSource('SkillDefinitionCard.tsx');
		const page = await skillsSource('SkillsPage.tsx');

		// Not the raw file in a `<pre>` reflowed with `break-words whitespace-pre-wrap` so the lines
		// at least stay on screen. Reflowing markdown source is the best a `<pre>` can do; rendering
...
describe('the skill detail card labels its blocks', () => {
	test('every mono block carries a caption naming what it holds', async () => {
		...
		for (const caption of ['Advisory declarations', 'Import source', 'Support files'])
			expect(source).toContain(`title="${caption}"`);
		expect(source).not.toContain('title="Usage"');
	...
	test('card titles collapse to one step', async () => {
		...
		// The title is not a hand-rolled `text-base font-semibold` heading here. That class pair
		// lives once, in CardHeader, so the property this test guards — one step, no
		...
		// There is no pluralized `n files` badge to guard: the SUPPORT FILES block on the same card
		// lists those files by name, so a badge would count what the reader can see three inches
		// below. See skills-rail-polish.test.ts.
	});
