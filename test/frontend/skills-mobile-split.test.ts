...
describe('the skills split has a layout below its split point', () => {
	...
	test('the container is an ancestor of the query, not the queried element', async () => {
		...
		const containerAt = page.lastIndexOf('className="@container', queryAt);
	...
	test('below the split point the two panes are alternatives, not a stack', async () => {
		...
		expect(page).toContain("showDetail ? 'hidden @min-[40rem]:block' : undefined");
	});
	...
	test('mobile selection reflects only visible detail and return restores place and focus', async () => {
		const page = stripComments(await read(PAGE));
		const state = stripComments(await read(MASTER_DETAIL));

		expect(page).toContain('const catalogSelectedId = catalogHasSelection');
		expect(page).toContain('activeId={selectedId}');
		expect(page).toContain('selectedId={catalogSelectedId}');
		expect(state).toContain('catalogScrollYRef.current = window.scrollY');
		expect(state).toContain("window.scrollTo({ behavior: 'auto', top: returnScrollY })");
