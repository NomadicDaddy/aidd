...
const pairedTables: { gate?: string; stack: string; table: string; tier: string }[] = [
	{
	...
	{
		gate: 'pages/projects/detail/workingTree/WorkingTreeCard.tsx',
	...
	{
		stack: 'pages/settings/SystemMetricsVitals.tsx',
		table: 'pages/settings/SystemMetricsVitals.tsx',
		tier: '@min-[45rem]',
	},
	{
...
const unpairedTables: { file: string; why: string }[] = [
	{
		file: 'components/shared/MarkdownTable.tsx',
		why: 'content-defined columns use a keyboard-reachable horizontal scroller',
	},
...
const exemptTables: { file: string; why: string }[] = [
	{
		file: 'pages/audits/tabs/OverridesList.tsx',
		why: 'responsive table renderer is mounted inside one shared measured OverflowScroller',
	},
	{
...
describe('wide tables are contained and replaced', () => {
	...
	test('a wide table is contained AND replaced, both halves at one tier', async () => {
		...
		for (const { gate, stack, table, tier } of pairedTables) {
			...
			const gateSource = gate ? await readPath(gate) : source.slice(0, tableAt);
			if (!gateSource.includes(`${tier}:block`)) {
				offenders.push(`${table}: table not gated ${tier}:block`);
			}
			const stackSource = gate
				? gateSource
				: stack === table
					? source
					: await readPath(stack);
	...
	test('a placeholder is never a table row only', async () => {
		...
		// Loading, error and empty must not exist solely as a `colSpan={6}` <td>, which a card
		// stack has no equivalent of — the stack would render as nothing at all.
	...
	test('the invocations stack carries the columns the table drops below the breakpoint', async () => {
		const table = await read('pages', 'telemetry', 'InvocationsTable.tsx');

		// Hiding Source and Project below the breakpoint only makes sense when there is nowhere
		// else to put them. With a stack there is, so no constant hides them.
