...
function renderAxes(): RenderedAxes {
	const script = [
		...
		'}));',
		'const ratioPoint = [{',
		'bucket: start, cachedTokens: 0, filesChanged: 1, inputTokens: 0,',
		'linesAdded: 2900, linesRemoved: 1000, outputTokens: 0, reasoningTokens: 0,',
		'runs: 1, runsWithFileData: 1, runsWithLineData: 1, runsWithTokenData: 0,',
		'}];',
		...
		'ratio: renderToStaticMarkup(createElement(OutputTimeseriesChart, { bucket: "hour", metric: "lines", points: ratioPoint })),',
...
function categoryLabels(markup: string): string[] {
	const row = /<div aria-hidden="true" class="mt-1\.5 flex h-3 gap-1">(.*?)<\/div>/.exec(markup);
...
describe('Telemetry chart axes', () => {
	...
	test('the value scale carries a max, a midpoint and a baseline on their gridlines', () => {
		...
		// The diverging chart puts zero at its center and signs the lower half, so the gutter stays
		// monotonic while both arms carry the same magnitude at the same distance from zero.
	...
	test('uses one domain for comparable lengths across the diverging baseline', () => {
		const heights = [...rendered.ratio.matchAll(/style="height:(\d+)%"/g)].map((match) =>
			Number(match[1]),
		);
		// The aria-hidden phone bar and desktop tooltip bar carry identical geometry.
		expect(heights).toEqual([73, 25, 73, 25]);
		const [produced, counterpart] = heights.slice(0, 2);
		if (produced === undefined || counterpart === undefined) {
			throw new Error('expected both output chart arms');
		}
		expect(produced / counterpart).toBeCloseTo(2.9, 1);
	});
	...
	test('the diverging chart draws its center baseline as a gridline, not a bespoke rule', async () => {
		...
		expect(output).toContain('divergingTicks(domain)');
		expect(output).not.toContain('top-1/2 h-px bg-muted');
		expect(output).not.toContain('border-dashed');
	});

	test('a count axis is rounded to a whole-number domain, not to the raw data max', () => {
		// An invocations axis labelling the raw max and half of it is how a chart of whole
		// invocations comes to carry a `20.5` tick. Rounding the domain — not just the label —
	...
	test('legend labels use the same readable text step as the chart axes', () => {
		expect(rendered.invocations).toContain(
			'class="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground"',
		);
		expect(rendered.invocations).not.toContain(
			'class="flex flex-wrap gap-x-3 gap-y-1 text-2xs text-muted-foreground"',
		);
	});
