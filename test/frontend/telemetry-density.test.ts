...
describe('telemetry lets the data lead', () => {
	...
	test('the summary tiles anchor their values to a common baseline and pack as the column widens', async () => {
		...
		// The baseline is `Metric`'s, not a `CountCard` class declared here, so this page does not
		// state it — but the property still has to survive, which is why the assertion reads the
		// rendered markup rather than a class name on this page.
		expect(summary).toContain(
			'@min-[20rem]:grid-cols-2 @min-[45rem]:grid-cols-4 @min-[61rem]:grid-cols-8',
		);
		...
		// down it. `justify-between` standing in for the baseline gets it wrong whenever a
		// tile carries a footer: the footer takes its height out of the row the value is being
		// pushed to the bottom of, leaving Priority Health's value 51px above its neighbours'.
	...
	test('the leaderboard columnises rather than leaving a void beside its lower half', async () => {
		...
		// an 876x488 empty region at 2250 — the largest on any surface in the app. The bounded
		// single track prevents one machine label from widening every row; once the card is wide
		// enough, the rows still split into the two columns that close the void.
		expect(leaderboard).toContain(
			'grid grid-cols-[minmax(0,1fr)] gap-2 @min-[45rem]:grid-cols-2',
		);
...
describe('recent invocations table', () => {
	...
	test('keeps below xl the two columns a table would drop', () => {
		// No `hidden xl:table-cell` on the eight cells: hiding Source and Project on a phone because
		// seven columns do not fit is the card stack standing in for itself — a column removed
		// rather than relocated, so the information is simply gone below the breakpoint. With a
		// real stack the hiding is not a trade-off worth making, and no constant expresses it.
	...
	test('the Inspect panel is a lazy-mounted sunken card, not a hand-rolled well', async () => {
		const details = await telemetrySource('InvocationDetails.tsx');

		expect(details).toContain('<Card className="@container w-full p-3" variant="sunken">');
		expect(details).toContain(
			"export const inspectTriggerClass = 'text-xs font-medium text-accent hover:underline'",
		);
		expect(details).toContain('{open ? (');
		expect(rendered.table).not.toContain('bg-muted/90 shadow-inner');
	});
	...
	test('both disclosure triggers on the page wear the app’s chevron', async () => {
		const [details, disclosure, marker, table] = await Promise.all([
			telemetrySource('InvocationDetails.tsx'),
			telemetrySource('TelemetryDisclosure.tsx'),
			readFile(
				join(FRONTEND_ROOT, 'src', 'components', 'shared', 'DisclosureMarker.tsx'),
				'utf8',
			),
			telemetrySource('InvocationsTable.tsx'),
		]);
		...
		expect(marker).toContain("from 'lucide-react/dist/esm/icons/chevron-right'");
		expect(details).toContain('marker:content-none');
		expect(details).toContain('<DisclosureMarker />');
		expect(disclosure).toContain('<DisclosureMarker />');
		expect(table).toContain('<DisclosureMarker open={open} />');
		expect(marker).toContain('group-open:rotate-90');
