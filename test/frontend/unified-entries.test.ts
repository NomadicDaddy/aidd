...
describe('unified execution entries', () => {
	...
	test('the initiator filter separates recorded values and keeps unrecorded rows their own', () => {
		const session = { kind: 'pipeline', session: makeSession() } as const;
		const operator = { kind: 'run', run: makeRun({ initiator: 'operator' }) } as const;
		const automatic = { kind: 'run', run: makeRun({ initiator: 'automatic' }) } as const;
		const unrecorded = { kind: 'run', run: makeRun({ initiator: null }) } as const;

		expect(entryMatchesFilters(operator, baseFilters({ initiator: 'operator' }))).toBe(true);
		expect(entryMatchesFilters(automatic, baseFilters({ initiator: 'operator' }))).toBe(false);
		expect(entryMatchesFilters(automatic, baseFilters({ initiator: 'automatic' }))).toBe(true);

		// The case the feature exists for: narrowing to "somebody asked for this" must not hand back
		// runs nobody recorded an answer for. They are reachable under their own value instead.
		expect(entryMatchesFilters(unrecorded, baseFilters({ initiator: 'operator' }))).toBe(false);
		expect(entryMatchesFilters(unrecorded, baseFilters({ initiator: 'automatic' }))).toBe(
			false,
		);
		expect(entryMatchesFilters(unrecorded, baseFilters({ initiator: 'unknown' }))).toBe(true);
		expect(entryMatchesFilters(unrecorded, baseFilters())).toBe(true);

		// Run-only, exactly as mode is: a recipe session records no initiator of its own.
		expect(entryMatchesFilters(session, baseFilters({ initiator: 'operator' }))).toBe(false);
		expect(entryMatchesFilters(session, baseFilters({ initiator: 'unknown' }))).toBe(false);
		expect(entryMatchesFilters(session, baseFilters())).toBe(true);

		// The badge text is part of the free-text haystack, so typing it filters too.
		expect(entryMatchesFilters(automatic, baseFilters({ query: 'automatic' }))).toBe(true);
		expect(entryMatchesFilters(operator, baseFilters({ query: 'automatic' }))).toBe(false);
	});
