...
} from '../../frontend/src/lib/tableStyles.ts';
import {
...
import { profileFacets } from './src/pages/projects/detail/profile/profile-facets.ts';
...
function matrixRowCells(showFacets) {
	return cells(
		createElement(ProfileMatrixRow, {
			...
			row: matrixRow,
			showFacets,
			visibleFacets: profileFacets.slice(0, 6),
		}),
	);
}
...
describe('wide table column strategy', () => {
	...
	test('the Profile Matrix defaults edit mode to the six posture-driving facets', () => {
		// Summary is five outcome columns. Edit adds the six default exposure facets, while the
		// chooser keeps all ten discoverable without adding an Actions column to the geometry.
		expect(rendered.matrix.summary).toBe(5);
		expect(rendered.matrix.edit).toBe(11);
	});
	...
	test('both tables sit in a scrollport that signals and exposes its overflow', async () => {
		...
		expect(scroller).toContain('role={role}');
	...
	test('shares wide-table slack instead of giving it all to project identity', async () => {
		const [columns, projects, row] = await Promise.all([
			readFile(
				join(frontendSource, 'pages', 'projects', 'projects-table-columns.ts'),
				'utf8',
			),
			readFile(join(frontendSource, 'pages', 'projects', 'ProjectsTableView.tsx'), 'utf8'),
			readFile(join(frontendSource, 'pages', 'projects', 'ProjectTableRow.tsx'), 'utf8'),
		]);

		expect(projects).toContain('className="@container ');
		expect(projects).toContain('<div ref={tableRef}>');
		expect(projects).not.toContain('contentRailClass');
		expect(contentSizedTableClass).toContain('table-auto');
		expect(contentSizedColumnClass).toBe('w-px');
		expect(columns).toContain('`${contentSizedColumnClass} px-3 py-3 whitespace-nowrap');
		expect(projects).toContain('className={contentSizedTableClass}');
		expect(columns).toContain(
			"'w-[min(54vw,18rem)] max-w-[min(54vw,18rem)] max-sm:left-auto sm:w-[44ch] sm:max-w-[44ch]'",
		);
		expect(columns).not.toContain('w-[40%]');
		expect(projects).toContain('projectTableNameCellClass');
		...
		expect(row).toContain('projectTableNameCellClass');
	});
	...
	test('the Profile Matrix header stays put in both axes', async () => {
		...
		// away with the page, which is the defect this replaces. The bound is measured from the
		// scrollport's own top edge, including the optional facet navigator above it.
		expect(matrix).toContain('scrollerClassName={cn(');
	...
	test('the chooser and the mode switch report their own state', async () => {
		...
		expect(chooser).toContain('Optional columns');
