...
describe('findCrossMilestoneViolations', () => {
	...
	test('marks a violation on a completed feature as passing', () => {
		// The severity the milestones callout renders: a violation on a feature that already passes
		// is roadmap accounting after the fact, because there is no work left for it to strand.
		const roadmap = roadmapSchema.parse({
			features: {
				done: { milestone: 'MVP' },
				open: { milestone: 'v1.0' },
		...
		const violations = findCrossMilestoneViolations(roadmap, [
			feature('done', { dependencies: ['late'], passes: true, status: 'completed' }),
			feature('open', { dependencies: ['late'] }),
			feature('late'),
		]);
		expect(violations.map((entry) => [entry.featureDirectory, entry.featurePasses])).toEqual([
			['done', true],
			['open', false],
		]);
	});
