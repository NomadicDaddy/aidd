...
import { auditReportPath } from '../../shared/src/metadata/audit-freshness/metadata.ts';
...
describe('check-audit-artifact-hygiene tool', () => {
	...
	test('accepts a report dated today', async () => {
		...
		try {
			...
			await mkdir(join(tmp, REPORTS), { recursive: true });
			await writeFile(
				auditReportPath(tmp, 'AI', new Date()),
				`# AI audit ${today}\n\n**Date:** ${today}\n`,
			);
	...
	test.each([
		{ zone: 'America/Chicago', instant: '2026-09-06T03:30:00Z', local: '2026-09-05' },
		{ zone: 'Asia/Tokyo', instant: '2026-09-05T18:30:00Z', local: '2026-09-06' },
	])('uses the producer calendar across midnight in $zone', async ({ zone, instant, local }) => {
		const tmp = await testTempDir('aidd-audit-hygiene-zone-');
		try {
			await mkdir(join(tmp, REPORTS), { recursive: true });
			await writeFile(
				auditReportPath(tmp, 'AI', new Date(instant)),
				`# AI audit ${local}\n\n**Date:** ${local}\n`,
			);
			const invoke = () =>
				Bun.spawnSync(
					[
						...
						'-e',
						`import { runAuditArtifactHygiene } from './scripts/check-audit-artifact-hygiene.ts';
				process.exit(runAuditArtifactHygiene(${JSON.stringify(tmp)}, new Date(${JSON.stringify(instant)})));`,
					],
			...
			await writeFile(auditReportPath(tmp, 'FUTURE', future), '# Future audit\n');
			const rejected = invoke();
			expect(rejected.exitCode).toBe(1);
			expect(rejected.stderr.toString()).toContain('(filename)');
		} finally {
			await removeTempTree(tmp);
		}
	});
