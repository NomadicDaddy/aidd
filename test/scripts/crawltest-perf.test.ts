...
import { TestResults } from '../../scripts/crawltest-results.ts';
...
describe('crawltest spernakit-style coverage helpers', () => {
	...
	test('records page assertion failures in the persisted report', () => {
		const results = new TestResults();

		results.addPageAssertionErrors('/projects/aidd', [
			'disabled Move project action was not visibly dimmed',
		]);
		...
		expect(report.errors[0]).toMatchObject({
			details: { route: '/projects/aidd' },
			message: 'disabled Move project action was not visibly dimmed',
			type: 'PAGE_ASSERTION',
		});
	});
