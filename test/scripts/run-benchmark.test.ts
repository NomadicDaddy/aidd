...
describe('benchmark cost resolution', () => {
	...
	test('resolves pricing via cohort target model family', () => {
		const glm = manifest.stacks.find((stack) => stack.label === 'native-glm51-high')!;
		const codex = manifest.stacks.find((stack) => stack.label === 'codex-gpt56-high')!;
		...
		expect(pricingForStack(glm, manifest)?.inputPerMtok).toBe(0.5);
		expect(pricingForStack(codex, manifest)?.inputPerMtok).toBe(1.25);
	...
	test('estimates cost from tokens when none was reported', () => {
		...
		// 1M input tokens at $0.50/Mtok = $0.50.
		expect(estimate).toBeCloseTo(0.5, 6);
		...
		// 1M codex input at $1.25/Mtok + small output.
		expect(resolveCost(0, usage(1_013_545, 11_887), codexPricing)).toBeCloseTo(1.382, 2);
	});

	test('charges cached tokens as an input discount, not additively', () => {
		const codex = manifest.stacks.find((stack) => stack.label === 'codex-gpt56-high')!;
		const pricing = pricingForStack(codex, manifest)!; // input $1.25, cached $0.125
		const allCached = {
		...
		// 1M input, all cached -> billed at the cached rate only ($0.125), not $1.25+$0.125.
		expect(estimateCostFromTokens(allCached, pricing)).toBeCloseTo(0.125, 6);
		const halfCached = {
		...
		// 500k fresh @ $1.25 + 500k cached @ $0.125 = 0.625 + 0.0625 = 0.6875.
		expect(estimateCostFromTokens(halfCached, pricing)).toBeCloseTo(0.6875, 6);
	});
