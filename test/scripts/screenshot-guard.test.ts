...
/** The tracked opt-in. Separate from addScreenshots because the two are independent states. */
const declareCapture = async (root: string): Promise<void> => {
	await writeFile(join(root, '.screenshot-capture'), 'releases carry a capture\n');
};
...
describe('screenshot guard', () => {
	...
	test('a repository that has not declared capture passes', async () => {
		// A CLI or a library never adds the declaration. Treating that as a missing capture would
		// block every tag push in every headless repository the guard ships to.
		const root = await makeRoot();
		const r = await runGuard(root, tagLine('v3.29.0'));
		expect(r.code).toBe(0);
	});

	test('once capture is declared, a tag with no capture under it blocks', async () => {
		// The declaration is what records the opt-in. Collapsing this case together with the one
		// above reads an opted-in repository's forgotten capture as an opted-out repository.
		const root = await makeRoot();
		await declareCapture(root);
		await mkdir(join(root, 'screenshots'), { recursive: true });
		const r = await runGuard(root, tagLine('v3.29.0'));
		expect(r.code).toBe(1);
		expect(r.err).toContain('has no v3.29.0/');
	});

	test('a declaration with no screenshots root at all still blocks', async () => {
		// The declaration alone is the opt-in, so a repository that says it captures and has
		// captured nothing is the same omission as one that captured the wrong version.
		const root = await makeRoot();
		await declareCapture(root);
		const r = await runGuard(root, tagLine('v3.29.0'));
		expect(r.code).toBe(1);
	...
	test('a screenshots root without the declaration does not block, and says why', async () => {
		// The C7 regression. screenshots/ is gitignored everywhere this guard runs, so a predicate
		// reading it opts in from untracked local state — and four repositories grew that directory
		// for something other than releases. Undeclared never blocks; it reports instead of
		// passing in silence, because it is either an unfinished opt-in or a different directory.
		const root = await makeRoot();
		await mkdir(join(root, 'screenshots'), { recursive: true });
		const r = await runGuard(root, tagLine('v3.29.0'));
		expect(r.code).toBe(0);
		expect(r.err).toContain('.screenshot-capture does not');
	});
	...
	test('this repository declares capture, and the declaration is tracked', async () => {
		// Tracked is the whole point: this repository captures, and an untracked declaration would
		// opt it back out in every fresh clone — which is the state C7 was filed against.
		const repoRoot = resolve(import.meta.dir, '..', '..');
		const p = Bun.spawn(['git', 'ls-files', '--error-unmatch', '.screenshot-capture'], {
			cwd: repoRoot,
			stderr: 'pipe',
			stdout: 'pipe',
			windowsHide: true,
