...
describe('self-contained tool', () => {
	...
	test('rejects references to an external source root in catalog files', async () => {
		const tmp = await testTempDir('aidd-sc-test-');
		try {
			for (const dir of [
				'backend',
	...
	test('rejects documentation dependencies on sibling workspace repositories', async () => {
		const tmp = await testTempDir('aidd-sc-test-');
		try {
			for (const dir of [
				'backend',
			...
			await writeFile(
				join(tmp, 'docs', 'leaks.md'),
				[
					'Read common/fleet-gate-layer-design.md.',
					'Read spernakit/docs/template/STACK.md.',
					'Read <applications-root>/ai/skills/example/SKILL.md.',
				].join('\n'),
			);
			await writeFile(join(tmp, 'audits', 'leaks.md'), 'Read <spernakit-root>/package.json.');

			const { exitCode, stdout } = await runSelfContained(tmp);
			expect(exitCode).toBe(1);
			expect(stdout).toContain('sibling common documentation');
			expect(stdout).toContain('unresolved Spernakit repository path');
			expect(stdout).toContain('unresolved audit root placeholder');
			expect(stdout).toContain('D:\\applications\\ai');
			expect(stdout).toContain('docs/leaks.md');
		} finally {
			await removeTempTree(tmp);
		}
	});
