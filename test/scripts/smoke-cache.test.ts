...
import {
	CI_WORKFLOW_TEST_INPUT,
	STEP_DEPENDENCIES,
} from '../../scripts/lib/smoke-cache/dependencies.ts';
import {
	canSkipStep,
	getSmokeCachePath,
	getSmokeCacheStatus,
...
async function trackSmokeProject(root: string): Promise<void> {
...
describe('smoke cache', () => {
	...
	test('misses fresh-release cache when public baseline content changes', async () => {
		...
		await writeFile(join(root, 'docs', 'CHANGELOG.md'), '## [3.0.1]\n');
	...
	test('misses test cache when a source file read as text changes outside LCOV', async () => {
		const root = await createSmokeProject();
		await writeCoverage(root, ['cli/src/covered.ts']);
		await trackSmokeProject(root);
	...
	test('records a fresh test result with an unstaged tracked deletion', async () => {
		const root = await createSmokeProject();
		await writeCoverage(root, ['cli/src/covered.ts']);
		await trackSmokeProject(root);
		await rm(join(root, 'frontend', 'src', 'pages', 'runs', 'RunsPage.tsx'));
	...
	test('test cache entries without covered files miss once', async () => {
		const root = await createSmokeProject();
		await writeFile(
	...
	test('always runs sibling-repository and local-runtime checks', async () => {
		const root = await createSmokeProject();
		...
		for (const step of [
			...
			await recordStepSuccess(root, step, 25);
			expect(await canSkipStep(root, step)).toBe(false);
		}
	});

	test('caches the leak-guard self-test and invalidates it when the guard changes', async () => {
		const root = await createSmokeProject();
		await mkdir(join(root, '.githooks'), { recursive: true });
		await writeFile(join(root, '.githooks', 'leak-guard.sh'), '# first guard');
		await recordStepSuccess(root, 'check:leak-guard', 25);
		expect(await canSkipStep(root, 'check:leak-guard')).toBe(true);
		await writeFile(join(root, '.githooks', 'leak-guard.sh'), '# changed guard');
		expect(await canSkipStep(root, 'check:leak-guard')).toBe(false);
	});

	test('caches deterministic repository checks', async () => {
		const root = await createSmokeProject();
		...
		for (const step of [
			...
			await recordStepSuccess(root, step, 25);
			expect(await canSkipStep(root, step)).toBe(true);
		}
	});

	// Bun.Glob skips dot-directories unless asked not to, so `.githooks/**/*` and `.github/**/*`
	// once resolved to nothing at all and their steps cached against an empty input set: editing a
	// hook or a workflow left the gate replaying an old pass over it. A pattern that names a
	// dot-directory and matches no file is that failure, not a declaration.
	test('dependency patterns naming a dot-directory resolve to real files', async () => {
		const offenders: string[] = [];

		for (const [step, patterns] of Object.entries(STEP_DEPENDENCIES)) {
			const dotPatterns = patterns.filter(
				(pattern) =>
					pattern.includes('*') &&
					pattern.split('/').some((segment) => segment.startsWith('.')),
			);
			if (dotPatterns.length === 0) continue;

			const resolved = await collectDependencies(process.cwd(), step);
			for (const pattern of dotPatterns) {
				const prefix = pattern.slice(0, pattern.indexOf('*'));
				if (!resolved.some((file) => file.startsWith(prefix))) {
					offenders.push(`${step}: ${pattern}`);
				}
			}
		}
	...
	// collectPrettierDependencies calls getFileInfo WITHOUT resolveConfig, which is only equivalent
	// while no config override assigns a parser. Add an `overrides` block naming an extension
	// prettier does not infer on its own, and those files silently leave format:check's cache key —
	// the step then caches a pass over files it never formatted. Restore resolveConfig if this
	// fails; do not just widen the assertion.
	test('.prettierrc declares no parser overrides', async () => {
		const config = (await Bun.file(join(process.cwd(), '.prettierrc')).json()) as Record<
			string,
			unknown
		>;
	...
	test('writing the smoke cache does not invalidate steps that scan scripts', async () => {
		const root = await createSmokeProject();
		await recordStepSuccess(root, 'check:git-window-hide', 25);

		expect(await canSkipStep(root, 'check:git-window-hide')).toBe(true);
	});
	...
	test('parses smoke wrapper flags and preserves qc step coverage order', () => {
		...
		expect(SMOKE_QC_STEPS.map((step) => step.label)).toEqual([
			...
			'bun test',
			'build:frontend',
			// All three read frontend/dist, so they must stay after the step that produces it.
			...
			'check:shared-core',
			// Last: it reruns the production build under the analysis overlay, so it must not
			// displace the plain build the byte-budget steps above measure.
			'build:analyze',
		]);
