...
describe('printJson', () => {
	...
	test('counts the trailing comma, so the same array breaks unless it is the last entry', () => {
		// Inline, this array's line is exactly printWidth. Prettier measures the whole rendered
		// line, so the comma after a non-final entry is what pushes it over. Both documents were
		// verified against `prettier --write`.
		const brink = ['x'.repeat(87)];
		expect(printJson({ a: 1, k: brink })).toContain('"k": ["xxx');
		expect(printJson({ k: brink, z: 1 })).toContain('"k": [\n');
	});
