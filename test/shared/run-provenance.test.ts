...
describe('aidd run provenance', () => {
	test('normalizes missing driver fields and rejects malformed driver provenance', () => {
		expect(parseAiddRunDriver({})).toEqual({
			driverId: null,
			driverKind: null,
			driverSha256: null,
		});
		...
		expect(parseAiddRunDriver({ driverKind: 'recipe' })).toBeUndefined();
		expect(parseAiddRunDriver({ driverId: 123 })).toBeUndefined();
		expect(parseAiddRunDriver({ driverSha256: 'not-a-sha' })).toBeUndefined();
	});
	...
	test('reads the canonical version and captures a clean checkout revision', async () => {
		...
		try {
			await Bun.write(join(rootDir, 'VERSION'), '3.0.0\n');
