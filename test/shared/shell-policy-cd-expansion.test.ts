...
// `cd "$AIDD_ROOT"` used to pass: the path check saw the literal segment `$AIDD_ROOT`, resolved it
// inside the workspace, and bash then expanded it back out. Every path check after a `cd` is
...
describe('cd/pushd destinations that expand at runtime are denied', () => {
	...
	test('$VAR outside a cd target stays allowed', () => {
		expect(checkBashWorkspacePolicy('echo "$MESSAGE" > notes/out.txt', cwd)).toBeNull();
		expect(checkBashWorkspacePolicy('bun test "$FILTER"', cwd)).toBeNull();
	});
